...
/**
 * The clubs standing behind an event — `app/EventLab/`.
 *
 * One table, and it exists because a competition's clubs arrive in two very
 * different states and the platform only had a name for one of them.
 *
 ...
 *
 * The second one may later BECOME the first: `tenant_id` is filled in when the
 * row is matched to a club that already existed, and `promoted_tenant_id` when
 * a real club is created from it. Both are plain references, never foreign
 * keys, so nothing here can drag a real tenant along behind it.
 *
 * Sandbox-only, like everything else in this module: drop this table, delete
 * `app/EventLab/` and its line in `config/modules.php` and the experiment is
 * gone. `event_id` is the one real thing it points at, and it cascades, so
 * removing a twin removes its clubs with it.
 */
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('sandbox_event_clubs', function (Blueprint $table) {
            $table->id();
    ...
    {
        Schema::dropIfExists('sandbox_event_clubs');
    }
};
