...
class HallScreenController extends Controller
{
    ...
    public function screen(Request $request, string $token)
    {
        ...
        // Phase M3: the React board, when the flag is on. Same data, same
        // stylesheet, same live link — see the note on the console's twin.
        return view(config('features.react_scoreboard')
            ? 'scoreboard::bjj.screen.mat-react'
            : 'scoreboard::bjj.screen.mat', [
            'event' => $event,
            'court' => $device->court,
            ...
            // The prefix the board appends a SLOT to. Built by trimming a
            // placeholder rather than with one, because a slot is a route
            // segment and route() would encode it.
            'audioBase' => \Illuminate\Support\Str::beforeLast(
                route('bjj-screen.audio', [$token, 'x'], false), 'x'
            ),
    ...
    public function overlay(Request $request, string $token)
    {
        ...
        return view('scoreboard::bjj.screen.overlay', [
            'event' => $device->event,
            'court' => $device->court,
            'state' => $state->present(),
            'stateUrl' => route('bjj-scoreboard.state', $token, false),
    ...
    public function pair(Request $request, ClubEvent $event): JsonResponse
    {
        ...
        if (! $device) {
            return response()->json([
                'success' => false,
                'message' => __('scoreboard::bjj_messages.pair_unknown'),
            ], 404);
        }
        ...
        return response()->json([
            'success' => true,
            'message' => __('scoreboard::bjj_messages.pair_done', [
    ...
    public function revokeScreen(Request $request, ClubEvent $event, int $device): JsonResponse
    {
        ...
        return response()->json([
            'success' => true,
            'message' => __('scoreboard::bjj_messages.pair_revoked'),
            'screens' => $this->screensFor($event),
        ]);
    }
    ...
    public function claim(Request $request, string $code)
    {
        ...
        return view('scoreboard::bjj.screen.claim', [
            'device' => $device,
            'code' => $code,
    ...
    public function storeClaim(Request $request, string $code)
    {
        ...
        return redirect()
            ...
            ->with('status', __('scoreboard::bjj_messages.pair_done', [
                'court' => $device->court, 'event' => $event->title,
            ]));
    }

    /**
     * The board's own typefaces, self-hosted.
     *
     * A wall screen in a hall has no internet to ask Google for a font, and a
     * board that falls back to a system face is a board that looks broken from
     * the back of the room. Public on purpose: a face is not data, and the
     * screen that needs it has no session.
     */
    public function font(string $file): \Symfony\Component\HttpFoundation\BinaryFileResponse
    {
        abort_unless(preg_match('/^[a-z0-9-]+\.woff2$/', $file), 404);

        // The faces live beside the board that uses them, in HallScreen/; this
        // file is in Controllers/, one directory across.
        $path = dirname(__DIR__).'/HallScreen/resources/fonts/'.basename($file);
        abort_unless(is_file($path), 404);

        return \Illuminate\Support\Facades\Response::file($path, [
            'Content-Type' => 'font/woff2',
            // Immutable: a face never changes under a given filename.
    ...
    public function claimed(Request $request, ScreenDevice $device)
    {
        ...
        return view('scoreboard::bjj.screen.claimed', ['device' => $device]);
    }
     ...
     */
    private function manageableEvents(\App\Members\Models\User $user)
    {
    ...
    /**
     * One of this event's sounds, for the board that is going to play it.
     *
     * Served from PRIVATE storage through the screen's own token: a club's
    ...
    public function audio(string $token, string $slot)
    {
        ...
        $media = ScreenMedia::slot($device->event, $slot);

        // Nothing uploaded for this slot is a 404, not an error: the board asks
        // for every slot on load and plays the ones that answer.
        ...
        return Storage::disk($media->disk)->response($media->path, null, [
            ...
            // screen never sees that path: it asks for the slot. A long max-age
            // here means an organiser replaces the music and every board in the
            // hall goes on playing yesterday's file until the cache expires —
            // a whole competition, in practice, and reloading the screen does
