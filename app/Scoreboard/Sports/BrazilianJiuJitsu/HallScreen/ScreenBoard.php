...
class ScreenBoard
{
    ...
    private function corner(string $corner, EventMatch $bout, array $entries): array
    {
        ...
        return [
            $corner.'Name' => $bout->{$side.'_name'} ?: __('scoreboard::bjj_messages.court_tbd'),
    ...
    private function stage(EventMatch $bout): string
    {
        ...
        $key = 'scoreboard::bjj_messages.round_'.strtolower(str_replace(' ', '_', $round));
        ...
        if (preg_match('/^Round of (\d+)$/', $round, $m)) {
            return __('scoreboard::bjj_messages.round_of', ['n' => $m[1]]);
        }
