...
namespace App\Scoreboard\Sports\Taekwondo\Mat;
...
class Scoring
{
    ...
    public function apply(ClubEvent $event, string $court, string $command, array $payload = []): MatState
    {
        ...
        if (in_array($command, self::NEEDS_BOUT, true) && ! $state->matchId) {
            throw new \RuntimeException(__('scoreboard::taekwondo_messages.commit_no_bout'));
        }
    ...
    private function awardRound(MatState $state, array $payload): void
    {
        ...
        // can read.
        if ($state->matchOver) {
            throw new \RuntimeException(__('scoreboard::taekwondo_messages.match_over'));
        }
        ...
        if ($state->phase === MatState::PHASE_REST) {
            throw new \RuntimeException(__('scoreboard::taekwondo_messages.round_not_running'));
        }

        $side = $this->side($payload) ?? $state->roundWinner();

        if (! $side) {
            throw new \RuntimeException(__('scoreboard::taekwondo_messages.round_level'));
        }
    ...
    private function commit(ClubEvent $event, string $court, MatState $state): void
    {
        ...
        if (! $winner) {
            throw new \RuntimeException(__('scoreboard::taekwondo_messages.commit_undecided'));
        }
