...
namespace App\Scoreboard\Sports\Taekwondo\HallScreen;
...
{
    /*
     * `taekwondo:court-pair`, not `court:pair`.
     *
     * Karate's copy of this class declares the same signature, and Artisan
     * keeps one command per name — so THIS one lost the registration and was
     * unreachable. Everything it does went to the karate fleet instead,
     * silently: `court:pair --prune`, including the nightly schedule, swept
     * karate_court_displays and never touched a Taekwondo screen, so this
     * package's open enrol endpoint had no counterweight at all.
     *
     * Renamed on the SHADOWED side only. `court:pair` still means exactly what
     * it has been meaning — Karate — so nothing that was working changes; this
     * name simply exists now.
     */
    protected $signature = 'taekwondo:court-pair
