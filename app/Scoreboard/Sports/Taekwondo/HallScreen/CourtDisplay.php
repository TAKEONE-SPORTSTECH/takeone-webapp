...
class CourtDisplay
{
    ...
    private function corner(string $corner, EventMatch $bout, array $entries, ClubEvent $event): array
    {
        ...
        return [
            $corner.'Name' => $bout->{$side.'_name'} ?: __('scoreboard::taekwondo_messages.court_tbd'),
    ...
    private function stage(EventMatch $bout): string
    {
        ...
        $key = 'scoreboard::taekwondo_messages.round_'.strtolower(str_replace(' ', '_', $round));
        ...
        if (preg_match('/^Round of (\d+)$/', $round, $m)) {
            return __('scoreboard::taekwondo_messages.round_of', ['n' => $m[1]]);
        }
