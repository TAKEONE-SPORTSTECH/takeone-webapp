...
class CourtDisplayController extends Controller
{
    ...
    public function preview(Request $request, ClubEvent $event, string $court)
    {
        ...
        return view('scoreboard::taekwondo.screen.board', [
    ...
    public function board(Request $request, string $token)
    {
        ...
        if ($surface === 'bout' || ($surface === null && $state->mode !== MatState::MODE_UPCOMING)) {
            return view('scoreboard::taekwondo.scoreboard.mat', [
                'event' => $device->event,
                'court' => $device->court,
                'state' => $state->toArray(),
    ...
    {
        return view('scoreboard::taekwondo.screen.screen');
    }
    ...
    public function claim(Request $request, string $code)
    {
        ...
        return view('scoreboard::taekwondo.screen.claim', [
            'device' => $device,
            'code' => $code,
    ...
    public function storeClaim(Request $request, string $code)
    {
        ...
        return redirect()
            ...
            ->with('status', __('scoreboard::taekwondo_messages.pair_done', [
    ...
    public function pair(Request $request, ClubEvent $event)
    {
        ...
        if (! $device) {
            return response()->json([
                'success' => false,
                'message' => __('scoreboard::taekwondo_messages.pair_unknown'),
            ], 404);
        }
        ...
        return response()->json([
            'success' => true,
            'message' => __('scoreboard::taekwondo_messages.pair_done', [
                'court' => $device->court, 'event' => $event->title,
            ]),
     ...
     * belongs, on `taekwondo:court-pair --revoke`, for a device that is lost or
     * stolen.
     */
    public function revokeScreen(Request $request, ClubEvent $event, int $device)
    {
        ...
        return response()->json([
            'success' => true,
            'message' => __('scoreboard::taekwondo_messages.pair_revoked'),
            'screens' => $this->screensFor($event),
        ]);
    }
    ...
    public function claimed(Request $request, CourtDisplayDevice $device)
    {
        ...
        return view('scoreboard::taekwondo.screen.claimed', ['device' => $device]);
    }
