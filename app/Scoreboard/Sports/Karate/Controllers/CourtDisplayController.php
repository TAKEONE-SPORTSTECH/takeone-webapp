...
use App\Scoreboard\Sports\Karate\HallScreen\CourtDisplay;
...
class CourtDisplayController extends Controller
{
    ...
    public function preview(Request $request, ClubEvent $event, string $court)
    {
        ...
        return view('scoreboard::karate.screen.board', [
    ...
    public function board(Request $request, string $token)
    {
        ...
        if ($surface === 'bout' || ($surface === null && $state->mode !== MatState::MODE_UPCOMING)) {
            return view('scoreboard::karate.scoreboard.mat', [
                'event' => $device->event,
                'court' => $device->court,
                'state' => $state->toArray(),
    ...
    {
        return view('scoreboard::karate.screen.screen');
    }
    ...
    public function claim(Request $request, string $code)
    {
        ...
        return view('scoreboard::karate.screen.claim', [
            'device' => $device,
            'code' => $code,
    ...
    public function storeClaim(Request $request, string $code)
    {
        ...
        // render Karate's scoreboard for an event that is not Karate. The
        ...
        return redirect()
            ...
            ->with('status', __('scoreboard::karate_messages.pair_done', [
    ...
    public function pair(Request $request, ClubEvent $event)
    {
        ...
        if (! $device) {
            return response()->json([
                'success' => false,
                'message' => __('scoreboard::karate_messages.pair_unknown'),
            ], 404);
        }
        ...
        return response()->json([
            'success' => true,
            'message' => __('scoreboard::karate_messages.pair_done', [
                'court' => $device->court, 'event' => $event->title,
            ]),
    ...
    public function revokeScreen(Request $request, ClubEvent $event, int $device)
    {
        ...
        return response()->json([
            'success' => true,
            'message' => __('scoreboard::karate_messages.pair_revoked'),
            'screens' => $this->screensFor($event),
        ]);
    }
    ...
    public function claimed(Request $request, CourtDisplayDevice $device)
    {
        ...
        return view('scoreboard::karate.screen.claimed', ['device' => $device]);
    }
    ...
    private function manageableEvents(\App\Members\Models\User $user)
    {
        ...
        return ClubEvent::query()
            ...
            ->limit(50)
            ->get()
            ->filter(fn (ClubEvent $e) => $e->sport === 'karate' && $access->canManage($e, $user))
            ->map(fn (ClubEvent $e) => [
                'uuid' => $e->uuid,
                'title' => $e->title,
    ...
    public function font(string $file): BinaryFileResponse
    {
        ...
        // file is in Controllers/, one directory across. Spelled out rather than
        // relative-to-__DIR__ because a controller that silently 404s every font
        // is a board that renders in a fallback typeface and looks broken from
        // the back of a hall.
