...
<title>{{ __('scoreboard::taekwondo_messages.ctl_title') }} · {{ $court }} · {{ $event->title }}</title>
...
@include('scoreboard::taekwondo.scoreboard.winner-celebration')
</head>
<body>
...
<div class="card" style="border:1px solid oklch(0.85 0.16 85 / 0.5); animation:headerIn 0.5s cubic-bezier(0.22,1,0.36,1) both;">
  <div style="display:flex; align-items:center; gap:18px; padding:14px 22px; border-bottom:1px solid var(--line-soft); flex-wrap:wrap;">
    <div style="flex:1 1 320px; min-width:260px; display:flex; flex-direction:column; gap:2px;">
      ...
      <div style="font-weight:700; font-size:16px; letter-spacing:0.24em; text-transform:uppercase; color:oklch(0.85 0.16 85);">
        {{ __('scoreboard::taekwondo_messages.ctl_title') }}
        <span style="color:rgba(232,230,224,0.45);">· {{ $screens ? __('scoreboard::taekwondo_messages.ctl_screens', ['count' => $screens]) : __('scoreboard::taekwondo_messages.ctl_no_screens') }}</span>
      </div>
    </div>
    <div style="display:flex; gap:10px; flex:0 1 auto; flex-wrap:wrap;">
      <button id="btnRefresh" class="btn">{{ __('scoreboard::taekwondo_messages.ctl_refresh_vs') }}</button>
      <button id="btnQueue" class="btn">{{ __('scoreboard::taekwondo_messages.ctl_queue') }}</button>
      {{-- The lenses pointed at this mat. Only when this door was given the
           camera addresses — a console with no camera surface shows no
           camera button. --}}
      @if ($cameraUrl ?? null)
        <button id="btnCameras" class="btn">{{ __('events.mat_cameras_tab') }}</button>
      @endif
      ...
      @endisset
      <button id="btnUndo" class="btn">{{ __('scoreboard::taekwondo_messages.ctl_undo') }}</button>
      <button id="btnCommit" class="btn btn-go">{{ __('scoreboard::taekwondo_messages.ctl_end_upload') }}</button>
      <button id="btnResetMatch" class="btn btn-danger">{{ __('scoreboard::taekwondo_messages.ctl_reset_match') }}</button>
      <button id="btnClear" class="btn btn-danger">{{ __('scoreboard::taekwondo_messages.ctl_clear_mat') }}</button>
    </div>
  </div>
  <div style="display:flex; align-items:center; padding:10px 22px; gap:0; flex-wrap:wrap;">
    <div style="display:flex; align-items:baseline; gap:8px; padding-right:22px; border-right:1px solid rgba(255,255,255,0.15);">
      <span class="meta-k">{{ __('scoreboard::taekwondo_messages.ctl_court') }}</span><span class="meta-v" id="mCourt"></span>
    </div>
    <div style="display:flex; align-items:baseline; gap:8px; padding:0 22px; border-right:1px solid rgba(255,255,255,0.15);">
      <span class="meta-k">{{ __('scoreboard::taekwondo_messages.ctl_match') }}</span><span class="meta-v" id="mNo"></span>
    </div>
    <div style="display:flex; align-items:baseline; gap:8px; padding:0 22px; border-right:1px solid rgba(255,255,255,0.15);">
      <span class="meta-k">{{ __('scoreboard::taekwondo_messages.ctl_class') }}</span>
    ...
    <div style="display:flex; align-items:baseline; gap:8px; padding:0 22px;">
      <span class="meta-k">{{ __('scoreboard::taekwondo_messages.ctl_category') }}</span>
...
{{-- ── The cameras on this mat, as a sheet ──────────────────────────────────
     Same shell as the queue's, because it is the same kind of thing: something
     read at the table between bouts. Everything inside it is the shared,
     sport-neutral panel — see resources/views/partials/mat-cameras.blade.php. --}}
@if ($cameraUrl ?? null)
<div id="camerasModal" hidden class="scrim" style="z-index:50;">
  <div class="sheet" style="width:980px; max-height:900px; overflow-y:auto;">
    <div class="sheet-head">
      <div class="sheet-title">{{ __('events.mat_cameras_title', ['court' => $court]) }}</div>
      <button id="camerasClose" class="btn sheet-close">✕</button>
    </div>
  ...
  <div class="sheet" style="width:820px; max-height:900px; overflow-y:auto;">
    <div class="sheet-head">
      <div style="display:flex; flex-direction:column; gap:2px; flex:1; min-width:0;">
        <div class="sheet-title">{{ __('scoreboard::taekwondo_messages.ctl_queue') }}</div>
        <div style="font-weight:600; font-size:14px; letter-spacing:0.06em; text-transform:uppercase; color:var(--muted);">{{ __('scoreboard::taekwondo_messages.ctl_queue_hint') }}</div>
      </div>
    ...
    <div style="display:flex; align-items:center; gap:12px; flex-wrap:wrap; border-bottom:1px solid var(--line-soft); padding-bottom:16px;">
      ...
      <div style="display:flex; align-items:center; gap:8px; flex:1;">
        <span class="meta-k">{{ __('scoreboard::taekwondo_messages.ctl_round') }}</span>
      ...
      <button id="btnNext" class="btn btn-go">{{ __('scoreboard::taekwondo_messages.ctl_start_next') }}</button>
    </div>
...
<div style="display:flex; gap:14px; flex-wrap:wrap;">
  <div class="card" style="flex:1; min-width:340px; display:flex; align-items:center; justify-content:center; gap:26px; border:2px solid oklch(0.85 0.16 85 / 0.7); padding:14px 26px;">
    ...
    <div style="display:flex; flex-direction:column; gap:8px;">
      ...
      <div style="display:flex; gap:8px;">
        <button id="btnRest" class="btn" style="flex:1; padding:9px 12px; font-size:17px;">{{ __('scoreboard::taekwondo_messages.ctl_rest') }} 1:00</button>
        <button id="btnResetRound" class="btn" style="flex:1; padding:9px 12px; font-size:17px;">{{ __('scoreboard::taekwondo_messages.ctl_reset_round') }}</button>
      </div>
    </div>
  </div>
  ...
  <div class="card" style="flex:1; min-width:340px; display:flex; align-items:center; justify-content:center; gap:22px; border:1px solid var(--line); padding:14px 26px;">
    <div style="display:flex; flex-direction:column; align-items:center; gap:6px;">
      <div style="font-weight:600; font-size:16px; letter-spacing:0.3em; text-transform:uppercase; color:rgba(232,230,224,0.55);">{{ __('scoreboard::taekwondo_messages.ctl_round') }}</div>
      ...
      <button id="btnAwardScore" class="btn" style="padding:8px 16px; font-size:15px;">{{ __('scoreboard::taekwondo_messages.ctl_award_round') }}</button>
    </div>
    <button id="btnGolden" class="btn" style="font-weight:800; font-size:20px; padding:14px 20px; border:2px solid oklch(0.85 0.16 85);">{{ __('scoreboard::taekwondo_messages.ctl_golden') }}</button>
  </div>
</div>
...
<div id="cropModal" hidden class="scrim" style="background:rgba(0,0,0,0.78); z-index:55;">
  <div class="sheet" style="padding:22px 26px 24px; gap:16px;">
    ...
    <div class="sheet-head">
      ...
      <div id="cropTitle" class="sheet-title">{{ __('scoreboard::taekwondo_messages.ctl_crop_title') }}</div>
    ...
    <div style="display:flex; align-items:center; gap:12px;">
      <div class="meta-k" style="flex:0 0 auto;">{{ __('scoreboard::taekwondo_messages.ctl_crop_zoom') }}</div>
    ...
    <div style="display:flex; align-items:center; gap:12px;">
      <div class="meta-k" style="flex:1; letter-spacing:0.14em;">{{ __('scoreboard::taekwondo_messages.ctl_crop_hint') }}</div>
      <button id="cropCancel" class="btn" style="padding:13px 22px;">{{ __('scoreboard::taekwondo_messages.ctl_crop_cancel') }}</button>
      <button id="cropSave" class="btn btn-go" style="padding:13px 30px;">{{ __('scoreboard::taekwondo_messages.ctl_crop_save') }}</button>
    </div>
  </div>
</div>
...
<div id="winnerOverlay" hidden style="position:absolute; inset:0; z-index:40; overflow:hidden;">
  ...
  <div id="winnerControls" style="display:flex; flex-direction:column; gap:10px;">
    <div style="display:flex; gap:14px; align-items:center; flex-wrap:wrap;">
      ...
      <button id="winnerBack" class="btn" style="font-size:24px; font-weight:800; padding:17px 39px; background:linear-gradient(135deg,#3c4460,#1b1f2e); color:#eef1f8; border-color:rgba(255,255,255,0.38); box-shadow:0 12px 34px rgba(0,0,0,0.5);">{{ __('scoreboard::taekwondo_messages.ctl_winner_back') }}</button>
    </div>
...
<div id="undoModal" hidden class="scrim" style="z-index:50;">
  <div class="sheet" style="width:560px; max-height:900px; overflow-y:auto;">
    <div class="sheet-head">
      <div class="sheet-title">{{ __('scoreboard::taekwondo_messages.ctl_undo_title') }}</div>
      <div style="flex:1;"></div>
      <button id="undoClose" class="btn sheet-close">✕</button>
    </div>
    <div id="undoList" style="display:flex; flex-direction:column; gap:8px;"></div>
    <button id="undoDo" class="btn btn-go" style="padding:13px;">{{ __('scoreboard::taekwondo_messages.ctl_undo') }}</button>
  </div>
</div>
...
(function () {
  ...
  var GAM_LIMIT = @json(\App\Scoreboard\Sports\Taekwondo\Mat\MatState::GAM_JEOM_LIMIT);
  var TBD = @json(__('scoreboard::taekwondo_messages.court_tbd'));
  var T = {
    start: @json(__('scoreboard::taekwondo_messages.ctl_start')),
    pause: @json(__('scoreboard::taekwondo_messages.ctl_pause')),
    waiting: @json(__('scoreboard::taekwondo_messages.ctl_waiting')),
    none: @json(__('scoreboard::taekwondo_messages.ctl_undo_none')),
    vs: @json(__('scoreboard::taekwondo_messages.ctl_vs')),
    hong: @json(__('scoreboard::taekwondo_messages.sb_hong')),
    chung: @json(__('scoreboard::taekwondo_messages.sb_chung')),
    gamjeom: @json(__('scoreboard::taekwondo_messages.ctl_gamjeom')),
    gamhint: @json(__('scoreboard::taekwondo_messages.ctl_gamjeom_hint')),
    winner: @json(__('scoreboard::taekwondo_messages.sb_winner')),
    byRounds: @json(__('scoreboard::taekwondo_messages.ctl_won_rounds')),
    byPun: @json(__('scoreboard::taekwondo_messages.ctl_won_pun')),
    byGolden: @json(__('scoreboard::taekwondo_messages.ctl_won_golden')),
    nextIs: @json(__('scoreboard::taekwondo_messages.ctl_next_is')),
    recordNext: @json(__('scoreboard::taekwondo_messages.ctl_record_next')),
    recordLast: @json(__('scoreboard::taekwondo_messages.ctl_record_last')),
    photoAdd: @json(__('scoreboard::taekwondo_messages.ctl_photo_add')),
    photoNoEntry: @json(__('scoreboard::taekwondo_messages.ctl_photo_no_entry')),
    photoBig: @json(__('scoreboard::taekwondo_messages.ctl_photo_big')),
    photoBad: @json(__('scoreboard::taekwondo_messages.ctl_photo_bad')),
    photoChange: @json(__('scoreboard::taekwondo_messages.ctl_photo_change')),
    crestAdd: @json(__('scoreboard::taekwondo_messages.ctl_crest_add')),
    crestChange: @json(__('scoreboard::taekwondo_messages.ctl_crest_change')),
    crestSaved: @json(__('scoreboard::taekwondo_messages.ctl_crest_saved')),
    cropTitle: @json(__('scoreboard::taekwondo_messages.ctl_crop_title')),
    crestTitle: @json(__('scoreboard::taekwondo_messages.ctl_crest_title')),
    photoSaved: @json(__('scoreboard::taekwondo_messages.ctl_photo_saved')),
    refreshed: @json(__('scoreboard::taekwondo_messages.ctl_refreshed'))
  };
  var ACTIONS = [
    { key: 'punch', n: 1, label: @json(__('scoreboard::taekwondo_messages.ctl_punch')) },
    { key: 'body', n: 2, label: @json(__('scoreboard::taekwondo_messages.ctl_body')) },
    { key: 'head', n: 3, label: @json(__('scoreboard::taekwondo_messages.ctl_head')) },
    { key: 'turn_body', n: 4, label: @json(__('scoreboard::taekwondo_messages.ctl_turn_body')) },
    { key: 'turn_head', n: 5, label: @json(__('scoreboard::taekwondo_messages.ctl_turn_head')) }
  ];
  ...
  function paintQueue() {
    ...
    QUEUE.forEach(function (q) {
      ...
      btn.textContent = @json(__('scoreboard::taekwondo_messages.ctl_load'));
  ...
  // The camera sheet. Rendered only when this door has camera addresses, so
  // every handler is guarded rather than assumed.
  if (el('btnCameras')) {
    el('btnCameras').onclick = function () { el('camerasModal').hidden = false; };
    el('camerasClose').onclick = function () { el('camerasModal').hidden = true; };
    el('camerasModal').onclick = function (e) { if (e.target === el('camerasModal')) el('camerasModal').hidden = true; };
  }
  ...
  document.addEventListener('keydown', function (e) {
    ...
    if (el('camerasModal')) el('camerasModal').hidden = true;
