...
<title>{{ __('scoreboard::taekwondo_messages.sb_page_title', ['court' => $court, 'event' => $event->title]) }}</title>
...
<div id="root"><div id="stage">
  ...
  {{-- ── Waiting ────────────────────────────────────────────────────────── --}}
  <div id="idle">
    <div class="t">{{ __('scoreboard::taekwondo_messages.court_idle_title') }}</div>
    <div class="s">{{ __('scoreboard::taekwondo_messages.court_idle_sub') }}</div>
  </div>
  ...
  <div id="sb" class="layer" hidden style="background:repeating-linear-gradient(115deg, #0b0b10 0 6px, #0d0d13 6px 12px); font-family:'Barlow Condensed',sans-serif; color:#e8e6e0; overflow:hidden;">
    ...
    <div style="position:absolute; top:0; left:0; right:0; height:110px; display:flex; align-items:center; background:rgba(6,6,9,0.95); border-bottom:2px solid oklch(0.85 0.16 85 / 0.6); z-index:6; animation:topIn 0.6s cubic-bezier(0.22,1,0.36,1) both;">
      <div style="display:flex; gap:12px; padding:0; width:300px; flex:0 0 auto; justify-content:center; border-right:1px solid rgba(255,255,255,0.15); height:100%; align-items:center;">
        <span style="font-weight:700; font-size:30px; letter-spacing:0.22em; color:rgba(232,230,224,0.6); text-transform:uppercase;">{{ __('scoreboard::taekwondo_messages.court_match') }}</span>
      ...
      <div style="display:flex; align-items:center; gap:12px; padding:0; width:300px; flex:0 0 auto; justify-content:center; border-left:1px solid rgba(255,255,255,0.15); height:100%;">
        <span style="font-weight:700; font-size:30px; letter-spacing:0.22em; color:rgba(232,230,224,0.6); text-transform:uppercase;">{{ __('scoreboard::taekwondo_messages.sb_court') }}</span>
    ...
    <div style="position:absolute; top:200px; left:0; right:0; height:570px; display:grid; grid-template-columns:1fr 500px 1fr; align-items:stretch; z-index:5;">
      <div style="position:relative; margin:0 -50px 0 0; background:linear-gradient(155deg, oklch(0.55 0.21 25) 0%, oklch(0.34 0.13 25) 100%); clip-path:polygon(0 0, 100% 0, calc(100% - 90px) 100%, 0 100%); display:flex; flex-direction:column; align-items:center; justify-content:center; padding-right:60px; box-shadow:0 30px 80px rgba(0,0,0,0.7), 0 0 60px oklch(0.6 0.22 25 / 0.35); animation:cardInL 0.8s 0.25s cubic-bezier(0.22,1,0.36,1) both;">
        <div style="font-weight:800; font-size:34px; letter-spacing:0.4em; text-transform:uppercase; color:rgba(255,255,255,0.75); margin-bottom:14px;">{{ __('scoreboard::taekwondo_messages.sb_hong') }}</div>
        ...
        <div style="display:flex; align-items:center; gap:14px; margin-top:18px;">
          <span style="font-weight:700; font-size:26px; letter-spacing:0.24em; text-transform:uppercase; color:rgba(255,255,255,0.7);">{{ __('scoreboard::taekwondo_messages.sb_gamjeom') }}</span>
      ...
      <div style="position:relative; margin:0 0 0 -50px; background:linear-gradient(205deg, oklch(0.5 0.18 255) 0%, oklch(0.3 0.11 255) 100%); clip-path:polygon(0 0, 100% 0, 100% 100%, 90px 100%); display:flex; flex-direction:column; align-items:center; justify-content:center; padding-left:60px; box-shadow:0 30px 80px rgba(0,0,0,0.7), 0 0 60px oklch(0.52 0.19 255 / 0.35); animation:cardInR 0.8s 0.4s cubic-bezier(0.22,1,0.36,1) both;">
        <div style="font-weight:800; font-size:34px; letter-spacing:0.4em; text-transform:uppercase; color:rgba(255,255,255,0.75); margin-bottom:14px;">{{ __('scoreboard::taekwondo_messages.sb_chung') }}</div>
  ...
  <div id="vs" class="layer" hidden style="z-index:12; background:radial-gradient(120% 90% at 50% 40%, #16161f 0%, #0a0a0e 65%, #050507 100%); font-family:'Barlow Condensed',sans-serif; color:#e8e6e0; overflow:hidden;">
    ...
    <div style="position:absolute; left:43.2px; bottom:118.8px; display:flex; flex-direction:column; align-items:flex-start; gap:13px; z-index:6; max-width:44%; min-width:0; animation:riseUp 0.8s 0.7s cubic-bezier(0.22,1,0.36,1) both;">
      <div style="font-weight:800; font-size:21.6px; letter-spacing:0.35em; color:#fff; background:#c8382c;background:oklch(0.55 0.2 25); padding:5.4px 15.1px 5.4px 18.9px;">{{ __('scoreboard::taekwondo_messages.sb_hong') }}</div>
    ...
    <div style="position:absolute; right:43.2px; bottom:118.8px; display:flex; flex-direction:column; align-items:flex-end; gap:13px; z-index:6; max-width:44%; min-width:0; text-align:right; animation:riseUp 0.8s 0.85s cubic-bezier(0.22,1,0.36,1) both;">
      <div style="font-weight:800; font-size:21.6px; letter-spacing:0.35em; color:#fff; background:#1f5aa8;background:oklch(0.5 0.16 255); padding:5.4px 15.1px 5.4px 18.9px;">{{ __('scoreboard::taekwondo_messages.sb_chung') }}</div>
    ...
    <div style="position:absolute; bottom:32.4px; left:50%; transform:translateX(-50%); display:flex; gap:17.3px; z-index:8; align-items:center; flex-wrap:wrap; justify-content:center; max-width:94%; animation:riseC 0.8s 1.3s cubic-bezier(0.22,1,0.36,1) both;">
      <div style="display:flex; align-items:baseline; gap:8.6px; background:rgba(10,10,14,0.72); border:1px solid rgba(253,196,54,0.45); border:1px solid oklch(0.85 0.16 85 / 0.45); padding:10.8px 23.8px; backdrop-filter:blur(6px);">
        <span style="font-weight:600; font-size:23.8px; letter-spacing:0.3em; color:rgba(232,230,224,0.65); text-transform:uppercase;">{{ __('scoreboard::taekwondo_messages.court_match') }}</span>
      ...
      <div style="display:flex; align-items:baseline; gap:8.6px; background:rgba(10,10,14,0.72); border:1px solid rgba(253,196,54,0.45); border:1px solid oklch(0.85 0.16 85 / 0.45); padding:10.8px 23.8px; backdrop-filter:blur(6px);">
        <span style="font-weight:600; font-size:23.8px; letter-spacing:0.3em; color:rgba(232,230,224,0.65); text-transform:uppercase;">{{ __('scoreboard::taekwondo_messages.court_court') }}</span>
      ...
      <div id="vsRefWrap" style="display:flex; align-items:baseline; gap:8.6px; background:rgba(10,10,14,0.72); border:1px solid rgba(253,196,54,0.45); border:1px solid oklch(0.85 0.16 85 / 0.45); padding:10.8px 23.8px; backdrop-filter:blur(6px);">
        <span style="font-weight:600; font-size:23.8px; letter-spacing:0.3em; color:rgba(232,230,224,0.65); text-transform:uppercase;">{{ __('scoreboard::taekwondo_messages.vs_referee') }}</span>
        <span id="vsReferee" style="font-weight:700; font-size:28.1px; letter-spacing:0.08em; color:#fff; text-transform:uppercase;"></span>
      </div>
    </div>
  </div>

  <div id="stale"><span>{{ __('scoreboard::taekwondo_messages.court_reconnecting') }}</span></div>
</div></div>
...
(function () {
  ...
  var T = {
    time: @json(__('scoreboard::taekwondo_messages.sb_time')),
    start: @json(__('scoreboard::taekwondo_messages.sb_start')),
    stop: @json(__('scoreboard::taekwondo_messages.sb_stop')),
    rest: @json(__('scoreboard::taekwondo_messages.sb_rest')),
    golden: @json(__('scoreboard::taekwondo_messages.sb_golden')),
    round: @json(__('scoreboard::taekwondo_messages.sb_round')),
...
@isset($screenLink)
@include('scoreboard::taekwondo.screen.partials.screen-link')
@endisset
</body>
</html>
