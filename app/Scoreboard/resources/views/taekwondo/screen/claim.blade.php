...
--}}
<div class="px-4 sm:px-6 lg:px-8 py-6 max-w-2xl mx-auto" x-data="{ picked: null, court: null, surface: 'follow' }">
    ...
    <div class="flex items-center gap-3 mb-1">
        ...
        <div>
            <h1 class="text-xl font-bold text-gray-900">{{ __('scoreboard::taekwondo_messages.claim_title') }}</h1>
            <p class="text-sm text-muted-foreground">
                {{ __('scoreboard::taekwondo_messages.claim_sub', ['code' => $code]) }}
            </p>
        </div>
    </div>
    ...
    @if ($events->isEmpty())
        <div class="mt-6 bg-white rounded-xl shadow-sm border border-gray-100 p-8 text-center">
            ...
            <p class="mt-3 text-sm text-muted-foreground">{{ __('scoreboard::taekwondo_messages.claim_no_events') }}</p>
        </div>
    @else
        <form method="POST" action="{{ route('court-display.claim.store', $code) }}" class="mt-6 space-y-6">
                            ...
                            <div class="mt-3 flex flex-wrap gap-2" x-show="picked === '{{ $event['uuid'] }}'" x-cloak
                                ...
                                @empty
                                    <p class="text-xs text-muted-foreground">
                                        {{ __('scoreboard::taekwondo_messages.claim_no_courts') }}
                                    </p>
                                @endforelse
                            </div>
                        </div>
            ...
            {{-- What this screen is FOR.
                 ...
                 Asked here, at the moment of pairing, because this is the only
                 moment anybody is looking at the screen and the person holding
                 the phone knows where it is hanging. A television has no URL
                 bar to add `?surface=` to, so the answer is stored on the
                 device and every later load honours it.

                 Selection cards rather than a dropdown: three known options,
                 tapped on a phone, in a scrolling sheet — Mobile Pattern
                 Language §3. --}}
            <div x-show="picked && court" x-cloak
                ...
                <p class="text-sm font-medium text-gray-700 mb-2">
                    {{ __('scoreboard::taekwondo_messages.claim_surface') }}
                </p>
                        ...
                        <label class="flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-colors"
                            ...
                            <span class="min-w-0 flex-1">
                                <span class="block text-sm font-medium text-gray-900">
                                    {{ __('scoreboard::taekwondo_messages.'.$label) }}
                                </span>
                                <span class="block text-xs text-muted-foreground">
                                    {{ __('scoreboard::taekwondo_messages.'.$hint) }}
                                </span>
                            </span>
            ...
            <button type="submit"
                ...
                <i class="bi bi-display mr-2"></i>{{ __('scoreboard::taekwondo_messages.claim_submit') }}
            </button>
        </form>
    @endif
</div>
