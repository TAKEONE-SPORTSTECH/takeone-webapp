...
<div id="root"><div id="stage">
  ...
  {{-- ── Waiting ────────────────────────────────────────────────────────── --}}
  <div id="idle">
    <div class="t">{{ __('scoreboard::karate_messages.court_idle_title') }}</div>
    <div class="s">{{ __('scoreboard::karate_messages.court_idle_sub') }}</div>
  </div>
  ...
  <div id="sb" class="layer" hidden style="background:#000; display:flex; flex-direction:column;">
    <div style="height:110px; background:#0a0b10; display:flex; align-items:center; justify-content:space-between; padding:0 48px; border-bottom:3px solid #1c1e28; animation:introDrop .6s cubic-bezier(.2,.8,.2,1) both;">
      ...
      <div style="display:flex; align-items:center; gap:36px; font-size:36px; font-weight:700; text-transform:uppercase;">
        <div style="display:flex; align-items:baseline; gap:12px;"><span style="color:#7d8296; font-size:26px; letter-spacing:.18em;">{{ __('scoreboard::karate_messages.court_match') }}</span><span id="sbMatchNo" style="font-family:'Anton',sans-serif;"></span></div>
        <div style="display:flex; align-items:baseline; gap:12px;"><span style="color:#7d8296; font-size:26px; letter-spacing:.18em;">{{ __('scoreboard::karate_messages.sb_tatami') }}</span><span id="sbCourt" style="font-family:'Anton',sans-serif;"></span></div>
  ...
  <div id="vs" class="layer" hidden style="z-index:12; background:radial-gradient(120% 90% at 50% 40%, #16161f 0%, #0a0a0e 65%, #050507 100%); font-family:'Barlow Condensed',sans-serif; color:#e8e6e0; overflow:hidden;">
    ...
    <div style="position:absolute; bottom:32.4px; left:50%; transform:translateX(-50%); display:flex; gap:17.3px; z-index:8; align-items:center; flex-wrap:wrap; justify-content:center; max-width:94%; animation:riseC 0.8s 1.3s cubic-bezier(0.22,1,0.36,1) both;">
      <div style="display:flex; align-items:baseline; gap:8.6px; background:rgba(10,10,14,0.72); border:1px solid rgba(253,196,54,0.45); border:1px solid oklch(0.85 0.16 85 / 0.45); padding:10.8px 23.8px; backdrop-filter:blur(6px);">
        <span style="font-weight:600; font-size:23.8px; letter-spacing:0.3em; color:rgba(232,230,224,0.65); text-transform:uppercase;">{{ __('scoreboard::karate_messages.court_match') }}</span>
      ...
      <div style="display:flex; align-items:baseline; gap:8.6px; background:rgba(10,10,14,0.72); border:1px solid rgba(253,196,54,0.45); border:1px solid oklch(0.85 0.16 85 / 0.45); padding:10.8px 23.8px; backdrop-filter:blur(6px);">
        <span style="font-weight:600; font-size:23.8px; letter-spacing:0.3em; color:rgba(232,230,224,0.65); text-transform:uppercase;">{{ __('scoreboard::karate_messages.court_court') }}</span>
      ...
      <div id="vsRefWrap" style="display:flex; align-items:baseline; gap:8.6px; background:rgba(10,10,14,0.72); border:1px solid rgba(253,196,54,0.45); border:1px solid oklch(0.85 0.16 85 / 0.45); padding:10.8px 23.8px; backdrop-filter:blur(6px);">
        <span style="font-weight:600; font-size:23.8px; letter-spacing:0.3em; color:rgba(232,230,224,0.65); text-transform:uppercase;">{{ __('scoreboard::karate_messages.vs_referee') }}</span>
        <span id="vsReferee" style="font-weight:700; font-size:28.1px; letter-spacing:0.08em; color:#fff; text-transform:uppercase;"></span>
      </div>
    </div>
  </div>

  <div id="stale"><span>{{ __('scoreboard::karate_messages.court_reconnecting') }}</span></div>
</div></div>
...
(function () {
  ...
  var CALLOUTS = @json(\App\Scoreboard\Sports\Karate\Mat\MatState::CALLOUTS);
  ...
  function paintClock() {
    ...
    text('sbStatus', over ? @json(__('scoreboard::karate_messages.sb_time'))
                          : (S.running ? @json(__('scoreboard::karate_messages.sb_hajime'))
                                       : @json(__('scoreboard::karate_messages.sb_yame'))));
  }
  ...
  var WINNER_LABEL = @json(__('scoreboard::karate_messages.sb_winner'));
