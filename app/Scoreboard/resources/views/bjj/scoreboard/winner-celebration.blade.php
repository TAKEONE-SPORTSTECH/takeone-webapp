{{--
    ...
    Transcribed from the approved layouts in ../../Sports/Karate/design/winner-{red,blue}.source.html, whose two halves
    this package reuses as BLUE and WHITE: BLUE puts the photograph on the right
    and the name on the left, WHITE mirrors it. Treat the visual output as fixed and restyle by agreement, never as a
    side effect.

    It belongs to Brazilian Jiu-Jitsu. Karate and Taekwondo each have their own copy of this file
    ...
    own buttons in.
        ...
        var scene = WinnerCelebration.paint(hostEl, {
            corner: 'blue',           // 'blue' | 'white' — which corner won
            name:   'FAWZIA ABDULLA',
            club:   'KARATE CLUB',
...
--}}
@once('bjj-winner-celebration-styles')
<style>
@php
...
@font-face {
  ...
  src: url("{{ route('bjj-screen.font', 'poppins-italic-'.$wcWeight.'-'.$wcSubset.'.woff2', false) }}") format('woff2');
...
@once('bjj-winner-celebration-script')
<script>
...
(function () {
  ...
  var CORNERS = {
    /* Blue is the corner the design's RED half was drawn for — the photograph
       on the right, the name across the left — because in jiu-jitsu blue is the
       left corner on every surface of this package. Only the palette changed. */
    blue: {
      a: '#1677FF', b: '#38BDF8', glow: 'rgba(46,160,255,.6)',
      confetti: ['#38BDF8', '#1677FF', '#bfe6ff', '#8ad4ff', '#5aa9ff', '#d6f1ff'],
      flame: ['#3d9bff', '#2ee6ff', '#6b9dff'], base: 'rgba(46,155,255,.32)',
      core: '#ffffff', spread: '45%', fade: '75%'
    },
    /* White is the other corner — never red, in this sport. Its sky is the
       one an ice-white gi reads against: silver and warm white rather than a
       second blue, so the two celebrations are told apart from the back of a
       hall by colour alone. */
    white: {
      a: '#F3F6FA', b: '#D8B25F', glow: 'rgba(232,238,246,.55)',
      confetti: ['#ffffff', '#F3F6FA', '#E2E8F0', '#D8B25F', '#EDCD85', '#CBD5E1'],
      flame: ['#F8FAFC', '#E2E8F0', '#EDCD85'], base: 'rgba(226,232,240,.3)',
  ...
  function paint(host, o) {
    ...
    var corner = CORNERS[o.corner] ? o.corner : 'blue';
    ...
    div(stage, 'position:absolute;inset:0;z-index:9;pointer-events:none;animation:wcFlashIn .55s ease-out both;' +
      'background:radial-gradient(circle at ' + (corner === 'blue' ? '40%' : '60%') + ' 45%, #fff 0%,' +
    ...
    if (photo) {
      var side = div(stage, 'position:absolute;inset:0;display:flex;align-items:center;pointer-events:none;' +
        (corner === 'blue' ? 'justify-content:flex-end;padding-right:' : 'justify-content:flex-start;padding-left:') +
        (5 * vw) + 'px;');
      var slide = div(side, 'animation:wcSlide' + (corner === 'blue' ? 'Right' : 'Left') +
        ' .6s cubic-bezier(.2,1,.3,1) both;');
      var cardW = Math.min(35 * vw, 58 * vh);
      var card = div(slide, 'transform:rotate(' + (corner === 'blue' ? '' : '-') + '2deg);' +
    ...
    div(stage, 'position:absolute;left:' + ((corner === 'blue' ? 40 : 60) * vw) + 'px;top:' + (20 * vh) + 'px;' +
    ...
    var mirrored = !! photo && corner === 'white';
    ...
    for (var i = 0; i < 44; i++) {
      ...
      var s = (corner === 'blue' ? 4 + rnd() * 6 : 3 + rnd() * 5) * k;
      div(box, 'position:absolute;top:0;left:' + ((i + 0.5) * (100 / 44)).toFixed(2) + '%;' +
        ...
        'box-shadow:0 0 ' + ((corner === 'blue' ? 8 + rnd() * 10 : 6 + rnd() * 8) * k).toFixed(0) + 'px ' +
        Math.max(1, k).toFixed(0) + 'px ' + c + ';will-change:transform;' +
        'animation:wcConfetti ' + (corner === 'blue' ? (3.4 + rnd() * 3) : (4.2 + rnd() * 3.4)).toFixed(2) + 's linear ' +
        (rnd() * 3).toFixed(2) + 's infinite, wcEmberFlicker ' +
        (corner === 'blue' ? (.5 + rnd() * .9) : (.8 + rnd() * 1.2)).toFixed(2) + 's ease-in-out infinite;');
    }
