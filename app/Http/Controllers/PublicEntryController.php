...
class PublicEntryController extends Controller
{
    public function show(Request $request, ClubEvent $event, PublicEvent $publisher, PublicEntry $entries)
    {
        ...
        if (! $state['open']) {
            return redirect()->route('events.public', $event->uuid);
        }
        ...
        if ($me === null) {
            $request->session()->put('url.intended', route('events.public.enter', $event->uuid));
        }
        ...
        return view('entry.public.enrol', [
            'e' => $e,
            'belts' => self::belts(),
     ...
     * scraping door is the shape rather than a permission: this event's own list
     * and nothing else, capped, throttled, and only the four fields a picker row
     * draws. The key it hands back is the EVENT CLUB's uuid — never an
     * auto-increment id (CLAUDE.md → Unpredictable Resource Identifiers) — and
     * the tenant behind it, if there is one, is resolved server-side when the
     * entry is made.
     */
    public function clubs(Request $request, ClubEvent $event, PublicEvent $publisher): JsonResponse
    {
        ...
        $clubs = EventClub::with('tenant:id,club_name,logo,country')
        ...
        return response()->json([
            'clubs' => $clubs->map(fn (EventClub $c) => [
                'name' => $c->tenant?->club_name ?: $c->name,
                // The event club's uuid IS the value: a club written down here
                // has no slug, because it has no tenant behind it. The field is
                // still called `slug` because that is what the picker and
                // resolveClub() have always exchanged, and the old key still
                // resolves (see App\Events\Support\PublicEntry::resolveClub).
                'slug' => $c->uuid,
    ...
    public function store(Request $request, ClubEvent $event, PublicEvent $publisher, PublicEntry $entries): JsonResponse
    {
         ...
         */
        $landing = route('events.public', ['event' => $event->uuid]);
        ...
        if (! empty($result['athlete_id'])) {
            ...
            if ($athlete) {
                ...
                $landing = route('events.public.my-entry', ['event' => $event->uuid]);
            }
        }
