...
/**
 * Taekwondo Tournament / Championship — the reference event-type package.
 *
 ...
 * from the Taekwondo combat-sport plug-in. Nothing outside this directory needs
 * to know Taekwondo exists.
 */
class Tournament extends AbstractEventType
{
    use SyncsDivisions;

    private const SPORT = 'taekwondo';
    ...
    {
        return 'taekwondo_tournament';
    }

    public function label(): string
    {
        return __('event-taekwondo_tournament::messages.type_label');
    }
    ...
    public function recordOutcome(ClubEvent $event, int $unitId, array $payload): array
    {
        ...
        abort_if($event->hasEnded(), 422, __('event-taekwondo_tournament::messages.ended_locked'));
    ...
    {
        if ($event->hasStarted()) {
            return ['success' => false, 'message' => __('event-taekwondo_tournament::messages.draw_final')];
        }
        ...
        $mats = collect($plan)
            ->map(fn ($p, $day) => __('event-taekwondo_tournament::messages.day_mats', ['day' => $day, 'count' => $p['courts']]))
            ->implode(' · ');

        return [
            'success' => true,
            'message' => __('event-taekwondo_tournament::messages.draw_generated').($mats ? ' · '.$mats : ''),
            'data' => ['plan' => $plan],
        ];
    }
    ...
    public function notificationSchedule(ClubEvent $event): array
    {
        ...
        if ($event->weigh_in_at) {
            $milestones[] = new Milestone(
                key: 'weigh_in',
                title: __('event-taekwondo_tournament::messages.notify_weigh_in_title', ['title' => $event->title]),
                body: __('event-taekwondo_tournament::messages.notify_weigh_in_body', [
        ...
        if ($event->categories()->where('draw_state', 'final')->exists()) {
            $milestones[] = new Milestone(
                key: 'draw_published',
                title: __('event-taekwondo_tournament::messages.notify_draw_title', ['title' => $event->title]),
                body: __('event-taekwondo_tournament::messages.notify_draw_body'),
     ...
     * resources/views/events/types/taekwondo_tournament/{mobile,desktop}/ and
     ...
     * squad, and the organiser running the mats — reads Taekwondo on the
    ...
    public function viewData(ClubEvent $event, User $viewer): array
    {
        ...
        return parent::viewData($event, $viewer) + [
            'division_label' => __('sport-taekwondo::messages.division_label'),
    ...
    private function sport(): \App\Sports\Combat\CombatSport
    {
        return $this->sports->get(self::SPORT)
            ?? throw new \RuntimeException('Taekwondo combat-sport plug-in is not registered in config/combat.php.');
    }
}
