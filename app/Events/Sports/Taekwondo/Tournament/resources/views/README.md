# Taekwondo Tournament — package screens

Blade views placed in this folder are registered automatically under the
`event-taekwondo_tournament::` namespace by `App\Events\EventPackageServiceProvider`
(no path is wired by hand anywhere).

```blade
{{-- app/Events/Sports/Taekwondo/Tournament/resources/views/mobile/show.blade.php --}}
→ view('event-taekwondo_tournament::mobile.show')
```
...
public function views(): array
{
    return [
        'show' => [
            'mobile'  => 'event-taekwondo_tournament::mobile.show',
            'desktop' => 'event-taekwondo_tournament::desktop.show',
        ],
        'run' => [
            'mobile'  => 'event-taekwondo_tournament::mobile.bracket',
            'desktop' => 'event-taekwondo_tournament::desktop.bracket',
        ],
    ];
}
```
