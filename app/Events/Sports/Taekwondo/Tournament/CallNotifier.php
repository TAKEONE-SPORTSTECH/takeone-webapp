...
class CallNotifier
{
    ...
    private function call(ClubEvent $event, EventMatch $bout, int $entryId, int $threshold, string $kind, int $ahead): int
    {
        ...
        $mat = $bout->court ?: __('event-taekwondo_tournament::messages.call_mat_tbc');
        $eta = $this->order->eta($event, $ahead);

        $title = $kind === 'call_room'
            ? __('event-taekwondo_tournament::messages.call_room_title', ['mat' => $mat])
            : __('event-taekwondo_tournament::messages.call_warmup_title', ['mat' => $mat]);

        $body = $kind === 'call_room'
            ? __('event-taekwondo_tournament::messages.call_room_body', [
                'mat' => $mat,
                'bout' => $bout->match_no ?? '—',
                'opponent' => $this->opponentName($bout, $entryId),
            ])
            : __('event-taekwondo_tournament::messages.call_warmup_body', [
                'ahead' => $ahead,
                'minutes' => $eta ?? 0,
            ]);
    ...
    public function pushResult(ClubEvent $event, EventMatch $bout): int
    {
        ...
        foreach ($bout->competitorIds() as $entryId) {
            ...
            $body = $next
                ? __('event-taekwondo_tournament::messages.result_body_next', [
                    'round' => $next['round'],
                    'mat' => $next['court'] ?? '—',
                    'minutes' => $next['eta_minutes'] ?? 0,
                ])
                : __('event-taekwondo_tournament::messages.result_body_done');
            ...
            foreach (array_merge([$entry->user->id], $this->guardiansOf($entry->user)) as $userId) {
                    ...
                    ? __('event-taekwondo_tournament::messages.result_win_title')
                    : __('event-taekwondo_tournament::messages.result_loss_title'), [
                        'body' => $body,
    ...
    private function opponentName(EventMatch $bout, int $entryId): string
    {
        ...
        return $bout->{$side.'_name'} ?: __('event-taekwondo_tournament::messages.call_opponent_tbc');
    }
}
