...
cd takeone-webapp/app/Events/Sports/Taekwondo/Tournament/CourtDisplay/pi
...
```bash
journalctl -u takeone-court -f          # what the screen is doing
sudo systemctl restart takeone-court    # restart the display
sudo systemctl status takeone-court
```

**Re-pair a screen** (move it to another mat or another event):

```bash
sudo rm /etc/takeone-court/token
sudo systemctl restart takeone-court    # back to a QR code
```
...
|---|---|
| Black screen, no output | `journalctl -u takeone-court -e`. Usually cog failing to open DRM — check `/dev/dri/card0` exists |
...
| Board shows, fonts look wrong | The fonts are served by the app; check `curl -I https://takeone.bh/court-display/font/anton-400-latin.woff2` |
