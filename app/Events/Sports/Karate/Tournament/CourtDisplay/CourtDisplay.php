...
class CourtDisplay
{
    ...
    private function corner(string $corner, EventMatch $bout, array $entries): array
    {
        ...
        return [
            $corner.'Name' => $bout->{$side.'_name'} ?: __('event-karate_tournament::messages.court_tbd'),
    ...
    private function stage(EventMatch $bout): string
    {
        ...
        $key = 'event-karate_tournament::messages.round_'.strtolower(str_replace(' ', '_', $round));
        ...
        if (preg_match('/^Round of (\d+)$/', $round, $m)) {
            return __('event-karate_tournament::messages.round_of', ['n' => $m[1]]);
        }
