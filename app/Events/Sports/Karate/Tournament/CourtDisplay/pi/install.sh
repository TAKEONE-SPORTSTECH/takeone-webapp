...
SERVER="${SERVER%/}"
CONF_DIR="/etc/takeone-karate-court"
...
install -m 755 "$HERE/takeone-karate-court" /usr/local/bin/takeone-karate-court
...
install -m 755 "$HERE/takeone-karate-court-link" /usr/local/bin/takeone-karate-court-link
install -m 644 "$HERE/takeone-karate-court.service" /etc/systemd/system/takeone-karate-court.service
...
if [ -f "$CONF" ] && ! grep -q '^# takeone-karate-court' "$CONF"; then
    cat >> "$CONF" <<'EOF'

# takeone-karate-court — the board scales, so 720p costs no design and buys frame rate
hdmi_group=1
hdmi_mode=4
disable_overscan=1
gpu_mem=128
...
if [ -f "$CONF" ] && grep -q '^# takeone-karate-court' "$CONF" && ! grep -q '^gpu_mem=' "$CONF"; then
    printf 'gpu_mem=128\n' >> "$CONF"
...
systemctl enable takeone-karate-court.service
systemctl restart takeone-karate-court.service
...
Done.
  ...
  Logs:     journalctl -u takeone-karate-court -f
  Restart:  sudo systemctl restart takeone-karate-court
  Re-pair:  sudo rm /etc/takeone-karate-court/token && sudo systemctl restart takeone-karate-court
