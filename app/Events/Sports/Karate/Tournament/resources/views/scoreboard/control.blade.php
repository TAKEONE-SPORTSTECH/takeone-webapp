...
@endforeach
  ...
  *{scrollbar-width:thin;scrollbar-color:var(--line-2) var(--ink);}
  ...
  .modal{background:var(--panel);border:1px solid var(--line-2);border-radius:20px;padding:28px 30px;
         display:flex;flex-direction:column;gap:18px;max-height:94%;animation:winIn .25s cubic-bezier(.2,.8,.2,1) both;}
...
<div id="root"><div id="stage" style="background:linear-gradient(120deg,#0a0b10 0%,#10121c 30%,#0a0b10 55%,#12101a 80%,#0a0b10 100%);background-size:300% 300%;animation:ambient 18s ease-in-out infinite;font-family:'Barlow Condensed',sans-serif;color:var(--text);display:flex;flex-direction:column;gap:22px;padding:26px 36px 22px;overflow:hidden;position:relative;">
  ...
  <div style="display:flex;align-items:flex-end;gap:32px;">
    <div style="display:flex;flex-direction:column;gap:2px;flex-shrink:0;">
      <div style="font-family:'Anton',sans-serif;font-size:34px;letter-spacing:.04em;text-transform:uppercase;">{{ __('event-karate_tournament::messages.ctl_heading') }}</div>
      <div style="font-size:18px;color:var(--muted);letter-spacing:.18em;text-transform:uppercase;">{{ __('event-karate_tournament::messages.ctl_subtitle') }}</div>
    </div>
    <div style="flex:1;min-width:0;display:flex;justify-content:flex-end;align-items:flex-end;gap:40px;">
      @foreach ([
          ['mTournament', __('event-karate_tournament::messages.ctl_tournament'), '420px'],
          ['mCategory', __('event-karate_tournament::messages.ctl_category'), '300px'],
          ['mMatchNo', __('event-karate_tournament::messages.ctl_match'), null],
          ['mCourt', __('event-karate_tournament::messages.ctl_tatami'), null],
          ['mRound', __('event-karate_tournament::messages.ctl_round'), '260px'],
      ] as [$id, $label, $cap])
  ...
  <div style="flex:1;display:grid;grid-template-columns:1fr 500px 1fr;gap:22px;min-height:0;">
    ...
    <div class="card" style="border-top:8px solid {{ $colour }};padding:24px 32px;display:flex;flex-direction:column;gap:18px;order:{{ $order }};animation:cardIn .6s {{ $delay }} cubic-bezier(.2,.8,.2,1) both;">
      ...
      <div style="{{ $rev }}display:flex;align-items:center;gap:16px;">
        <div style="font-size:19px;letter-spacing:.2em;color:var(--muted);text-transform:uppercase;width:104px;{{ $mirror ? 'text-align:right;' : '' }}">{{ __('event-karate_tournament::messages.ctl_penalty') }}</div>
        <div id="pen-{{ $side }}" style="{{ $rev }}display:flex;gap:10px;flex:1;"></div>
      </div>

      <div style="{{ $rev }}display:flex;align-items:center;gap:16px;">
        <div style="font-size:19px;letter-spacing:.2em;color:var(--muted);text-transform:uppercase;width:104px;{{ $mirror ? 'text-align:right;' : '' }}">{{ __('event-karate_tournament::messages.ctl_senshu') }}</div>
        <button id="senshu-{{ $side }}" class="bout" style="flex:1;font-size:24px;font-weight:700;letter-spacing:.16em;text-transform:uppercase;border-radius:12px;height:58px;cursor:pointer;transition:background .3s,color .3s,border-color .3s;" data-cmd="senshu" data-side="{{ $side }}">{{ __('event-karate_tournament::messages.ctl_senshu') }}</button>
      </div>
      ...
      <button data-participant="{{ $side }}" style="{{ $rev }}display:flex;align-items:center;gap:14px;background:var(--ink);border:1px solid var(--line);border-radius:12px;padding:10px 16px;cursor:pointer;text-align:{{ $mirror ? 'right' : 'left' }};">
        ...
        <span style="color:var(--muted);font-size:17px;letter-spacing:.1em;">{{ $mirror ? '▾ ' : '' }}{{ __('event-karate_tournament::messages.ctl_view') }}{{ $mirror ? '' : ' ▾' }}</span>
      </button>
    </div>
    @endforeach
    ...
    <div class="card" style="padding:30px 30px 26px;display:flex;flex-direction:column;align-items:center;gap:22px;order:2;animation:cardIn .6s .18s cubic-bezier(.2,.8,.2,1) both;">
      ...
      <button id="btnBouts" style="width:100%;flex:1.2;min-height:96px;font-size:44px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;background:var(--gold);color:#000;border:none;border-radius:16px;padding:26px 0;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:22px;">
        ...
        {{ __('event-karate_tournament::messages.ctl_bouts') }}
        <svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M5 6 L12 13 L19 6"></path><path d="M5 12 L12 19 L19 12"></path></svg>
      </button>
      ...
      <div style="width:100%;display:flex;flex-direction:column;align-items:center;gap:6px;padding:16px 20px 18px;border-radius:12px;background:var(--ink);">
        <div style="font-size:19px;letter-spacing:.32em;color:var(--muted);text-transform:uppercase;">{{ __('event-karate_tournament::messages.ctl_bout_timer') }}</div>
        ...
        <div id="status" style="font-size:32px;font-weight:700;letter-spacing:.28em;text-transform:uppercase;color:var(--gold);">{{ __('event-karate_tournament::messages.ctl_pause') }}</div>
      </div>

      <button id="btnStart" class="bout" style="width:100%;flex:1.2;min-height:96px;font-size:44px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:#000;border:none;border-radius:16px;padding:26px 0;cursor:pointer;box-shadow:0 8px 0 -2px rgba(0,0,0,.5);transition:transform .08s;">{{ __('event-karate_tournament::messages.ctl_start') }}</button>
      ...
      <div style="width:100%;display:grid;grid-template-columns:1fr 1fr;gap:12px;">
        ...
        <button id="btnKo" class="cbtn bout" style="color:var(--alarm);border:1px solid rgba(255,59,71,.5);">{{ __('event-karate_tournament::messages.ctl_finish') }}</button>
        ...
        @endisset
        <button id="btnSettings" class="cbtn" style="color:var(--gold);border:1px solid var(--gold);">{{ __('event-karate_tournament::messages.ctl_settings') }}</button>
        <button id="btnHardware" class="cbtn" style="color:var(--text);border:1px solid var(--line-2);">{{ __('event-karate_tournament::messages.ctl_hardware') }}</button>
        <button id="btnReset" class="cbtn bout" style="background:transparent;color:var(--aka-ink);border:1px solid rgba(255,107,120,.5);">{{ __('event-karate_tournament::messages.ctl_reset') }}</button>
        {{-- Deliberately NOT disabled with the rest of the bout controls: a hung
             ...
             no button at all. --}}
        <button id="btnResync" class="cbtn" style="color:var(--ao-ink);border:1px solid rgba(138,180,255,.45);">{{ __('event-karate_tournament::messages.ctl_resync') }}</button>
      </div>

      <div style="font-size:16px;color:var(--faint);letter-spacing:.06em;text-align:center;">{{ __('event-karate_tournament::messages.ctl_resync_hint') }}</div>
    </div>
  </div>
  ...
  <div id="bouts" hidden class="scrim" style="z-index:20;">
    <div class="modal" style="width:1100px;height:900px;gap:16px;">
      <div class="mhead">
        <div class="mtitle" style="font-size:26px;color:var(--green);">{{ __('event-karate_tournament::messages.ctl_bouts') }}</div>
      ...
      <div style="font-size:20px;color:var(--faint);">{{ __('event-karate_tournament::messages.ctl_bouts_hint') }}</div>
    </div>
  </div>
  ...
  <div id="settings" hidden class="scrim" style="z-index:21;">
    <div class="modal" style="width:1020px;gap:22px;overflow-y:auto;">
      <div class="mhead">
        <div class="mtitle" style="color:var(--gold);">{{ __('event-karate_tournament::messages.ctl_settings_title') }}</div>
      ...
      <div style="display:flex;gap:8px;border-bottom:1px solid var(--line);padding-bottom:14px;">
        @foreach ([['rules', __('event-karate_tournament::messages.ctl_tab_rules')], ['timer', __('event-karate_tournament::messages.ctl_tab_timer')], ['sounds', __('event-karate_tournament::messages.ctl_tab_sounds')]] as [$tab, $tabLabel])
      ...
      <div class="spanel" data-panel="rules" style="display:flex;flex-direction:column;gap:12px;">
        @foreach ([
            ['senshuRule', __('event-karate_tournament::messages.ctl_rule_senshu'), __('event-karate_tournament::messages.ctl_rule_senshu_hint')],
            ['autoSenshu', __('event-karate_tournament::messages.ctl_rule_auto_senshu'), __('event-karate_tournament::messages.ctl_rule_auto_senshu_hint')],
            ['winByPenalties', __('event-karate_tournament::messages.ctl_rule_penalties'), __('event-karate_tournament::messages.ctl_rule_penalties_hint')],
            ['atoshiWarn', __('event-karate_tournament::messages.ctl_rule_atoshi'), __('event-karate_tournament::messages.ctl_rule_atoshi_hint')],
            ['timeUpBuzzer', __('event-karate_tournament::messages.ctl_rule_buzzer'), __('event-karate_tournament::messages.ctl_rule_buzzer_hint')],
        ...
        <div class="rrow" style="display:flex;align-items:center;gap:14px;background:var(--inset);border:1px solid var(--line);border-radius:12px;padding:12px 16px;">
          ...
          <span style="flex:1;display:flex;flex-direction:column;gap:2px;">
            <span style="font-size:20px;font-weight:600;color:var(--text);letter-spacing:.04em;">{{ __('event-karate_tournament::messages.ctl_rule_gap') }}</span>
            <span style="font-size:15px;color:var(--faint);letter-spacing:.02em;">{{ __('event-karate_tournament::messages.ctl_rule_gap_hint') }}</span>
          </span>
      ...
      <div class="spanel" data-panel="timer" hidden style="display:flex;flex-direction:column;gap:20px;">
        <label style="display:flex;flex-direction:column;gap:8px;">
          <span style="font-size:17px;letter-spacing:.18em;text-transform:uppercase;color:var(--muted);">{{ __('event-karate_tournament::messages.ctl_duration') }}</span>
        ...
        <label style="display:flex;flex-direction:column;gap:8px;">
          <span style="font-size:17px;letter-spacing:.18em;text-transform:uppercase;color:var(--muted);">{{ __('event-karate_tournament::messages.ctl_warning_at') }}</span>
          <span style="display:flex;gap:12px;align-items:center;">
          ...
          <span style="font-size:14px;color:var(--faint);letter-spacing:.04em;">{{ __('event-karate_tournament::messages.ctl_warning_hint') }}</span>
        </label>
      </div>
      ...
      <div class="spanel" data-panel="sounds" hidden style="display:grid;grid-template-columns:1fr 1fr;gap:4px 28px;">
        @foreach ([
          ...
          <div style="display:flex;align-items:center;gap:14px;padding:10px 0;">
            <div style="flex:1;min-width:0;">
              <div style="font-size:23px;font-weight:600;color:var(--text);">{{ $slotLabel }}</div>
      ...
      <button id="btnApply" style="font-size:24px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;background:var(--gold);color:#000;border:none;border-radius:14px;padding:16px 0;cursor:pointer;">{{ __('event-karate_tournament::messages.ctl_apply') }}</button>
      <div style="font-size:16px;color:var(--faint);letter-spacing:.04em;text-align:center;">{{ __('event-karate_tournament::messages.ctl_apply_hint') }}</div>
    </div>
  </div>
    ...
    <div class="modal" style="width:640px;gap:20px;">
      ...
      <div style="display:flex;gap:24px;align-items:stretch;">
        <button id="pPhoto" style="flex:0 0 auto;align-self:center;width:210px;height:280px;border-radius:14px;background-color:var(--ink);background-size:cover;background-position:center 15%;cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;color:var(--faint);font-size:15px;letter-spacing:.08em;text-transform:uppercase;padding:0;overflow:hidden;">
          ...
          <span id="pPhotoLabel">{{ __('event-karate_tournament::messages.ctl_photo_label') }}</span>
        </button>
        <div style="flex:1;min-width:0;display:flex;flex-direction:column;gap:14px;">
          <div style="display:flex;flex-direction:column;gap:5px;">
            <span style="font-size:15px;letter-spacing:.18em;text-transform:uppercase;color:var(--muted);">{{ __('event-karate_tournament::messages.ctl_name') }}</span>
            <div id="pName" class="ro" style="font-size:26px;font-weight:600;"></div>
          </div>
          <div style="display:flex;flex-direction:column;gap:5px;">
            <span style="font-size:15px;letter-spacing:.18em;text-transform:uppercase;color:var(--muted);">{{ __('event-karate_tournament::messages.ctl_club') }}</span>
            <div id="pClub" class="ro" style="font-size:24px;"></div>
          </div>
          <div style="display:flex;flex-direction:column;gap:5px;">
            <span style="font-size:15px;letter-spacing:.18em;text-transform:uppercase;color:var(--muted);">{{ __('event-karate_tournament::messages.ctl_country_from_club') }}</span>
          ...
          <div style="font-size:16px;color:var(--faint);letter-spacing:.04em;">{{ __('event-karate_tournament::messages.ctl_participant_note') }}</div>
        </div>
      </div>

      <button data-close="participant" style="font-size:24px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;background:var(--green);color:#000;border:none;border-radius:14px;padding:16px 0;cursor:pointer;">{{ __('event-karate_tournament::messages.ctl_done') }}</button>
    </div>
  </div>
    ...
    <div class="modal" style="width:520px;padding:26px 28px;">
      ...
      <div id="cropView" style="width:330px;height:440px;margin:0 auto;overflow:hidden;position:relative;background:var(--ink);border:1px solid var(--line-2);border-radius:10px;cursor:grab;touch-action:none;">
        ...
        <div id="cropEmpty" style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;color:var(--faint);font-size:18px;letter-spacing:.08em;text-transform:uppercase;">
          <span style="font-size:40px;color:var(--gold);">◎</span>{{ __('event-karate_tournament::messages.ctl_crop_empty') }}
        </div>
      </div>

      <div style="display:flex;flex-direction:column;gap:10px;">
        <label style="display:flex;align-items:center;gap:14px;">
          <span style="width:70px;font-size:16px;letter-spacing:.14em;text-transform:uppercase;color:var(--muted);">{{ __('event-karate_tournament::messages.ctl_crop_zoom') }}</span>
          <input id="cropZoom" type="range" min="1" max="4" step="0.01" value="1" style="flex:1;accent-color:#ffe135;">
        </label>
        <label style="display:flex;align-items:center;gap:14px;">
          <span style="width:70px;font-size:16px;letter-spacing:.14em;text-transform:uppercase;color:var(--muted);">{{ __('event-karate_tournament::messages.ctl_crop_tilt') }}</span>
        ...
        <div style="font-size:15px;color:var(--faint);letter-spacing:.04em;text-align:center;">{{ __('event-karate_tournament::messages.ctl_crop_drag') }}</div>
      </div>
      ...
      <div style="display:flex;gap:12px;">
        <label style="flex:1;text-align:center;font-size:19px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;background:var(--fill);color:var(--ao-ink);border:1px solid rgba(138,180,255,.4);border-radius:12px;padding:14px 0;cursor:pointer;">
          {{ __('event-karate_tournament::messages.ctl_crop_upload') }}
          <input id="cropFile" type="file" accept="image/*" style="display:none;">
        </label>
        <button id="cropRemove" style="flex:1;font-size:19px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;background:var(--fill);color:var(--aka-ink);border:1px solid rgba(255,107,120,.45);border-radius:12px;padding:14px 0;cursor:pointer;">{{ __('event-karate_tournament::messages.ctl_crop_remove') }}</button>
        <button id="cropSave" disabled style="flex:1.4;font-size:19px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;background:var(--green);color:#000;border:none;border-radius:12px;padding:14px 0;cursor:pointer;opacity:.4;">{{ __('event-karate_tournament::messages.ctl_crop_save') }}</button>
      </div>
    </div>
  </div>
  ...
  <div id="endBout" hidden class="scrim" style="z-index:22;">
    <div class="modal" style="width:760px;gap:20px;">
      <div class="mhead">
        <div class="mtitle" style="color:var(--gold);">{{ __('event-karate_tournament::messages.end_title') }}</div>
      ...
      <button id="endAuto" style="display:flex;flex-direction:column;gap:6px;text-align:left;background:#0f1a12;border:1px solid rgba(122,229,130,.45);border-radius:14px;padding:18px 22px;cursor:pointer;">
        <span style="font-size:24px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--green);">{{ __('event-karate_tournament::messages.end_auto') }}</span>
        <span id="endAutoWho" style="font-size:19px;color:var(--second);"></span>
      </button>

      <div style="height:1px;background:var(--line-2);"></div>
      <div style="font-size:19px;font-weight:600;letter-spacing:.14em;text-transform:uppercase;color:var(--muted);">{{ __('event-karate_tournament::messages.end_manual') }}</div>
      ...
      <div style="display:flex;flex-wrap:wrap;gap:10px;">
        @foreach (['hansoku', 'shikkaku', 'kiken', 'medical', 'no_show', 'other'] as $reason)
          <button class="endReason" data-reason="{{ $reason }}" style="font-size:20px;font-weight:600;letter-spacing:.06em;text-transform:uppercase;background:var(--fill);color:var(--text);border:2px solid transparent;border-radius:11px;padding:12px 20px;cursor:pointer;">{{ __('event-karate_tournament::messages.end_reason_'.$reason) }}</button>
        @endforeach
      </div>

      <input id="endNote" type="text" maxlength="200" placeholder="{{ __('event-karate_tournament::messages.end_note') }}" class="fld">

      <button id="endDeclare" disabled style="font-size:26px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;background:var(--aka-ink);color:#000;border:none;border-radius:14px;padding:18px 0;cursor:pointer;opacity:.4;">{{ __('event-karate_tournament::messages.end_declare') }}</button>
      <div style="font-size:17px;color:var(--faint);letter-spacing:.04em;text-align:center;">{{ __('event-karate_tournament::messages.end_hint') }}</div>
    </div>
  </div>
  ...
  <div id="hardware" hidden class="scrim" style="z-index:21;">
    <div class="modal" style="width:640px;">
      <div class="mhead">
        <div class="mtitle" style="color:var(--text);">{{ __('event-karate_tournament::messages.ctl_hardware') }}</div>
      ...
      <div style="display:flex;flex-direction:column;align-items:center;gap:10px;padding:40px 0;color:var(--faint);">
        ...
        <div style="font-size:22px;letter-spacing:.06em;">{{ __('event-karate_tournament::messages.ctl_hardware_empty') }}</div>
        <div style="font-size:17px;letter-spacing:.04em;text-align:center;">{{ __('event-karate_tournament::messages.ctl_hardware_hint') }}</div>
      </div>
    </div>
  </div>
</div></div>
...
(function () {
  ...
  var TBD = @json(__('event-karate_tournament::messages.court_tbd'));
  var FAILED = @json(__('event-karate_tournament::messages.ctl_failed'));
...
@endisset
  ...
  function paintClock() {
    ...
    el('status').textContent = !STATE.matchId
      ? @json(__('event-karate_tournament::messages.ctl_waiting'))
      : over ? @json(__('event-karate_tournament::messages.ctl_time_up'))
             : STATE.running ? @json(__('event-karate_tournament::messages.ctl_start'))
                             : @json(__('event-karate_tournament::messages.ctl_pause'));
    el('status').style.color = STATE.running ? 'var(--green)' : 'var(--gold)';

    var start = el('btnStart');
    start.textContent = STATE.running ? @json(__('event-karate_tournament::messages.ctl_pause'))
                                      : @json(__('event-karate_tournament::messages.ctl_start'));
  ...
  function paintParticipant() {
    ...
    el('pTitle').textContent = pSide.toUpperCase() + ' — ' + @json(__('event-karate_tournament::messages.ctl_participant'));
  ...
  function openCrop() {
    ...
    if (!PHOTO_BASE) { alertBar(@json(__('event-karate_tournament::messages.ctl_photo_unavailable'))); return; }
    ...
    el('cropTilt').value = 0;
    el('cropFile').value = '';
    el('cropTitle').textContent = @json(__('event-karate_tournament::messages.ctl_crop_title')) + ' — ' +
  ...
  el('btnResync').onclick = function () {
    ...
    send('resync').then(function () {
      b.textContent = @json(__('event-karate_tournament::messages.ctl_resynced'));
  ...
  function uploadAudio(slot, input) {
    ...
    if (!AUDIO_BASE) { alertBar(@json(__('event-karate_tournament::messages.ctl_sound_unavailable'))); input.value = ''; return; }
