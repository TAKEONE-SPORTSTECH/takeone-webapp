...
@endforeach
  ...
  *{box-sizing:border-box;}
  ...
  /* The console is authored at 1920x1080 and only SCALES — same rule as the
  ...
  <div style="flex:1;display:grid;grid-template-columns:1fr 500px 1fr;gap:22px;min-height:0;">
    ...
    <div style="background:#12141d;border:1px solid #1f2230;border-top:8px solid {{ $colour }};border-radius:18px;padding:24px 32px;display:flex;flex-direction:column;gap:18px;order:{{ $order }};animation:cardIn .7s {{ $delay }} cubic-bezier(.2,.8,.2,1) both,{{ $breath }} 3.4s 1.2s ease-in-out infinite;">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;border-top:1px solid #1f2230;padding-top:22px;">
        <label class="col"><span class="lbl" style="font-size:17px;">Name</span><input id="name-{{ $side }}" class="fld" style="font-size:24px;background:#0a0b10;padding:13px 14px;" data-corner="name" data-side="{{ $side }}"></label>
    ...
    <div style="background:#12141d;border:1px solid #1f2230;border-radius:18px;padding:34px 30px;display:flex;flex-direction:column;align-items:center;gap:24px;order:2;animation:cardIn .7s .25s cubic-bezier(.2,.8,.2,1) both;">
      ...
      <div style="font-size:16px;color:#5c6175;letter-spacing:.06em;text-align:center;">Changes appear on the scoreboard instantly.</div>
    </div>
  </div>
...
@endisset
  ...
  function alertBar(msg) {
    ...
    b.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:40;background:#ff3b47;color:#fff;font-size:22px;' +
  ...
  function paintWinner() {
    var host = el('winner');
    if (!STATE.finished || !(STATE.akaLeads || STATE.aoLeads) || !STATE.matchId) {
    ...
    var name = ((aka ? STATE.aka : STATE.ao) || {}).name || '';
    var palette = ['#ffd666', '#fff', '#ff3b47', '#4d9aff'];
    for (var i = 0; i < 26; i++) {
      var c = document.createElement('div');
      c.style.cssText = 'position:absolute;left:' + (i * 3.8 % 100) + '%;top:-40px;width:' + (8 + i % 5 * 3) + 'px;height:' +
        (18 + i % 4 * 5) + 'px;background:' + palette[i % 4] + ';transform:rotate(' + (i * 37 % 360) + 'deg);' +
        'animation:confettiFall ' + (2.4 + i % 5 * .4) + 's ' + (i % 7 * .3) + 's linear infinite;';
      host.appendChild(c);
    }
    var stamp = document.createElement('div');
    stamp.style.cssText = 'position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);display:flex;flex-direction:column;' +
      'align-items:center;gap:14px;animation:stampIn .8s cubic-bezier(.2,.8,.2,1) both;pointer-events:none;';
    var w = document.createElement('div');
    w.style.cssText = "font-size:34px;font-weight:700;letter-spacing:.5em;color:#ffd666;text-transform:uppercase;";
    w.textContent = 'Winner';
    var plate = document.createElement('div');
    plate.style.cssText = 'background:linear-gradient(135deg,' + colour + ',#000);padding:20px 60px;box-shadow:0 0 90px ' + colour + ';';
    var n = document.createElement('div');
    n.style.cssText = "font-family:'Anton',sans-serif;font-size:96px;line-height:1;color:#fff;text-transform:uppercase;";
    n.textContent = name;
    plate.appendChild(n); stamp.appendChild(w); stamp.appendChild(plate);

    // The way out of the celebration, and the only control that WRITES: it
    // records the result, advances the bracket and calls the next bout. The
    // celebration otherwise sits there until an official decides the bout is
    // genuinely over — which is right, because a scoreboard that files a
    // result on a timer files it before anyone has looked at it.
    var go = document.createElement('button');
    go.id = 'btnCommit';
    go.style.cssText = 'pointer-events:auto;margin-top:16px;font-family:\'Barlow Condensed\',sans-serif;font-size:30px;font-weight:700;' +
    ...
    hint.style.cssText = 'pointer-events:none;font-size:17px;color:#a7abbe;letter-spacing:.06em;max-width:620px;text-align:center;';
    hint.textContent = @json(__('event-karate_tournament::messages.ctl_commit_hint'));

    stamp.appendChild(go);
    stamp.appendChild(hint);
  ...
  function paint() { paintMeta(); paintSide('aka'); paintSide('ao'); paintWinner(); paintClock(); paintQueue(); checkGap(); paintArmed(); }
  ...
  el('btnKo').onclick = function () { send('finish'); };
