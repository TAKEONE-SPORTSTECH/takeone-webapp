...
<div class="px-4 sm:px-6 lg:px-8 py-6 max-w-2xl mx-auto" x-data="{ picked: null, court: null, surface: 'bout' }">
    ...
    <div class="flex items-center gap-3 mb-1">
        ...
        <div>
            <h1 class="text-xl font-bold text-gray-900">{{ __('event-karate_tournament::messages.claim_title') }}</h1>
            <p class="text-sm text-muted-foreground">
                {{ __('event-karate_tournament::messages.claim_sub', ['code' => $code]) }}
            </p>
        </div>
    </div>
    ...
    @if ($events->isEmpty())
        <div class="mt-6 bg-white rounded-xl shadow-sm border border-gray-100 p-8 text-center">
            ...
            <p class="mt-3 text-sm text-muted-foreground">{{ __('event-karate_tournament::messages.claim_no_events') }}</p>
        </div>
    @else
                            ...
                            <div class="mt-3 flex flex-wrap gap-2" x-show="picked === '{{ $event['uuid'] }}'" x-cloak
                                ...
                                @empty
                                    <p class="text-xs text-muted-foreground">
                                        {{ __('event-karate_tournament::messages.claim_no_courts') }}
                                    </p>
                                @endforelse
                            </div>
                        </div>
...
<div x-show="picked && court" x-cloak
            ...
            <button type="submit"
                ...
                <i class="bi bi-display mr-2"></i>{{ __('event-karate_tournament::messages.claim_submit') }}
            </button>
        </form>
    @endif
</div>
