...
<div id="root">
  <div id="stage">
    ...
    <div id="head">
      ...
      <div style="flex:1; display:flex; justify-content:flex-end; align-items:center; gap:18px;">
        <div id="courtBadge" style="display:flex; align-items:baseline; gap:12px; background:rgba(10,10,14,0.72); border:1px solid rgba(253,196,54,0.5); border:1px solid oklch(0.85 0.16 85 / 0.5); padding:10px 28px;">
          <span style="font-weight:600; font-size:32px; letter-spacing:0.3em; color:rgba(232,230,224,0.65); text-transform:uppercase;">{{ __('event-karate_tournament::messages.court_court') }}</span>
    ...
    <div id="stale"><span class="dot"></span><span>{{ __('event-karate_tournament::messages.court_reconnecting') }}</span></div>
  </div>
</div>
...
(function () {
  ...
  var IDLE_T = @json(__('event-karate_tournament::messages.court_idle_title'));
  var IDLE_S = @json(__('event-karate_tournament::messages.court_idle_sub'));
  ...
  function plate(m, isNext) {
    ...
    line.appendChild(el('span', 'font-weight:600; font-size:26px; letter-spacing:0.24em; text-transform:uppercase; color:' + muted + ';', @json(__('event-karate_tournament::messages.court_match'))));
