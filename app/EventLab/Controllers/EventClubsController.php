 ...
 * `sandbox_event_clubs`, which is what every screen then reads, so nothing
...
class EventClubsController extends Controller
{
    ...
    public function page(Request $request, ClubEvent $event)
    {
         ...
         */
        return view('eventlab::personal.mobile.clubs', [
    ...
    public function store(Request $request, ClubEvent $event): JsonResponse
    {
        ...
        $exists = SandboxEventClub::where('event_id', $event->id)
        ...
        if ($exists) {
            return response()->json([
                'success' => false,
                'field' => 'name',
                'message' => __('eventlab::messages.club_exists'),
            ], 422);
        }
        ...
        if (($data['instagram'] ?? '') !== '' && $instagram === null) {
            return response()->json([
                'success' => false,
                'field' => 'instagram',
                'message' => __('eventlab::messages.club_instagram_invalid'),
            ], 422);
        }
        ...
        if (! $logo) {
            return response()->json([
                'success' => false,
                'field' => 'logo',
                'message' => __('eventlab::messages.club_logo_rejected'),
            ], 422);
        }

        $club = SandboxEventClub::create([
            'event_id' => $event->id,
            'name' => $name,
            'logo' => $logo,
            'instagram' => $instagram,
        ...
        return response()->json([
            'success' => true,
            'message' => __('eventlab::messages.club_added', ['name' => $club->name]),
            'club' => $club->present(),
    ...
    public function invite(Request $request, ClubEvent $event): JsonResponse
    {
        ...
        if (! $tenant) {
            return response()->json([
                'success' => false,
                'message' => __('eventlab::messages.club_not_found'),
            ], 422);
        }

        $already = SandboxEventClub::where('event_id', $event->id)
        ...
        if ($already) {
            return response()->json([
                'success' => false,
                'message' => __('eventlab::messages.club_already_invited'),
                'clubs' => $this->all($event),
            ], 422);
        }
        ...
        $club = SandboxEventClub::create([
            ...
            'state' => 'invited',
            'invited_user_id' => $tenant->owner_user_id,
            'invited_at' => now(),
            'created_by' => $actor->id,
        ]);

        // An invitation nobody is told about is a row, not an invitation.
        // notifyUser writes the record AND pushes it live (CLAUDE.md, Realtime).
        if ($tenant->owner_user_id) {
            UserNotification::notifyUser(
                (int) $tenant->owner_user_id,
                'event_club_invite',
                __('eventlab::messages.club_invite_title', ['event' => $event->title]),
                [
                    'actor_id' => $actor->id,
                    'tenant_id' => $tenant->id,
                    'body' => __('eventlab::messages.club_invite_body', [
                        'club' => $tenant->club_name,
                        'event' => $event->title,
                    ]),
                    // The event's own page: it needs no verified address to
                    // open, and the invitation banner is on it.
                    'action_url' => route('testcode.e', $event->uuid),
                    'icon' => 'bi-people-fill',
        ...
        return response()->json([
            'success' => true,
            'message' => __('eventlab::messages.club_invited', ['name' => $tenant->club_name]),
            'club' => $club->present(),
            'clubs' => $this->all($event),
        ]);
    }
    ...
    public function search(Request $request, ClubEvent $event): JsonResponse
    {
        ...
        $taken = SandboxEventClub::where('event_id', $event->id)
     ...
     * chosen again.
     */
    public function update(Request $request, ClubEvent $event, SandboxEventClub $eventClub): JsonResponse
    {
        $this->guard($event);
        ...
        if ($eventClub->isOnPlatform()) {
            return response()->json([
                'success' => false,
                'message' => __('eventlab::messages.club_not_editable'),
            ], 422);
        }
        ...
        $clash = SandboxEventClub::where('event_id', $event->id)
        ...
        return response()->json([
            'success' => true,
            'message' => __('eventlab::messages.club_updated', ['name' => $name]),
     ...
     */
    public function destroy(Request $request, ClubEvent $event, SandboxEventClub $eventClub): JsonResponse
    {
        $this->guard($event);
         ...
         * between them: `sandbox_event_club_entrants` cascades from the club,
        ...
        return response()->json([
            'success' => true,
            'message' => $athletes
                ? __('eventlab::messages.club_removed_athletes', ['name' => $name, 'n' => $athletes])
                : __('eventlab::messages.club_removed', ['name' => $name]),
     ...
     */
    public function respond(Request $request, ClubEvent $event, SandboxEventClub $eventClub): JsonResponse
    {
        $me = Auth::user();
        ...
        if ($organiser = $event->created_by) {
            UserNotification::notifyUser(
                ...
                __('eventlab::messages.club_answer_title', [
                    'club' => $eventClub->name,
                    'answer' => __('eventlab::messages.club_state_'.$data['answer']),
                ]),
                [
                    'actor_id' => $me->id,
                    'body' => $event->title,
        ...
        return response()->json([
            'success' => true,
            'message' => __('eventlab::messages.club_answered'),
            'club' => $eventClub->fresh()->present(),
        ]);
    }
    ...
    public function entrants(Request $request, ClubEvent $event): JsonResponse
    {
        ...
        $links = DB::table('sandbox_event_club_entrants as l')
     ...
     */
    public function assign(Request $request, ClubEvent $event, SandboxEventClub $eventClub): JsonResponse
    {
        $actor = $this->guard($event);
        ...
        DB::transaction(function () use ($eventClub, $ids, $actor) {
            ...
            foreach ($ids as $id) {
                ...
                DB::table('sandbox_event_club_entrants')->updateOrInsert(
                    ['registration_id' => $id],
                    [
                        'sandbox_event_club_id' => $eventClub->id,
                        'added_by' => $actor->id,
                        'updated_at' => now(),
                        'created_at' => now(),
                    ]
        ...
        return response()->json([
            'success' => true,
            'message' => __('eventlab::messages.club_athletes_saved', ['n' => count($ids), 'name' => $eventClub->name]),
            'clubs' => $this->all($event),
        ]);
    }
     ...
     *
     * The manual half of `eventlab:promote-clubs`, for an organiser who has
    ...
    public function promote(Request $request, ClubEvent $event): JsonResponse
    {
        ...
        if ($result['reason'] === 'not_over') {
            return response()->json([
                'success' => false,
                'message' => __('eventlab::messages.club_promote_too_early'),
            ], 422);
        }

        return response()->json([
            'success' => true,
            'message' => __('eventlab::messages.club_promoted_done', [
    ...
    {
        return SandboxEventClub::with(['tenant:id,club_name,logo,country,owner_user_id', 'entrants:id'])
            ->where('event_id', $event->id)
            ->orderByRaw("case state when 'accepted' then 0 when 'listed' then 1 when 'invited' then 2 else 3 end")
            ->orderBy('name')
            ->get()
            ->map(fn (SandboxEventClub $c) => $c->present())
            ->all();
    }
