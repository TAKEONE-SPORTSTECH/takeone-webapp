 ...
 */
/*
 * COPY — this file is `app/Http/Controllers/PublicEventController.php`, copied verbatim into the
...
class PublicEventController extends Controller
{
    public function show(Request $request, ClubEvent $event, PublicEvent $publisher)
    {
        ...
        if ($me) {
            ...
            $asked = ! $entered && \Illuminate\Support\Facades\Schema::hasTable('event_public_entries')
                && \Illuminate\Support\Facades\DB::table('event_public_entries')
        ...
        // An invitation this reader can actually answer.
        //
        // The notification that announced it links here, so this page is where
        // the answer has to be possible — a message that leads to a page with
        // no way to reply is the dead end the platform's own rule forbids.
        $invite = null;

        if ($me) {
            $invite = \App\EventLab\Models\SandboxEventClub::with('tenant:id,club_name,logo,country,owner_user_id')
                ->where('event_id', $event->id)
                ->where('state', 'invited')
                ->get()
                ->first(fn ($c) => (int) $c->invited_user_id === (int) $me->id
                    || (int) ($c->tenant?->owner_user_id) === (int) $me->id);
        }

        return view('eventlab::entry.public.mobile', [
            'e' => $e,
            'console' => $console,
            'mine' => $mine,
            'invite' => $invite,
    ...
    public function section(Request $request, ClubEvent $event, string $section, PublicEvent $publisher)
    {
        ...
        return view('eventlab::entry.public.section-mobile', [
    ...
    public function signOut(Request $request, ClubEvent $event, PublicEvent $publisher)
    {
        ...
        return redirect()->route('testcode.e', ['event' => $event->uuid])
    ...
    public function manage(Request $request, ClubEvent $event, PublicEvent $publisher, EventAccess $access)
    {
         ...
         */
        return view('eventlab::entry.public.manage', [
            'e' => $e,
    ...
    public function signIn(Request $request, ClubEvent $event, PublicEvent $publisher, AuthenticatedSessionController $auth, EventAccess $access)
    {
        ...
        if (! $request->session()->has('url.intended')) {
            $request->session()->put('url.intended', route('testcode.e.manage', $event->uuid));
        }
