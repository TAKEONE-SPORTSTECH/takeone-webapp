...
class PublicEntry
{
    ...
    public function enrol(ClubEvent $event, array $data, ?string $ip = null): array
    {
        ...
        try {
            ...
            if (! empty($data['photo'])) {
                ...
                /*
                 * Visible, and deliberately so (decided 2026-09-06) — the same
                 * decision as App\Events\Support\PublicEntry, which carries the
                 * full reasoning. In short: entering REQUIRES a photo, every
                 * competition surface then shows it, and hiding it left the
                 * athlete's own profile drawing a silhouette one tap away.
     ...
     *
     * A SLUG is what the page sends, never an id — an auto-increment id is not
     * a public key (CLAUDE.md → Unpredictable Resource Identifiers) — and it is
     * resolved here rather than trusted: an unknown or inactive slug simply
    ...
    private function resolveClub(array $data, ?ClubEvent $event = null): array
    {
         ...
         * temporary one has no tenant, so its NAME is carried and the entry is
         * linked to it separately once the registration exists.
         */
        if ($key !== '' && $event) {
            $onEvent = SandboxEventClub::with('tenant:id,club_name')
            ...
            if ($onEvent) {
                return [
                    ...
                    'event_club_id' => (int) $onEvent->id,
                ];
            }
        }

        /*
         * A club nobody put on this event is not an answer.
         *
         * The typed name survives — an organiser still wants to see where
         * somebody says they came from — but it names no club and confers
         * nothing.
         */
        ...
        return [
            'tenant_id' => null,
            'name' => $typed !== '' ? mb_substr($typed, 0, 120) : null,
            'event_club_id' => null,
        ];
    }

    /**
     * File a new entry under the event club it named.
     *
     * Separate from resolveClub() because a temporary club has nothing to put
     * in a foreign key: the link is the answer, and it can only be written once
     * the registration exists.
     */
    private function linkEventClub(?int $eventClubId, ?int $registrationId): void
    {
        if (! $eventClubId || ! $registrationId) {
            return;
        }

        \Illuminate\Support\Facades\DB::table('sandbox_event_club_entrants')->updateOrInsert(
            ['registration_id' => $registrationId],
            ['sandbox_event_club_id' => $eventClubId, 'updated_at' => now(), 'created_at' => now()]
        );
    }
        ...
        $registration = DB::transaction(function () use ($entry, $event, $athlete, $actor, $optionKeys) {
            $registration = ClubEventRegistration::updateOrCreate(
                ...
                [
                     ...
                     * board, the bracket, the entry list. But it is stored on
                     * their profile with `profile_picture_is_public` off (see
                     * store(), above), because coming to compete is not the same
                     * as publishing your face across the platform. Both of those
                     * are right, and together they meant the thing we insisted on
                     ...
                     * when the entry goes. So the photo does the job it was
                     * asked for, and their profile stays as private as they left
                     * it.
                     */
            ...
            /*
             * File them under the event club they named when they entered.
             *
             * Matched here rather than carried on the pending row, because a
             * request is not an entry: it holds a tenant id when the club has
             * an account and a NAME when it does not, and both of those find
             * the same row on the event's list without adding a column to a
             * table this experiment does not own.
             */
            $onEvent = SandboxEventClub::where('event_id', $event->id)
                ->where('state', '!=', 'declined')
                ->when(
                    $entry->representing_tenant_id,
                    fn ($q) => $q->where('tenant_id', $entry->representing_tenant_id),
                    fn ($q) => $q->whereNull('tenant_id')->whereRaw('lower(name) = ?', [mb_strtolower((string) $entry->club_name)])
                )
                ->first();

            $this->linkEventClub($onEvent?->id, (int) $registration->id);
    ...
    private function notifyOrganiser(ClubEvent $event, EventPublicEntry $entry): void
    {
        ...
        if (! $organiserId) {
            ...
            __('events.public_review_notify_title', ['title' => $event->title]), [
                ...
                'action_url' => route('testcode.me.events.manage', $event->uuid),
    ...
    private function notifyAthlete(EventPublicEntry $entry, bool $accepted, ?string $division): void
    {
        ...
        $body = $accepted
            ...
            /*
             * Where "you're in" actually takes them.
             *
             * It used to be `me.events.show`, which sits behind `verified` —
             * and this door makes an email OPTIONAL, so the very people it
             * notifies are the ones who can never pass that gate. The message
             * arrived and the link led to a verification wall.
             *
             * Their own entry needs `auth` and nothing more, and it is the
             * screen the news is about: what was accepted, and how to correct
             * it. A decline still goes to the poster, which needs nothing.
             */
            'action_url' => $accepted
                ? route('testcode.e.my-entry', $event->uuid)
                : route('testcode.e', $event->uuid),
            'subject_type' => (new ClubEvent)->getMorphClass(),
            'subject_id' => $event->id,
            'tenant_id' => $event->tenant_id,
        ]), null, false);
