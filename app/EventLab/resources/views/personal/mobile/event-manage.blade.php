...
@php
    ...
    if (! empty($scoringUrl)) {
        $links[] = [
            ...
            'href' => $scoringUrl,
            // A standalone document with its own shell — never swapped into the
            // mobile shell, which would strand the console inside a tab bar it
            // was not built for.
            'external' => true,
        ];
    }
...
<div @include('eventlab::partials.event-show-script') class="pb-4">
    ...
    <header class="m-hero -mx-4 -mt-4 px-5 pt-5 pb-8 text-white relative overflow-hidden"
        ...
        <div class="flex items-center justify-between relative z-50">
            <a href="{{ route('testcode.me.events.show', $e['key']) }}" data-shell-link data-route="testcode.me.events"
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="{{ isset($shell) ? url('/e/'.$e['key']) : route('testcode.me.events.show', $e['key']) }}"
                   @if(! isset($shell)) data-shell-link data-route="testcode.me.events" @endif
    ...
    @if(count($links))
        <div class="space-y-2">
            @foreach($links as $row)
                <a href="{{ $row['href'] }}"
                   @if(empty($row['external'])) data-shell-link data-route="testcode.me.events" @endif
    ...
    @if($canManage)
        ...
        <div class="space-y-2">
            ...
            <button type="button" @click="openResults()"
                ...
                <span class="min-w-0 flex-1">
                    <span class="block text-sm font-bold text-foreground">{{ __('personal.event_podium_title') }}</span>
            ...
            <x-eventlab::event-documents :event="$e['key']" :documents="$documents ?? []"
