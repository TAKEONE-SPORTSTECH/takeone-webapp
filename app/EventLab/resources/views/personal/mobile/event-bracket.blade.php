...
<div x-data="{
        ...
        saveUrlBase: '{{ url('testcode/me/events/'.$e['key'].'/categories') }}',
        ...
        async generateNewDraw() {
            ...
            this.busy = true;
            try {
                const res = await fetch('{{ route('testcode.me.events.action', [$e['key'], 'generate_draw']) }}', {
                    method: 'POST',
        ...
        <div class="flex items-center justify-between gap-2 relative z-50">
           ...
           aria-label="{{ isset($shell) ? __('personal.event_manage_title') : __('personal.event_show_event') }}" title="{{ isset($shell) ? __('personal.event_manage_title') : __('personal.event_show_event') }}">
            ...
            <div class="flex items-center gap-2">
                ...
                @endif
                <x-qr-code
                    :url="route('testcode.me.events.show', ['event' => $e['key']])"
            ...
            @if($c['status'] === 'enrolling')
                <div class="m-card rounded-2xl p-4" x-data="{
                        ...
                        async enter() {
                            ...
                            this.busy = true;
                            try {
                                const res = await fetch('{{ route('testcode.me.events.register', $e['key']) }}', {
                                    method: 'POST',
