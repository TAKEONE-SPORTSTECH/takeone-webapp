...
@section('title', __('eventlab::messages.clubs_title').' · '.$e['title'])
...
<div class="pb-4">
        ...
        <div class="flex items-center justify-between relative z-50" x-data>
            ...
            @if($canManage && collect($eventClubs)->contains('temporary', true))
                <button type="button" @click="$dispatch('clubs-promote')"
                        ...
                        aria-label="{{ __('eventlab::messages.clubs_promote') }}"
                        title="{{ __('eventlab::messages.clubs_promote') }}">
        ...
        <div class="relative z-10 mt-6">
            <div class="flex items-center gap-1.5 flex-wrap">
                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide bg-white/20 backdrop-blur">
                    <i class="bi bi-shield-fill-check"></i> {{ trans_choice('eventlab::messages.clubs_count', count($eventClubs), ['count' => count($eventClubs)]) }}
                </span>
            ...
            <h1 class="text-2xl font-black mt-3 leading-tight">{{ __('eventlab::messages.clubs_title') }}</h1>
    ...
    <div class="mt-4 relative z-10">
        <x-eventlab::event-clubs :event="$e['key']"
                                 :clubs="$eventClubs"
