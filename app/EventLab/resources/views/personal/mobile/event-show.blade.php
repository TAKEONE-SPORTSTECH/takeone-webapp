...
{{-- ⚠️ AN ADDRESS, NOT A GESTURE.
            ...
            @php
                ...
                $backHref = isset($shell) ? url('/e/'.$e['key']) : route('testcode.me.events');
           ...
           aria-label="{{ $backLabel }}" title="{{ $backLabel }}">
            ...
            <div class="flex items-center gap-2">
                ...
                @if(($canManage ?? false) || ($canOfficiate ?? false))
                    <a href="{{ route('testcode.me.events.manage', $e['key']) }}" data-shell-link data-route="testcode.me.events"
                ...
                @endif
                <x-qr-code
                    :url="$shareUrl ?? route('testcode.me.events.show', ['event' => $e['key']])"
    ...
    @php
        ...
        if ($hasBrackets) {
            ...
            $doors[] = [
                'href' => route('testcode.me.events.bracket', $e['key']),
        ...
        $doors[] = [
            'href' => route('testcode.me.events.gallery', $e['key']),
    ...
    <div class="px-4 mt-4">
        <div class="space-y-3">
            @foreach($doors as $d)
                <a href="{{ $d['href'] }}" data-shell-link data-route="testcode.me.events"
            ...
            {{-- Participants — the roster, on its own page. Bound to this page's
                 ...
                 without a reload. --}}
            <a href="{{ route('testcode.me.events.people', $e['key']) }}" data-shell-link data-route="testcode.me.events"
               class="m-press rounded-2xl p-4 text-white relative overflow-hidden shadow-lg flex items-center gap-3.5"
               style="background: linear-gradient(135deg, {{ $e['color'] }}, #1f2937);">
                <div class="absolute -right-6 -top-6 w-24 h-24 rounded-full bg-white/10"></div>
                <div class="relative w-12 h-12 rounded-2xl bg-white/15 border border-white/25 backdrop-blur grid place-items-center flex-shrink-0">
