...
@php
     ...
     */
    $submitUrl   = $isEdit
        ? (isset($shell) ? url('/e/'.$ev->uuid.'/admin') : route('testcode.me.events.update', $ev->uuid))
        : route('testcode.me.events.store');
...
<div x-data="{
        ...
        async save() {
            ...
            try {
                ...
                setTimeout(() => { window.location.href = data.redirect || '{{ route('testcode.me.events') }}'; }, 600);
...
{{-- ⚠️ AN ADDRESS, NOT A GESTURE.
                 ...
                 address. --}}
            @php
                $backHref = $isEdit ? route('testcode.me.events.show', $ev->uuid) : route('testcode.me.events');
