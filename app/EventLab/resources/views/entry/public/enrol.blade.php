...
@if($me)
    @include('eventlab::entry.public.partials.enrol-mine')
@else
<div x-data="enrolFlow()" class="-mx-4 -mt-4">
    ...
    <header class="relative overflow-hidden text-white"
        ...
        <div class="flex items-center justify-between gap-3 relative z-10">
            <a href="{{ route('testcode.e', ['event' => $e['uuid']]) }}"
        ...
        <div class="e-inR">
            ...
            <div style="background:#fff; border-radius:18px; border-top:3px solid {{ $ev }};
                ...
                <p class="text-[12px] text-muted-foreground">
                    ...
                    <a href="{{ route('testcode.e.manage', ['event' => $e['uuid']]) }}"
...
function enrolFlow() {
    return {
        ...
        async submit() {
            ...
            this.fieldErrors = {};
            try {
                const res = await fetch(@js(route('testcode.e.enter.store', ['event' => $e['uuid']])), {
                    method: 'POST',
                    headers: {
                ...
                this.sent = true;
                window.location.href = d.redirect || @js(route('testcode.e', ['event' => $e['uuid']]));
            } catch (e) {
                this.saving = false;
                notice(e.message);
            }
