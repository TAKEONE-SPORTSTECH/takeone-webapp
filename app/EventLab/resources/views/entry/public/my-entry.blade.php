...
{{--
    ...
    in App\EventLab\Support\EntryEditor, and every rule about leaving lives in
...
@php
    ...
    $islandProps = [
        ...
        'eventTitle' => $e['title'],
        'backUrl'    => route('testcode.e', ['event' => $e['key']]),
        'urls' => [
            'state'      => route('testcode.e.my-entry.state', ['event' => $event->uuid]),
            'update'     => route('testcode.e.my-entry.update', ['event' => $event->uuid]),
            'withdraw'   => route('testcode.e.my-entry.withdraw', ['event' => $event->uuid]),
            'clubSearch' => route('testcode.e.enter.clubs', ['event' => $event->uuid]),
        ],
        'entry'       => $entry,
...
function myEntryPhoto() {
    return {
        ...
        async savePhoto(base64) {
            ...
            try {
                const res = await fetch(@js(route('testcode.e.my-entry.update', ['event' => $event->uuid])), {
                    method: 'PUT',
                    headers: {
...
<div x-data="myEntry()" class="-mx-4 -mt-4">
    ...
    <header class="relative overflow-hidden text-white"
        ...
        <div class="flex items-center justify-between gap-3 relative z-10">
            <a href="{{ route('testcode.e', ['event' => $e['key']]) }}"
    ...
    <div class="mx-auto w-full max-w-lg px-4 -mt-3 relative z-10">
        <div class="pb-[max(4rem,calc(3rem+env(safe-area-inset-bottom)))] mobile-stagger space-y-3.5">
            ...
            {{-- ===== The fee =====
                 ...
                 none of which this screen used to say. There is no gateway by
                 decision, so settling is: transfer, photograph the receipt,
                 send it, and the organiser confirms. The state line is the part
                 that matters: somebody who has paid and cannot tell whether it
                 arrived phones the organiser. --}}
            @if($payment)
                <div class="mt-5">
                    <h2 class="text-[11px] font-bold uppercase text-muted-foreground px-1 mb-2" style="letter-spacing:.1em;">
                        {{ __('eventlab::messages.pay_title') }}
                    </h2>
                    ...
                    <div class="m-card rounded-2xl overflow-hidden">
                        ...
                        <template x-if="pay.state !== 'approved'">
                            <div class="border-t border-gray-100 px-4 py-3.5">
                                <p class="text-[11.5px] text-muted-foreground leading-relaxed">{{ __('eventlab::messages.pay_how') }}</p>

                                {{-- The club's own account, so the transfer can
                                     actually be made from this screen. --}}
                                <template x-if="pay.bank">
                                    <div class="mt-3 rounded-xl p-3" style="background: {{ Palette::alpha($ev, .06) }};">
                                        <p class="text-[10px] font-bold uppercase text-muted-foreground" style="letter-spacing:.1em;">
                                            {{ __('eventlab::messages.pay_account') }} <span x-text="pay.club"></span>
                                        </p>
                                ...
                                <label class="m-press mt-3 w-full h-12 rounded-2xl font-black text-[13.5px] text-white
                                    ...
                                    <span x-text="pay.has_proof
                                        ? @js(__('eventlab::messages.pay_replace_proof'))
                                        : @js(__('eventlab::messages.pay_send_proof'))"></span>
            ...
            <div class="mt-5">
                <h2 class="text-[11px] font-bold uppercase text-muted-foreground px-1 mb-2" style="letter-spacing:.1em;">
                    {{ __('eventlab::messages.contact_title') }}
                </h2>
                            ...
                            <button type="button" @click="editable(row.field) && openSheet(row.field)"
                                ...
                                <span class="min-w-0 flex-1">
                                    ...
                                    <span x-show="row.field === 'email' && entry.email && !entry.email_verified" x-cloak
                                        ...
                                        <i class="bi bi-exclamation-circle-fill me-1"></i>{{ __('eventlab::messages.contact_unverified') }}
                                    </span>
                                    <span x-show="!editable(row.field)" x-cloak
                                          class="block text-[11px] text-muted-foreground mt-1" x-text="reason(row.field)"></span>
                                </span>
                ...
                <p class="text-[11.5px] text-muted-foreground mt-2 px-1 flex items-start gap-1.5">
                    ...
                    <span>{{ __('eventlab::messages.contact_hint') }}</span>
                </p>
            </div>
            ...
            <div x-show="sheet"
                ...
                <div class="flex-1 overflow-y-auto px-5 pt-4 pb-3">
                    ...
                    <div x-show="sheet === 'email'">
                        <label class="block text-[12px] font-bold text-foreground mb-1.5">{{ __('eventlab::messages.contact_email_label') }}</label>
                        <input type="email" x-model="draft.email" maxlength="190" autocomplete="email"
                               inputmode="email" placeholder="{{ __('eventlab::messages.contact_email_placeholder') }}"
                               class="e-field w-full h-12 px-4 rounded-2xl text-[15px]">
                        <p class="text-[11.5px] text-muted-foreground mt-2 flex items-start gap-1.5">
                            <i class="bi bi-info-circle-fill mt-0.5 flex-shrink-0"></i>
                            <span>{{ __('eventlab::messages.contact_hint') }}</span>
                    ...
                    <div x-show="sheet === 'mobile'">
                        <label class="block text-[12px] font-bold text-foreground mb-1.5">{{ __('eventlab::messages.contact_mobile_label') }}</label>
...
<div class="px-4 pb-[max(2rem,calc(1.5rem+env(safe-area-inset-bottom)))]">
    <form method="POST" action="{{ route('testcode.e.sign-out', ['event' => $event->uuid]) }}">
        @csrf
        <button type="submit"
...
function myEntry() {
    return {
        ...
        /* The fee, as the server last described it. Re-read on every refresh
           like everything else on this panel. */
        pay: @js($payment ?? null) || {},
        payBusy: false,

        get payLabel() {
            if (this.pay.state === 'approved')  return @js(__('eventlab::messages.pay_approved'));
            if (this.pay.state === 'submitted') return @js(__('eventlab::messages.pay_submitted'));
            return @js(__('eventlab::messages.pay_owed'));
        },
        ...
        /* A photograph of the receipt. Read in the browser, posted as a data
           ...
           every other image on the platform takes. */
        async sendProof(ev) {
            const file = ev.target.files && ev.target.files[0];
            ev.target.value = '';
            ...
            try {
                ...
                const res = await fetch(@js(route('testcode.e.my-entry.payment-proof', ['event' => $event->uuid])), {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content || '',
                ...
                if (!res.ok || !data.success) {
                    window.showToast('error', data.message || @js(__('eventlab::messages.pay_proof_rejected')));
                    return;
                }
                ...
                window.showToast('error', @js(__('eventlab::messages.pay_proof_rejected')));
            } finally {
                this.payBusy = false;
            }
        },
        ...
        sheet: '',
        draft: { weight: null, belt_colour: null, belt_grade: '', club: '', name: '', birthdate: '', gender: '', reason: '' },
        saving: false,
        notices: [],
        _n: 0,
        ...
        /* The account rows, kept apart from the entry rows so the two cards
           stay two answers to two different questions. */
        get contactRows() {
            return [
                { field: 'email',  icon: 'bi-envelope-fill', label: @js(__('eventlab::messages.contact_email_label')) },
                { field: 'mobile', icon: 'bi-telephone-fill', label: @js(__('eventlab::messages.contact_mobile_label')) },
            ];
        },
        ...
        value(f) {
            ...
            if (f === 'mobile')    return this.entry.mobile || '—';
            return '—';
        },
        ...
        get sheetTitle() {
            return {
                ...
                email:     @js(__('eventlab::messages.contact_email_label')),
                mobile:    @js(__('eventlab::messages.contact_mobile_label')),
        ...
        commit() {
            ...
            /* Both go up together whenever one changes: the server refuses a
               change that would leave the account with neither, and it can only
               judge that if it can see both. */
        ...
        async withdraw() {
            ...
            try {
                const res = await fetch(@js(route('testcode.e.my-entry.withdraw', ['event' => $event->uuid])), {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
        ...
        async takeBack() {
            ...
            try {
                const res = await fetch(@js(route('testcode.e.my-entry.withdraw.cancel', ['event' => $event->uuid])), {
                    method: 'DELETE',
                    headers: {
                        'Accept': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]')?.content || '',
        ...
        async refresh() {
            try {
                const res = await fetch(@js(route('testcode.e.my-entry.state', ['event' => $event->uuid])), {
        ...
        async searchClubs() {
            ...
            try {
                const url = @js(route('testcode.e.enter.clubs', ['event' => $event->uuid])) + '?q=' + encodeURIComponent(q);
