...
@forelse($e['gallery']['divisions'] as $div)
    <p class="px-4 mt-4 text-[9.5px] font-bold uppercase tracking-[0.16em] text-muted-foreground/70">
        {{ $div['title'] }}@if($div['subtitle'] ?? null) <span class="normal-case tracking-normal font-semibold">· {{ $div['subtitle'] }}</span>@endif
    </p>

    {{-- A shelf per division: the tiles scroll sideways so a division
         with nine bouts does not push the sections below it off the
         bottom of the page. --}}
    <div class="overflow-x-auto mt-2 px-4 pb-1">
        <div class="flex gap-2.5 w-max">
            @foreach($div['bouts'] as $b)
                <button type="button" class="m-press text-start flex-none w-[190px]"
                    ...
                    <span class="block relative overflow-hidden rounded-xl aspect-video" style="background:#0b132b;">
                        @if($b['poster'] ?? null)
                            <img src="{{ $b['poster'] }}" alt="" loading="lazy" class="w-full h-full object-cover opacity-90">
                        @endif
                        <span class="absolute inset-0 grid place-items-center">
                            <span class="w-9 h-9 rounded-full grid place-items-center text-white text-sm bg-black/45 border border-white/35">
