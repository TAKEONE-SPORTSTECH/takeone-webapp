{{--
    ...
    Expects $e, $me, $alreadyIn, $alreadyAsked, $belts, $ev.
--}}
                ...
                @if($alreadyIn)
                    <a href="{{ route('testcode.e.my-entry', ['event' => $e['key']]) }}"
                       ...
                       style="background: {{ $ev }}; box-shadow: 0 18px 40px -18px {{ $ev }};">
                        {{ __('eventlab::messages.my_entry_open') }}<i class="bi bi-arrow-right"></i>
                    </a>
                @endif
...
function enrolMine() {
    return {
        ...
        async searchClubs() {
            ...
            try {
                const url = @js(route('testcode.e.enter.clubs', ['event' => $e['uuid']])) + '?q=' + encodeURIComponent(q);
        ...
        async submit() {
            ...
            this.saving = true;
            try {
                const res = await fetch(@js(route('testcode.e.enter.mine', ['event' => $e['uuid']])), {
                    method: 'POST',
                    headers: {
