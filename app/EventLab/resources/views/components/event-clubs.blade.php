...
@php
    ...
    $clubUrls = [
        'search'  => route('testcode.me.events.clubs.search', $event),
        'store'   => route('testcode.me.events.clubs.store', $event),
        'invite'  => route('testcode.me.events.clubs.invite', $event),
        'update' => route('testcode.me.events.clubs.update', ['event' => $event, 'eventClub' => '__UUID__']),
        'destroy' => route('testcode.me.events.clubs.destroy', ['event' => $event, 'eventClub' => '__UUID__']),
        'entrants' => route('testcode.me.events.clubs.entrants', $event),
        'athletes' => route('testcode.me.events.clubs.athletes', ['event' => $event, 'eventClub' => '__UUID__']),
        'promote' => route('testcode.me.events.clubs.promote', $event),
    ];

    $clubLabels = [
        'tab_invite' => __('eventlab::messages.clubs_tab_invite'),
        'tab_new' => __('eventlab::messages.clubs_tab_new'),
        'state_listed' => __('eventlab::messages.club_state_listed'),
        'state_invited' => __('eventlab::messages.club_state_invited'),
        'state_accepted' => __('eventlab::messages.club_state_accepted'),
        'state_declined' => __('eventlab::messages.club_state_declined'),
        'remove_confirm' => __('eventlab::messages.club_remove_confirm'),
        'remove_body' => __('eventlab::messages.club_remove_body'),
        'remove_body_n' => __('eventlab::messages.club_remove_body_n'),
        'shape_square' => __('eventlab::messages.shape_square'),
        'shape_circle' => __('eventlab::messages.shape_circle'),
        'edit' => __('eventlab::messages.club_edit'),
        'save' => __('eventlab::messages.club_save'),
        'add' => __('eventlab::messages.clubs_add'),
        'remove' => __('shared.delete'),
        'athletes' => __('eventlab::messages.club_athletes'),
        'athletes_none' => __('eventlab::messages.club_athletes_none'),
        'athletes_taken' => __('eventlab::messages.club_athletes_taken'),
        'temporary' => __('eventlab::messages.club_temporary'),
        'promoted' => __('eventlab::messages.club_promoted'),
        'promote' => __('eventlab::messages.clubs_promote'),
        'promote_confirm' => __('eventlab::messages.clubs_promote'),
    ];
@endphp
...
<div x-data="eventClubs()" x-init="init()"
    ...
    @if($canManage)
        <button type="button" @click="open()"
            ...
            <span class="min-w-0 flex-1">
                <span class="block text-[15px] font-black text-foreground leading-tight">{{ __('eventlab::messages.clubs_add') }}</span>
                <span class="block text-xs text-muted-foreground mt-0.5">{{ __('eventlab::messages.clubs_tile_sub') }}</span>
            </span>
    ...
    <div class="mt-2.5 space-y-2.5">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <p class="text-xs text-muted-foreground mt-1 flex items-center gap-2 flex-wrap">
                        ...
                        <span class="inline-flex items-center gap-1 font-bold"
                            ...
                            <span x-text="club.athletes
                                ? @js(__('eventlab::messages.club_athletes')).replace(':n', club.athletes)
                                : @js(__('eventlab::messages.club_athletes_none'))"></span>
                        </span>
                    </p>
                    ...
                    <p class="text-xs mt-1 flex items-center gap-2 flex-wrap">
                        <span x-show="club.temporary" x-cloak
                            ...
                            <i class="bi bi-clock-history"></i>{{ __('eventlab::messages.club_temporary') }}
                        </span>
                        ...
                        <span x-show="club.promoted" x-cloak
                            ...
                            <i class="bi bi-patch-check-fill"></i>{{ __('eventlab::messages.club_promoted') }}
                        </span>
        ...
        <div x-show="!clubs.length" x-cloak class="bg-white rounded-2xl shadow-sm border border-gray-100">
            @include('eventlab::entry.public.partials.empty', [
                ...
                'title' => __('eventlab::messages.clubs_empty'),
            ])
        </div>
    ...
    @if($canManage)
        {{-- The rule, written where the clubs are. Promotion itself runs on
             the nightly sweep (or the @clubs-promote window event). --}}
        <div x-show="clubs.some(c => c.temporary)" x-cloak class="mt-2 px-1">
            <p class="text-[11.5px] text-muted-foreground flex items-start gap-1.5">
                ...
                <span>{{ __('eventlab::messages.club_temporary_hint') }}</span>
            </p>
        </div>
            ...
            <div x-show="sheet"
                    ...
                    <div class="relative flex items-start gap-3">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="text-[12px] text-white/85 mt-0.5"
                               x-text="editing ? editing.name : @js(__('eventlab::messages.clubs_sheet_hint'))"></p>
                        </div>
                ...
                <div class="flex-1 overflow-y-auto px-5 py-4">
                    ...
                    <div x-show="mode === 'invite' && !editing" class="mt-4">
                        <label class="block text-[12px] font-bold text-foreground mb-1.5">{{ __('eventlab::messages.clubs_search_label') }}</label>
                        <div class="relative">
                            <i class="bi bi-search absolute start-3.5 top-1/2 -translate-y-1/2 text-muted-foreground text-sm"></i>
                            <input type="search" x-model="query" @input.debounce.300ms="find()"
                                   placeholder="{{ __('eventlab::messages.clubs_search_placeholder') }}"
                        ...
                        <div class="mt-3 space-y-2">
                            ...
                            <p x-show="query.length >= 2 && !results.length && !searching" x-cloak
                               class="text-[12px] text-muted-foreground text-center py-4">{{ __('eventlab::messages.clubs_search_none') }}</p>
                        </div>

                        <p class="text-[11.5px] text-muted-foreground mt-3 flex items-start gap-1.5">
                            <i class="bi bi-info-circle-fill mt-0.5 flex-shrink-0"></i>
                            <span>{{ __('eventlab::messages.clubs_search_hint') }}</span>
                        </p>
                    </div>
                    ...
                    <div x-show="mode === 'new'" class="mt-4 space-y-4">
                        <div>
                            <label class="block text-[12px] font-bold text-foreground mb-1.5">{{ __('eventlab::messages.club_name_label') }}</label>
                            <input type="text" x-model="form.name" maxlength="120"
                                   placeholder="{{ __('eventlab::messages.club_name_placeholder') }}"
                                   class="w-full h-12 px-4 rounded-2xl border border-gray-200 text-[15px] bg-white
                                          focus:ring-2 focus:border-transparent" style="--tw-ring-color: {{ $ev }};">
                        </div>

                        <div>
                            <label class="block text-[12px] font-bold text-foreground mb-1.5">{{ __('eventlab::messages.club_logo_label') }}</label>
                                ...
                                <div class="min-w-0 flex-1">
                                    <button type="button" @click="pickLogo()"
                                        ...
                                        <span x-text="form.logo
                                            ? @js(__('eventlab::messages.club_logo_change'))
                                            : @js(__('eventlab::messages.club_logo_pick'))"></span>
                                    </button>
                                    <p class="text-[11px] text-muted-foreground mt-1.5 leading-relaxed">{{ __('eventlab::messages.club_logo_hint') }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            <label class="flex items-center gap-2 text-[12px] font-bold text-foreground mb-1.5">
                                {{ __('eventlab::messages.club_instagram_label') }}
                                <span class="px-2 py-0.5 rounded-full bg-muted text-[10px] font-bold text-muted-foreground uppercase" style="letter-spacing:.08em;">
                                    {{ __('eventlab::messages.club_instagram_optional') }}
                                </span>
                            </label>
                            <div class="relative">
                                ...
                                <input type="text" x-model="form.instagram" maxlength="200" dir="ltr"
                                       placeholder="{{ __('eventlab::messages.club_instagram_placeholder') }}"
                                       class="w-full h-12 ps-10 pe-4 rounded-2xl border border-gray-200 text-[15px] bg-white
                                              focus:ring-2 focus:border-transparent" style="--tw-ring-color: {{ $ev }};">
                            </div>
                            <p class="text-[11.5px] text-muted-foreground mt-2 flex items-start gap-1.5">
                                <i class="bi bi-info-circle-fill mt-0.5 flex-shrink-0"></i>
                                <span>{{ __('eventlab::messages.club_instagram_hint') }}</span>
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex-shrink-0 px-5 pt-3 border-t border-gray-100 bg-background"
                    ...
                    <button type="button" @click="submit()" :disabled="busy || !ready"
                            ...
                            style="background: {{ $ev }};">
                        <i class="bi" :class="mode === 'invite' && !editing ? 'bi-send-fill' : 'bi-check-lg'"></i>
                        <span x-text="editing
                            ? labels.save
                            : (mode === 'invite'
                                ? @js(__('eventlab::messages.club_invite'))
                                : @js(__('eventlab::messages.club_add_cta')))"></span>
                    </button>
                </div>
            </div>
        </div>
            ...
            <div x-show="athletesFor"
                ...
                <div class="flex-shrink-0 px-5 pt-3 pb-4 rounded-t-3xl text-white relative overflow-hidden"
                    ...
                    <div class="relative flex items-start gap-3">
                        ...
                        <div class="min-w-0 flex-1">
                            <h3 class="text-lg font-black leading-tight">{{ __('eventlab::messages.club_athletes_title') }}</h3>
                ...
                <div class="flex-1 overflow-y-auto px-5 py-4">
                    <p class="text-[11.5px] text-muted-foreground mb-3">{{ __('eventlab::messages.club_athletes_hint') }}</p>

                    <div class="relative mb-3">
                        <i class="bi bi-search absolute start-3.5 top-1/2 -translate-y-1/2 text-muted-foreground text-sm"></i>
                        <input type="search" x-model="athleteQuery"
                               placeholder="{{ __('eventlab::messages.club_athletes_search') }}"
                ...
                <div class="flex-shrink-0 px-5 pt-3 border-t border-gray-100 bg-background"
                    ...
                    <button type="button" @click="saveAthletes()" :disabled="busy"
                        ...
                        <i class="bi bi-check-lg"></i>{{ __('eventlab::messages.club_athletes_save') }}
                    </button>
                </div>
            </div>
        </div>
    ...
    {{-- ===== The crop editors =====
         ...
         "Teleport fixed overlays to body"). Parked at `left:-9999px` rather
    ...
    <div aria-hidden="true"
        ...
        <x-takeone-cropper id="clubLogoSquare" mode="form" :inline="true"
                           ...
                           :uploadAsIs="true"
                           saveText="{{ __('eventlab::messages.club_logo_crop') }}"
                           uploadAsIsText="{{ __('eventlab::messages.club_logo_as_is') }}" />
