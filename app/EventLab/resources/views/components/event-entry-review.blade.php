...
<div x-data="{
        ...
        async refresh() {
            ...
            this.loading = true;
            try {
                const res = await fetch(@js(route('testcode.me.events.public-entries', $event)), {
        ...
        async decide(uuid, verdict) {
            ...
            try {
                const url = verdict === 'accept'
                    ? @js(route('testcode.me.events.public-entries.accept', ['event' => $event, 'entry' => '__U__']))
                    : @js(route('testcode.me.events.public-entries.decline', ['event' => $event, 'entry' => '__U__']));
