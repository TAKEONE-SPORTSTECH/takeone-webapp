                   ...
                   class="m-press flex items-center gap-3 m-card p-3 active:bg-muted/40 transition-colors">
                    ...
                    <span class="w-11 h-11 rounded-full bg-muted flex items-center justify-center overflow-hidden flex-shrink-0">
                        @if($u->profile_picture)
                            <img src="{{ file_url($u->profile_picture) }}?v={{ optional($u->updated_at)->timestamp }}" alt="" class="w-[33px] h-11 object-cover">
                        @else
            ...
            <div x-show="bulkSheetOpen"
                ...
                <div class="flex-shrink-0 px-5 pt-3 pb-4 rounded-t-3xl text-white relative overflow-hidden"
                    ...
                    <div class="relative flex items-start gap-3">
                        ...
                        <div class="min-w-0 flex-1">
                            <h3 class="text-lg font-black leading-tight">{{ __('admin.club_members_index_bulk_enroll_modal_title') }}</h3>
                            <p class="text-[12px] text-white/85 mt-0.5">{{ __('admin.club_members_index_bulk_enroll_modal_subtitle') }}</p>
                        </div>
                        <button type="button" @click="if (!bulkSubmitting) bulkSheetOpen = false" aria-label="{{ __('shared.close') }}"
...
@include('clubs::members.partials.mobile-add-member')
