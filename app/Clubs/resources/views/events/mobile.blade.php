...
<div class="-mx-4 -mt-4"
                            ...
                            <div class="flex items-center gap-1.5 flex-shrink-0" @click.stop>
                                ...
                                <div class="relative">
                                    ...
                                    <div x-show="openMenu" x-cloak @click.outside="openMenu = false" class="absolute right-0 mt-1 w-44 bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden z-20">
                                        {{-- The event's own page, first in the menu — see the note in events/index. --}}
                                        <a href="{{ route('me.events.show', $e->uuid) }}" class="w-full text-left px-4 py-3 text-sm text-foreground hover:bg-muted/60 flex items-center gap-3"><span class="w-7 h-7 rounded-lg bg-primary/10 flex items-center justify-center shrink-0"><i class="bi bi-box-arrow-up-right text-primary text-xs"></i></span><span class="font-medium">{{ __('personal.event_show_event') }}</span></a>
    ...
    @include('clubs::events.mobile-form')
