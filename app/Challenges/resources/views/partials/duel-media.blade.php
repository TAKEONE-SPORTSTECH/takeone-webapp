...
<div class="flex items-start justify-between gap-2">
    <div>
        <h3 class="text-sm font-bold text-foreground flex items-center gap-2"><i class="bi bi-camera-reels-fill text-primary"></i> {{ __('challenge.partials_duel_media_title') }}</h3>
        <p class="text-[11px] text-muted-foreground mt-0.5">{{ __('challenge.partials_duel_media_subtitle') }}</p>
    </div>
        ...
        <div class="relative group aspect-square rounded-xl overflow-hidden bg-muted border border-border">
            ...
            <template x-if="m.type === 'link'">
                <a :href="m.url" target="_blank" rel="noopener" class="w-full h-full grid place-items-center text-center p-2 bg-accent">
                    <span><i class="bi bi-play-btn-fill text-2xl text-primary"></i><span class="block text-[10px] font-semibold text-primary mt-1 truncate" x-text="m.caption || '{{ __('challenge.partials_duel_media_video_link') }}'"></span></span>
                </a>
            </template>
            <button type="button" x-show="m.mine" @click.prevent="removeMedia(m)"
                    class="absolute top-1 end-1 w-6 h-6 rounded-full bg-black/55 text-white grid place-items-center text-xs"><i class="bi bi-x-lg"></i></button>
        </div>
    </template>
</div>
<div x-show="!media.length" x-cloak class="mt-3 rounded-xl border border-dashed border-border bg-muted/30 py-4 text-center">
    ...
    <p class="text-[11px] text-muted-foreground mt-1">{{ __('challenge.partials_duel_media_empty') }}</p>
</div>
...
<div class="mt-3">
    <div class="grid grid-cols-3 gap-2">
        <label class="m-press cursor-pointer rounded-xl border-2 border-dashed border-border hover:border-primary hover:bg-accent/40 transition-colors flex flex-col items-center justify-center py-3 gap-1"
            ...
            <span class="text-[11px] font-bold text-foreground">{{ __('challenge.partials_duel_media_photo') }}</span>
        ...
        <label class="m-press cursor-pointer rounded-xl border-2 border-dashed border-border hover:border-primary hover:bg-accent/40 transition-colors flex flex-col items-center justify-center py-3 gap-1"
            ...
            <span class="text-[11px] font-bold text-foreground">{{ __('challenge.partials_duel_media_video') }}</span>
        ...
        <button type="button" @click="showLink = !showLink"
            ...
            <span class="text-[11px] font-bold text-foreground">{{ __('challenge.partials_duel_media_link') }}</span>
        </button>
    </div>
    ...
    <div x-show="showLink" x-cloak
        ...
        <input x-model="linkUrl" type="url" placeholder="{{ __('challenge.partials_duel_media_link_placeholder') }}"
        ...
        <button type="button" @click="addLink()" :disabled="mediaBusy"
                class="m-press px-4 py-2.5 rounded-xl text-white text-sm font-bold disabled:opacity-50 flex-shrink-0" style="background: {{ $d['color'] }};">{{ __('challenge.partials_duel_media_add') }}</button>
    </div>

    <div x-show="mediaBusy" x-cloak class="flex items-center justify-center gap-2 mt-2 text-[11px] font-semibold text-primary">
        <i class="bi bi-arrow-repeat animate-spin"></i> {{ __('challenge.partials_duel_media_uploading') }}
    </div>
</div>
