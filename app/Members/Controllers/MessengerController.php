...
use App\Members\Models\Conversation;
...
class MessengerController extends Controller
{
    ...
    public function searchUsers(Request $request)
    {
        ...
        $connectedIds = \App\Members\Models\UserConnection::where('status', 'accepted')
        ...
        $blockedIds = \App\Members\Models\UserBlock::where('blocker_id', $me->id)->pluck('blocked_id')
            ->merge(\App\Members\Models\UserBlock::where('blocked_id', $me->id)->pluck('blocker_id'));
    ...
    private function pushRealtime(Conversation $conversation, Message $message, string $action = 'new'): void
    {
        ...
        // The tray is served by the SAME publish above: the Android app holds an
        // MQTT connection in a foreground service and posts what arrives while
        // the app is not in front (MqttNotificationService). There is no second
        // delivery path to fan out to.
    }
    ...
    private function pick(string $view): string
    {
        ...
        $mobile = 'members::messenger.mobile';

        return $isMobile && view()->exists($mobile) ? $mobile : 'members::messenger.index';
    }
}
