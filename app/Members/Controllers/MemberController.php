...
class MemberController extends Controller
{
    ...
    public function index(KinshipService $kin)
    {
        ...
        $view = $isMobile && view()->exists('members::family.mobile.index') ? 'members::family.mobile.index' : 'members::family.index';
    ...
    {
        return view('members::family.create');
    }
    ...
    public function show($uuid, \App\Support\ProfileHistory $profileHistory)
    {
        ...
        $challengesTotal = \App\Challenges\Models\Duel::where('status', 'completed')
            ->where(fn ($q) => $q->where('challenger_id', $memberId)->orWhere('opponent_id', $memberId))
            ->count();
        $challengeWins = \App\Challenges\Models\Duel::where('status', 'completed')->where('winner_id', $memberId)->count();
        ...
        $memberChallenges = \App\Challenges\Models\Duel::where(fn ($q) => $q->where('challenger_id', $memberId)->orWhere('opponent_id', $memberId))
            ->with([
            ...
            ->map(function ($d) use ($memberId) {
                ...
                return (object) [
                    ...
                    'metric' => $d->metric,
                    'format' => \App\Challenges\Models\Duel::formatLabel($d->format),
                    'type' => $d->type,
                    'status' => $d->status,
        ...
        $memberView = $isMobile
            ? 'members::profile.mobile.show'
            : 'members::profile.show';
    ...
    public function edit($id)
    {
        ...
        return view('members::profile.edit', compact('relationship'));
    }
    ...
    public function update(UpdateMemberRequest $request, $id)
    {
        ...
        if ($request->input('remove_profile_picture') == '1') {
            // Only if nothing else still names the file. An event entry can
            // point AT a profile picture rather than carry a copy, and duplicate
            // accounts share one path — so an unguarded delete here blanked a
            // competitor on every entry list, draw and hall screen at once.
            // See App\Events\Support\EntryPhoto.
        ...
        /*
         * ⚠️ Only when the request actually CARRIED it.
         *
         * A checkbox that is off is not submitted, so `has()` cannot
         * tell "the member switched their picture to private" from "this
         * form has no privacy toggle on it at all" — and the old
         * `has() ? true : false` read the second as the first. Every
         * save through a form without that field silently turned the
         * member's face private, which is how a club's whole entry list
         * ended up with no photographs on it.
         *
         * Absent is not the same as blank (CLAUDE.md → Who Fills The
         * Form). Forms that DO carry the toggle post a hidden `0`
         * alongside it (components/partials/profile-modal-fields), so an
         * unchecked switch still records false and reaches this branch.
         */
        if ($request->has('profile_picture_is_public')) {
            $optional['profile_picture_is_public'] = $request->boolean('profile_picture_is_public');
        }
    ...
    public function uploadPicture(UploadImageRequest $request, $id)
    {
        ...
        try {
            ...
            // The OLD one goes only if nothing else still names it — an event
            // entry that points at this exact path, one of the member's own
            // gallery photos, or a duplicate account carrying the same value.
            // See App\Events\Support\EntryPhoto.
            EntryPhoto::discardShared($member->profile_picture, $member->id);
    ...
    public function removeProfilePicture($id)
    {
        ...
        // Same guard as the other two paths: the row above has already given
        // up its claim, so what remains is somebody ELSE naming this file — an
        // event entry, or a duplicate account. See App\Events\Support\EntryPhoto.
        EntryPhoto::discardShared($member->profile_picture, $member->id);
    ...
    public function requestWorkVerification(Request $request, $id, string $uuid, \App\Services\AchievementVerificationService $service)
    {
        ...
        $work = \App\Members\Models\MemberWorkHistory::where('uuid', $uuid)
    ...
    public function storeMemberEvent(\App\Http\Requests\StoreMemberEventRequest $request, $id)
    {
        ...
        $event = \App\Members\Models\MemberEvent::create([
            'user_id' => $id,
            'title' => $validated['title'],
