...
class FamilyController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        return view('members::family.index', compact('user', 'dependents'));
    }
    ...
    public function editProfile()
    {
        ...
        return view('members::family.profile-edit', compact('user'));
    }
    ...
    public function updateProfile(UpdateProfileRequest $request)
    {
        ...
        if ($request->input('remove_profile_picture') == '1') {
            ...
            EntryPhoto::discardShared($user->profile_picture, $user->id);
        ...
        // Profile picture visibility — only when the request carried it. An
        // unchecked switch is not submitted, so writing false on absence turned
        // a member's face private on every save through a form with no such
        // toggle. See the note in update() below.
        if ($request->has('profile_picture_is_public')) {
            $validated['profile_picture_is_public'] = $request->boolean('profile_picture_is_public');
        } else {
            unset($validated['profile_picture_is_public']);
        }
    ...
    public function update(UpdateFamilyMemberRequest $request, $id)
    {
        ...
        if ($request->input('remove_profile_picture') == '1') {
            ...
            // point AT a profile picture rather than carry a copy, and duplicate
            // accounts share one path. See App\Events\Support\EntryPhoto.
            EntryPhoto::discardShared($dependent->profile_picture, $dependent->id);
    ...
    public function uploadFamilyMemberPicture(UploadImageRequest $request, $id)
    {
        ...
        try {
            ...
            // point AT a profile picture rather than carry a copy, and duplicate
            // accounts share one path. See App\Events\Support\EntryPhoto.
            if ($familyMember->profile_picture !== $fullPath) {
                EntryPhoto::discardShared($familyMember->profile_picture, $familyMember->id);
            }
