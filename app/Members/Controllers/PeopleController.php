...
class PeopleController extends Controller
{
    ...
    public function index(Request $request, PeopleRecommendationService $recommender)
    {
        ...
        return view($isMobile ? 'members::people.mobile.index' : 'members::people.desktop.index', compact('suggestions', 'hasConfirmedClub'))
    ...
    public function search(Request $request)
    {
        ...
        $blockedIds = \App\Members\Models\UserBlock::idsBlockedEitherWayWith($me->id);
    ...
    /**
     * The same public profile, reached from inside a sealed event page.
     *
     * A thin wrapper and deliberately nothing else — the authorization, the
     * payload and the view are `show()`'s, unchanged.
     *
     * It exists for one reason: Laravel hands controller arguments over in
     * ROUTE ORDER, and the sealed URI is `/e/{event:uuid}/admin/person/{uuid}`.
     * `show(Request, string $uuid)` therefore received the EVENT's uuid, looked
     * for a user with it, and 404'd on every name an organiser tapped. Typing
     * the event here consumes it as a bound model, so the scalar that follows is
     * the person's uuid.
     */
    public function showInEvent(Request $request, \App\Models\ClubEvent $event, string $uuid)
    {
        // Where BACK goes. This page is reachable from several places inside
        // the sealed app — the public participants list, the draw, an
        // officiating sheet, the organiser's entry list — and it used to send
        // everybody to /e/{uuid}/admin/people regardless. A reader who came
        // from the public participants list was dropped on the organiser's
        // screen, which is both the wrong page and, for most of them, one they
        // may not even open.
        //
        // The sender says where it came from with `?from=`, and this maps that
        // KEY to a route. Never a URL from the query string — that is an open
        // redirect with extra steps; an unknown key simply falls through to the
        // default.
        view()->share('sealedBack', $this->sealedBackUrl($request, $event));

        return $this->show($request, $uuid);
    }

    /**
     * The back destination for a profile opened inside a sealed event.
     *
     * Default when nothing says otherwise: the organiser's entry list for
     * somebody who may manage the event, and the public participants list for
     * everybody else — so the fallback is at least a page the reader is allowed
     * to open.
     */
    private function sealedBackUrl(Request $request, \App\Models\ClubEvent $event): string
    {
        $uuid = $event->uuid;

        $sections = ['participants', 'draw', 'officials', 'gallery'];
        $from = (string) $request->query('from', '');

        if (in_array($from, $sections, true)) {
            return route('events.public.section', ['event' => $uuid, 'section' => $from]);
        }

        if ($from === 'poster') {
            return route('events.public', ['event' => $uuid]);
        }

        if ($from === 'people') {
            return url('/e/'.$uuid.'/admin/people');
        }

        $me = Auth::user();

        return ($me && app(\App\Events\Support\EventAccess::class)->canManage($event, $me))
            ? url('/e/'.$uuid.'/admin/people')
            : route('events.public.section', ['event' => $uuid, 'section' => 'participants']);
    }

    /**
     * Where BACK goes on a public profile opened from the PLATFORM.
     *
     * This used to be a hardcoded `route('me.people')`, so every reader was
     * dropped on Find People no matter which door they came through — an
     * organiser tapping a name on an event's entry list, somebody following a
     * bracket, a face in the feed. Only one of those actually came from Find
     * People.
     *
     * `history.back()` is still refused for the reason written on the button
     * itself: an empty history (a shared link, a fresh tab) has nowhere to go,
     * and inside a sealed event the gesture escapes the seal because the
     * rewriter only touches href attributes.
     *
     * So the page the browser says it came from, and it must be a page on THIS
     * host — a referer is attacker-supplied like any other header, and echoing
     * one into an href unchecked is an open redirect. Only the path and query
     * survive; scheme, host, port, and any userinfo are discarded rather than
     * trusted, so nothing can be smuggled through in them.
     *
     * A missing referer is the normal case, not a failure: a shared link, a
     * QR code, a privacy-minded browser. Those fall through to Find People,
     * which is where this always went.
     */
    private function platformBackUrl(Request $request): string
    {
        $default = route('me.people');
        ...
        // Same origin, compared whole — a host check alone lets
        // http://takeone.bh.evil.test through on a sloppy match, and lets a
        // downgraded scheme through on a strict one.
        $origin = ($parts['scheme'] ?? '').'://'.$parts['host'].(isset($parts['port']) ? ':'.$parts['port'] : '');

        if (! hash_equals($request->getSchemeAndHttpHost(), $origin)) {
            return $default;
        }

        $path = $parts['path'] ?? '/';

        // A back button that reloads the page you are on is worse than one that
        // goes somewhere: it looks broken. Reached by a refresh, where the
        // browser sends the page's own URL as the referer.
    ...
    public function show(Request $request, string $uuid)
    {
        ...
        // Shared rather than passed in $data because showInEvent() calls this
        // same method: inside a sealed event `$sealedBack` is already shared
        // and the view prefers it, so this is only ever the platform answer.
        view()->share('platformBack', $this->platformBackUrl($request));
        ...
        /*
         * …and the clubs this person has COMPETED FOR.
         *
         * The third source, and the one that was missing: an athlete entered
         * into a competition by an organiser has neither an affiliation nor a
         * membership, but the entry names the club they fought for — either a
         * tenant (`representing_tenant_id`) or a club written down for that one
         * event. Without this, somebody who competed under a club crest on the
         * wall board yesterday reads as having no club at all, which is not a
         * blank the reader can interpret — it looks like a bug, and was one.
         *
         * Added AFTER the real ones and de-duplicated against them, so a club
         * the person actually belongs to is still shown as a membership.
         */
        $competitionClubs = \App\Members\Support\CompetitionClubs::forUser(
            $person,
            $activeAffil->pluck('tenant_id')->merge($pastAffil->pluck('tenant_id'))
                ->filter()->map(fn ($id) => (int) $id)->all(),
            $activeAffil->concat($pastAffil)
                ->map(fn ($a) => mb_strtolower(trim((string) ($a->club_name ?? ''))))
                ->filter()->all(),
        );

        $activeAffil = $activeAffil->concat($competitionClubs)->values();
        ...
        $verifiedMedals = \App\Members\Models\TournamentEvent::where('user_id', $person->id)
            ->where('verification_status', \App\Members\Models\TournamentEvent::STATUS_VERIFIED)
        ...
        $vouchable = \App\Members\Models\TournamentEvent::where('user_id', $person->id)
            ->whereIn('verification_status', [\App\Members\Models\TournamentEvent::STATUS_SELF_REPORTED, \App\Members\Models\TournamentEvent::STATUS_PENDING])
        ...
        $isFamily = $me !== null && \App\Members\Models\UserRelationship::where(fn ($q) => $q->where('guardian_user_id', $me->id)->where('dependent_user_id', $person->id))
        ...
        $compBouts = $compRegIds->isEmpty()
            ? collect()
            /*
             * `contested()` — and this is the whole of the second half of the
             * rule above. A draw is full of rows carrying `status = done` and a
             * winner nobody fought: a BYE is one competitor, no opponent, marked
             * decided so the winner advances. Counting those made a first-round
             * bye read as a victory over "opponent TBD", which flatters the
             * athlete and misleads the reader. Both corners must have been
             * filled for a bout to be part of anybody's record.
             */
            : EventMatch::contested()
         ...
         * never "verified".
         */
        $certifications = \App\Members\Models\MemberCertification::where('user_id', $person->id)
         ...
         * The portrait.
         *
         * A member who never set a profile picture may still have a face on
         * this platform: the photograph an organiser took of them at a
         * competition desk, which lives on the entry row and nowhere else. The
         * entry list showed it and this profile did not, one tap apart.
         *
         * EntryPhoto::publicFaceFor() falls back to that picture only when the
         * competition already publishes it — the same test App\Support\FileAccess
         * applies before serving the bytes — so nothing becomes visible here
         * that was not visible on the list the reader arrived from. A member
         * who HAS chosen a profile picture always wins, and one who hid theirs
         * (profile_picture_is_public off) still has it withheld by FileAccess.
         */
        $avatarPath = $person->profile_picture
            ?: EntryPhoto::publicFaceFor((int) $person->id);

        $avatarUrl = $avatarPath
            ? file_url($avatarPath).'?v='.optional($person->updated_at)->timestamp
            : null;
        ...
        return view($isMobile ? 'members::people.mobile.show' : 'members::people.desktop.show', $data);
    }
    ...
    private function sportsPractised(User $person, array $competitionBySport = []): \Illuminate\Support\Collection
    {
        ...
        $grades = \App\Members\Models\SkillAcquisition::where('user_id', $person->id)
            ->where('verification_status', \App\Members\Models\SkillAcquisition::STATUS_VERIFIED)
