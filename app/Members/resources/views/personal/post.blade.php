...
<div x-data="postPage()" class="min-h-screen bg-background pb-12">
        ...
        <div class="flex items-center justify-between gap-3 relative z-10 max-w-xl mx-auto">
            <div class="flex items-center gap-3 min-w-0">
                <button type="button" onclick="history.length > 1 ? history.back() : (window.location.href='{{ route('me.home') }}')"
                        class="m-press shrink-0 inline-flex items-center w-10 h-10 justify-center rounded-full bg-white/20 border border-white/30 backdrop-blur text-white text-sm font-semibold active:scale-95 transition-transform" aria-label="{{ __('shared.back') }}">
    ...
    <div class="max-w-xl mx-auto sm:mt-4">
        <div class="mobile-stagger">
            @include('members::personal.partials.post-card', ['bare' => true])
        </div>
    </div>
    ...
    @include('members::personal.partials.post-viewers-modal')
</div>
@endsection
