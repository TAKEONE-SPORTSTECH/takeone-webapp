...
<div x-data="getAppHub()" x-init="init()" class="max-w-md mx-auto space-y-5">

    {{-- ===== App identity ===== --}}
    <div class="m-card relative overflow-hidden rounded-3xl border border-gray-100 shadow-sm bg-white p-6 text-center">
        ...
        <div class="relative">
            <div class="w-20 h-20 mx-auto rounded-[1.4rem] bg-primary/10 grid place-items-center mb-3 overflow-hidden ring-1 ring-primary/10">
    ...
    {{-- ===== Loading ===== --}}
    <div x-show="mode==='loading'" x-cloak class="m-card rounded-3xl border border-gray-100 shadow-sm bg-white p-8 flex flex-col items-center gap-3">
    ...
    <div x-show="mode==='browser'" x-cloak class="space-y-4">
        <div class="m-card rounded-3xl border border-gray-100 shadow-sm bg-white p-6">
            ...
            @if($apkExists)
                <a :href="apkUrl" download
                   class="mt-4 w-full inline-flex items-center justify-center gap-2 bg-primary text-white px-4 py-3 rounded-xl font-semibold hover:bg-primary/90 transition-colors shadow-sm">
                    <i class="bi bi-download text-lg"></i> {{ __('personal.personal_get_app_download_android') }}
                </a>
                <p class="text-[11px] text-muted-foreground mt-2 text-center">{{ __('personal.personal_get_app_version_line', ['version' => $versionName]) }}</p>
            @else
                <div class="mt-4 w-full inline-flex items-center justify-center gap-2 bg-muted text-muted-foreground px-4 py-3 rounded-xl font-semibold cursor-not-allowed">
        ...
        {{-- Install steps --}}
        <div class="m-card rounded-3xl border border-gray-100 shadow-sm bg-white p-5">
        ...
        <p class="text-center text-[11px] text-muted-foreground px-4">
            {{ __('personal.personal_get_app_already_installed') }}
        </p>
    </div>

    {{-- ===== Installed & up to date ===== --}}
    <div x-show="mode==='uptodate'" x-cloak class="m-card rounded-3xl border border-emerald-100 shadow-sm bg-gradient-to-br from-emerald-50 to-white p-6 text-center">
    ...
    <div x-show="mode==='update'" x-cloak class="space-y-4">
        <div class="m-card rounded-3xl border border-amber-200 shadow-sm bg-gradient-to-br from-amber-50 to-white p-6 text-center">
        ...
        {{-- What's new --}}
        <div x-show="notes" x-cloak class="m-card rounded-3xl border border-gray-100 shadow-sm bg-white p-5">
...
{{-- Inline (not @push) so it re-runs on mobile shell swaps. `window.` avoids redeclare errors. --}}
<script>
