...
<{{ $tag }} @if($clubUrl) href="{{ $clubUrl }}" @endif
    ...
    <div style="flex:1;min-width:0">
        ...
        <p style="margin:3px 0 0;font-size:11px;color:#8a8fa3">
            {{-- A club the person COMPETED for carries its own line ("represented
                 at <event>") instead of a joining date, because it is a different
                 fact and "member since" would state something untrue. An ordinary
                 affiliation has no `note` and reads exactly as it always has. --}}
            @if(!empty($a->note))
                {{ $a->note }}
            @elseif($active)
    ...
    @if(!empty($a->badge))
        {{-- Same chip shape, different word: a competition entry is not an
             active membership and must not borrow its green dot. --}}
        <span style="flex-shrink:0;font-size:10px;font-weight:700;padding:4px 9px;border-radius:999px;background:#f2effe;color:#6d4bd8">{{ $a->badge }}</span>
    @elseif($active)
