...
<{{ $tag }} @if($clubUrl) href="{{ $clubUrl }}" @endif
    ...
    <div class="min-w-0 flex-1">
        ...
        <p class="text-[11px] text-muted-foreground mt-0.5">
            {{-- See the mobile card: a club the person COMPETED for says so,
                 rather than claiming a joining date it does not have. --}}
            @if(!empty($a->note))
                {{ $a->note }}
            @elseif($active)
    ...
    @if(!empty($a->badge))
        <span class="shrink-0 px-2 py-0.5 rounded-full text-[10px] font-semibold bg-accent text-primary">{{ $a->badge }}</span>
    @elseif($active)
