...
<div class="px-4 sm:px-6 lg:px-8 py-6" x-data="peopleSearch(@js($suggestions))">
    ...
    <div class="relative mb-6">
        ...
        <input type="search" x-model="q" @input.debounce.300ms="run()"
               class="w-full pl-10 rtl:pl-3 rtl:pr-10 pr-3 py-2.5 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent"
    ...
    <div class="flex items-center justify-between mb-3">
        ...
        <div class="inline-flex bg-gray-100 rounded-lg p-0.5" role="tablist">
            <button type="button" role="tab" @click="setView('list')" :aria-selected="view === 'list'"
                    :class="view === 'list' ? 'bg-white shadow text-primary' : 'text-gray-500 hover:text-gray-700'"
                    class="px-3 h-8 rounded-md flex items-center gap-1.5 text-sm font-medium transition-colors">
                <i class="bi bi-list-ul"></i><span>{{ __('personal.view_list') }}</span>
            </button>
            <button type="button" role="tab" @click="setView('cards')" :aria-selected="view === 'cards'"
                    :class="view === 'cards' ? 'bg-white shadow text-primary' : 'text-gray-500 hover:text-gray-700'"
                    class="px-3 h-8 rounded-md flex items-center gap-1.5 text-sm font-medium transition-colors">
                <i class="bi bi-grid-3x3-gap-fill"></i><span>{{ __('personal.view_cards') }}</span>
            </button>
        </div>
    </div>
    ...
    <template x-if="loading">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <template x-for="i in 6" :key="i">
                <div class="bg-white rounded-xl border border-gray-100 p-4 flex items-center gap-3 animate-pulse">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 flex items-center gap-3">
                    <a :href="p.profile_url" class="flex items-center gap-3 min-w-0 flex-1 no-underline">
                        <span class="w-12 h-12 rounded-full overflow-hidden flex-shrink-0 grid place-items-center bg-accent">
                            <template x-if="p.avatar"><img :src="p.avatar" alt="" x-on:error="p.avatar = null" class="w-9 h-12 object-cover"></template>
                    ...
                    <button type="button" @click="toggleFollow(p)"
                            ...
                            :class="p.is_following ? 'bg-muted text-muted-foreground' : 'bg-primary text-white hover:bg-primary/90'"
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 flex flex-col items-center text-center">
                    <a :href="p.profile_url" class="no-underline w-full">
                        <span class="w-20 h-20 rounded-2xl overflow-hidden grid place-items-center bg-accent mx-auto">
                            <template x-if="p.avatar"><img :src="p.avatar" alt="" x-on:error="p.avatar = null" class="w-[60px] h-20 object-cover"></template>
                    ...
                    <p class="text-[11px] text-muted-foreground truncate w-full mt-0.5 min-h-[16px]" x-text="p.reason || (p.is_trainer ? '{{ __('personal.people_trainer') }}' : '')"></p>
                    <button type="button" @click="toggleFollow(p)"
                            class="w-full mt-3 text-xs font-semibold py-2 rounded-lg transition-colors"
                            :class="p.is_following ? 'bg-muted text-muted-foreground' : 'bg-primary text-white hover:bg-primary/90'"
                            x-text="p.is_following ? '{{ __('personal.following') }}' : '{{ __('personal.follow') }}'"></button>
                </div>
            </template>
