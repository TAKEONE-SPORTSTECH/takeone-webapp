...
<div class="tf-container">
        ...
        <div class="flex">
            ...
            <div class="relative" style="width: 180px; aspect-ratio: 3 / 4; border-start-start-radius: 0.375rem; border-end-start-radius: 0.375rem;">
                ...
                @if($relationship->relationship_type == 'self' || Auth::id() == $relationship->guardian_user_id || $relationship->relationship_type == 'admin_view')
                    <button type="button"
                            onclick="event.preventDefault(); window.dispatchEvent(new CustomEvent('open-photo-edit-modal'));"
                ...
                <div class="flex justify-between items-start mb-2">
                    ...
                    @endif
                            ...
                            @if($selfReportedTotal > 0)
                                <div class="flex items-center gap-1.5 mb-3 text-xs text-gray-400" title="{{ __('Self-reported achievements are shown on the profile but are not counted as verified until a club confirms them.') }}">
    ...
    <div class="tab-content" id="profileTabsContent">
                                    ...
                                    <div class="flex items-center mb-3">
                                        ...
                                        <div class="flex-1">
                                            ...
                                            @if($goal->status === 'completed' && $goal->days_taken !== null)
                                                <div class="mt-2 text-success small fw-semibold"><i class="bi bi-trophy-fill me-1"></i>{{ $goal->days_taken }} {{ __('member.days_to_achieve') }}</div>
                                            @endif
                                        </div>
                                    </div>
                                </div>
        ...
        <div class="tab-pane fade" id="affiliations" role="tabpanel">
            @include('members::family.partials.derived-history', [
                'rows' => $derivedAffiliations ?? collect(),
                'kind' => 'affiliations',
            ])

            @include('members::family.partials.affiliations-enhanced')
        </div>
            ...
            <form id="goalEditForm" method="POST">
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        ...
                        <div class="col-12 hidden" id="goalEditAfterPhotoWrap">
                            <label for="goal_edit_after_photo" class="form-label">{{ __('member.goal_after_photo') }} <span class="text-danger">*</span></label>
                            <p class="text-muted small mb-1">{{ __('member.mark_as_achieved_hint') }}</p>
                            <input type="file" accept="image/*" id="goal_edit_after_photo" class="form-control">
                            <input type="hidden" name="after_proof" id="goal_edit_after_photo_b64">
                            <img id="goal_edit_after_photo_preview" class="hidden mt-2 h-24 rounded-md object-cover border" alt="">
                        </div>
                    </div>
                </div>
        ...
        function patchGoalCard(goal) {
            ...
            if (goal.status === 'completed' && (goal.before_proof || goal.after_proof) && !card.querySelector('[data-goal-photos]')) {
                ...
                if (badges) {
                    ...
                    photos.innerHTML =
                        (goal.before_proof ? '<div class="flex-1 min-w-0"><img src="' + goal.before_proof + '" class="w-full h-20 rounded-md object-cover border" alt=""><small class="text-muted block text-center mt-1">{{ __("member.before") }}</small></div>' : '') +
                        (goal.after_proof ? '<div class="flex-1 min-w-0"><img src="' + goal.after_proof + '" class="w-full h-20 rounded-md object-cover border" alt=""><small class="text-muted block text-center mt-1">{{ __("member.after") }}</small></div>' : '');
                    badges.insertAdjacentElement('afterend', photos);

                    if (goal.days_taken !== null && goal.days_taken !== undefined) {
                        const daysEl = document.createElement('div');
                        daysEl.className = 'mt-2 text-success small fw-semibold';
                        daysEl.innerHTML = '<i class="bi bi-trophy-fill me-1"></i>' + goal.days_taken + ' {{ __("member.days_to_achieve") }}';
                ...
                col.innerHTML =
                    '<div class="card shadow-sm border-0 h-full relative" id="goal-' + goal.id + '">' +
                        ...
                        '<div class="card-body p-4">' +
                            ...
                            (goal.before_proof
                                ? '<div class="flex gap-2 mt-3"><div class="flex-1 min-w-0"><img src="' + goal.before_proof + '" class="w-full h-20 rounded-md object-cover border" alt=""><small class="text-muted block text-center mt-1">{{ __("member.before") }}</small></div></div>'
                                : '') +
                        '</div>' +
                    '</div>';
                return col;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Read chosen evidence into a hidden base64 field (server re-sniffs real bytes; SVG rejected).
    function readTournamentEvidence(input) {
        ...
        if (!file) return;
        if (!/^image\/(jpeg|png|webp|gif)$/.test(file.type)) { window.showToast && window.showToast('error', '{{ __('Unsupported image type.') }}'); input.value = ''; return; }
        if (file.size > 5 * 1024 * 1024) { window.showToast && window.showToast('error', '{{ __('Image is too large (max 5MB).') }}'); input.value = ''; return; }
        const reader = new FileReader();
        reader.onload = e => {
    ...
    function buildVerifyBlock(t) {
        ...
        if (v.evidence_url) html += `<a href="${escapeHtml(v.evidence_url)}" target="_blank" rel="noopener" class="inline-flex items-center gap-1 text-[11px] text-gray-500 hover:text-primary"><i class="bi bi-paperclip"></i>{{ __('Evidence') }}</a>`;
        if (v.can_request && v.request_url) html += `<button type="button" data-verify-btn onclick="requestAchievementVerification(this)" data-verify-url="${escapeHtml(v.request_url)}" class="inline-flex items-center gap-1 text-[11px] font-medium text-primary hover:underline"><i class="bi bi-patch-check"></i>{{ __('Request verification') }}</button>`;
        return html;
    }
    async function requestAchievementVerification(btn) {
        ...
        try {
            ...
            } else { btn.disabled = false; window.showToast && window.showToast('error', data.message || '{{ __('Could not request verification.') }}'); }
        } catch (e) { btn.disabled = false; window.showToast && window.showToast('error', '{{ __('Something went wrong.') }}'); }
    }
    function patchVerifyRow(row, v) {
        ...
        if (['verified', 'pending'].includes(v.status)) { const b = row.querySelector('[data-verify-btn]'); if (b) b.remove(); }
    }
