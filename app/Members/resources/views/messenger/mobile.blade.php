...
<header class="sticky top-0 z-40 bg-white border-b border-border">
    <div class="flex items-center gap-2 px-3 h-14">
        <button type="button" onclick="history.length > 1 ? history.back() : (window.location.href='{{ route('me.home') }}')" class="flex items-center justify-center w-10 h-10 rounded-xl flex-shrink-0" aria-label="{{ __('shared.back') }}">
            <i class="bi bi-chevron-left text-xl text-foreground"></i>
        </button>
...
<div class="px-3 pt-3 pb-24">
    ...
    <div x-show="searchOpen" x-cloak class="fixed inset-0 z-[60] bg-background flex flex-col"
        ...
        <div class="flex items-center gap-2 px-3 py-3 border-b border-border bg-white">
            <button type="button" class="w-9 h-9 rounded-lg flex items-center justify-center m-press" @click="closeSearch()"><i class="bi bi-chevron-left text-xl"></i></button>
    ...
    <div x-show="threadOpen" x-cloak class="fixed inset-0 z-[60] bg-background flex flex-col"
        ...
        <div class="flex items-center gap-3 px-2.5 py-2.5 border-b border-border bg-white">
            <button type="button" class="w-9 h-9 rounded-lg flex items-center justify-center m-press" @click="closeThread()"><i class="bi bi-chevron-left text-xl"></i></button>
            <span class="shrink-0">
            ...
            <div class="relative" x-data="{ o: false }">
                ...
                <div x-show="o" x-cloak @click.outside="o = false"
                    ...
                    <a x-show="partner.uuid" :href="partner.uuid ? ('/member/' + partner.uuid) : '#'" @click="o = false" class="w-full flex items-center gap-2.5 px-3 py-2.5 text-sm text-foreground m-press text-left no-underline"><i class="bi bi-person-circle"></i> {{ __('messenger.view_profile') }}</a>
