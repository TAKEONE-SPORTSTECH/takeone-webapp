...
<div class="px-4 sm:px-6 lg:px-8 py-4">
    ...
    <div class="flex justify-between items-center mb-4">
        ...
        <div>
            <button onclick="window.history.back()" class="border border-primary text-primary bg-transparent w-10 h-10 justify-center rounded-md inline-flex items-center text-sm font-medium hover:bg-primary hover:text-white transition-colors"
           aria-label="{{ __('shared.back') }}" title="{{ __('shared.back') }}">
    ...
    <div x-data="{ activeTab: (window.location.hash ? window.location.hash.substring(1) : 'overview') }"
        ...
        <div x-show="activeTab === 'affiliations'" x-transition id="affiliations" role="tabpanel">
            ...
            @include('members::profile.partials.derived-history', [
                'rows' => $derivedAffiliations ?? collect(),
                'kind' => 'affiliations',
            ])

            @include('members::profile.partials.affiliations-enhanced')
        </div>
                                                ...
                                                <div class="flex gap-1.5 mt-1.5 flex-wrap">
                                                    ...
                                                    @if($platform && $platform['event']['url'])
                                                        <a href="{{ $platform['event']['url'] }}" class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold bg-accent text-primary hover:bg-primary hover:text-white transition-colors">
                                                            <i class="bi bi-calendar2-event"></i>{{ $platform['event']['title'] }}<i class="bi bi-arrow-right"></i>
                                                        </a>
                                                    @endif
                                                </div>
        ...
        if (docs.length) {
            docsList.innerHTML = '<div class="flex flex-col gap-3">' + docs.map(d =>
                `<div class="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                    ...
                    ${d.file_path ? `<button type="button" data-media-lightbox data-src="/file/${d.file_path}" data-label="${d.type || ''}" class="flex-shrink-0 w-8 h-8 rounded-lg border border-primary text-primary flex items-center justify-center hover:bg-primary hover:text-white transition-colors"><i class="bi bi-eye" style="font-size:0.85rem;"></i></button>` : ''}
                </div>`
            ).join('') + '</div>';
        } else {
