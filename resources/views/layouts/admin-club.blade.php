@extends('layouts.app')

@section('content')
<!-- Mobile Sidebar Toggle -->
<div class="lg:hidden sticky top-16 z-40 bg-background border-b border-border p-4" x-data="{ sidebarOpen: false }">
    <button @click="sidebarOpen = !sidebarOpen"
            class="flex items-center gap-2 px-4 py-2 border border-border rounded-lg bg-white font-semibold cursor-pointer">
        <i class="bi bi-list"></i>
        <span>{{ $club->club_name ?? 'Club Menu' }}</span>
    </button>
</div>

<!-- Club Admin Wrapper -->
<div class="max-w-[1400px] mx-auto px-4 py-5 flex flex-col lg:flex-row gap-5" x-data="{ sidebarOpen: false }">
    <!-- Sidebar -->
    <aside class="w-full lg:w-72 lg:min-w-72 bg-muted/30 p-6 rounded-xl h-fit lg:sticky lg:top-24"
           :class="{ 'hidden lg:block': !sidebarOpen, 'block': sidebarOpen }">
        @if(isset($club) && $club->logo)
            <div class="w-full p-4 flex items-center justify-center mb-4">
                <img src="{{ asset('storage/' . $club->logo) }}" alt="{{ $club->club_name }}" class="max-w-full max-h-30 object-contain">
            </div>
        @else
            <h2 class="text-lg font-bold text-foreground px-2 pb-4 text-center">{{ $club->club_name ?? 'Club Admin' }}</h2>
        @endif

        <!-- Action Buttons -->
        <div class="flex items-center justify-center gap-2 pb-4 mb-4 border-b border-border">
            <a href="{{ route('admin.platform.clubs') }}"
               class="w-9 h-9 rounded-lg flex items-center justify-center bg-transparent text-foreground hover:bg-accent transition-all"
               title="Back to Clubs">
                <i class="bi bi-arrow-left"></i>
            </a>
            <a href="{{ route('clubs.explore') }}"
               class="w-9 h-9 rounded-lg flex items-center justify-center bg-transparent text-foreground hover:bg-accent transition-all"
               title="Preview Club" target="_blank">
                <i class="bi bi-eye"></i>
            </a>
            <button class="w-9 h-9 rounded-lg flex items-center justify-center bg-transparent text-foreground hover:bg-accent transition-all cursor-pointer"
                    title="Send Notification">
                <i class="bi bi-send"></i>
            </button>
        </div>

        <nav class="flex flex-col gap-1">
            @php
                $clubId = $club->id ?? null;
                $currentRoute = request()->route()->getName();
            @endphp

            <a href="{{ route('admin.club.dashboard', $clubId) }}"
               class="flex items-center gap-3 px-4 py-3 rounded-lg transition-all no-underline text-sm font-medium
                      {{ $currentRoute === 'admin.club.dashboard'
                         ? 'bg-primary text-white shadow-lg'
                         : 'text-foreground hover:bg-muted' }}">
                <i class="bi bi-speedometer2 w-5 text-lg"></i>
                <span>Dashboard</span>
            </a>

            <a href="{{ route('admin.club.details', $clubId) }}"
               class="flex items-center gap-3 px-4 py-3 rounded-lg transition-all no-underline text-sm font-medium
                      {{ $currentRoute === 'admin.club.details'
                         ? 'bg-primary text-white shadow-lg'
                         : 'text-foreground hover:bg-muted' }}">
                <i class="bi bi-building w-5 text-lg"></i>
                <span>Club Details</span>
            </a>

            <a href="{{ route('admin.club.gallery', $clubId) }}"
               class="flex items-center gap-3 px-4 py-3 rounded-lg transition-all no-underline text-sm font-medium
                      {{ $currentRoute === 'admin.club.gallery'
                         ? 'bg-primary text-white shadow-lg'
                         : 'text-foreground hover:bg-muted' }}">
                <i class="bi bi-images w-5 text-lg"></i>
                <span>Gallery</span>
            </a>

            <a href="{{ route('admin.club.facilities', $clubId) }}"
               class="flex items-center gap-3 px-4 py-3 rounded-lg transition-all no-underline text-sm font-medium
                      {{ $currentRoute === 'admin.club.facilities'
                         ? 'bg-primary text-white shadow-lg'
                         : 'text-foreground hover:bg-muted' }}">
                <i class="bi bi-geo-alt w-5 text-lg"></i>
                <span>Facilities</span>
            </a>

            <a href="{{ route('admin.club.instructors', $clubId) }}"
               class="flex items-center gap-3 px-4 py-3 rounded-lg transition-all no-underline text-sm font-medium
                      {{ $currentRoute === 'admin.club.instructors'
                         ? 'bg-primary text-white shadow-lg'
                         : 'text-foreground hover:bg-muted' }}">
                <i class="bi bi-people w-5 text-lg"></i>
                <span>Instructors</span>
            </a>

            <a href="{{ route('admin.club.activities', $clubId) }}"
               class="flex items-center gap-3 px-4 py-3 rounded-lg transition-all no-underline text-sm font-medium
                      {{ $currentRoute === 'admin.club.activities'
                         ? 'bg-primary text-white shadow-lg'
                         : 'text-foreground hover:bg-muted' }}">
                <i class="bi bi-activity w-5 text-lg"></i>
                <span>Activities</span>
            </a>

            <a href="{{ route('admin.club.packages', $clubId) }}"
               class="flex items-center gap-3 px-4 py-3 rounded-lg transition-all no-underline text-sm font-medium
                      {{ $currentRoute === 'admin.club.packages'
                         ? 'bg-primary text-white shadow-lg'
                         : 'text-foreground hover:bg-muted' }}">
                <i class="bi bi-box w-5 text-lg"></i>
                <span>Packages</span>
            </a>

            <a href="{{ route('admin.club.members', $clubId) }}"
               class="flex items-center gap-3 px-4 py-3 rounded-lg transition-all no-underline text-sm font-medium
                      {{ $currentRoute === 'admin.club.members'
                         ? 'bg-primary text-white shadow-lg'
                         : 'text-foreground hover:bg-muted' }}">
                <i class="bi bi-person-plus w-5 text-lg"></i>
                <span>Members</span>
            </a>

            <a href="{{ route('admin.club.roles', $clubId) }}"
               class="flex items-center gap-3 px-4 py-3 rounded-lg transition-all no-underline text-sm font-medium
                      {{ $currentRoute === 'admin.club.roles'
                         ? 'bg-primary text-white shadow-lg'
                         : 'text-foreground hover:bg-muted' }}">
                <i class="bi bi-shield-check w-5 text-lg"></i>
                <span>Roles</span>
            </a>

            <a href="{{ route('admin.club.financials', $clubId) }}"
               class="flex items-center gap-3 px-4 py-3 rounded-lg transition-all no-underline text-sm font-medium
                      {{ $currentRoute === 'admin.club.financials'
                         ? 'bg-primary text-white shadow-lg'
                         : 'text-foreground hover:bg-muted' }}">
                <i class="bi bi-currency-dollar w-5 text-lg"></i>
                <span>Financials</span>
            </a>

            <a href="{{ route('admin.club.messages', $clubId) }}"
               class="flex items-center gap-3 px-4 py-3 rounded-lg transition-all no-underline text-sm font-medium
                      {{ $currentRoute === 'admin.club.messages'
                         ? 'bg-primary text-white shadow-lg'
                         : 'text-foreground hover:bg-muted' }}">
                <i class="bi bi-chat-dots w-5 text-lg"></i>
                <span>Messages</span>
            </a>

            <a href="{{ route('admin.club.analytics', $clubId) }}"
               class="flex items-center gap-3 px-4 py-3 rounded-lg transition-all no-underline text-sm font-medium
                      {{ $currentRoute === 'admin.club.analytics'
                         ? 'bg-primary text-white shadow-lg'
                         : 'text-foreground hover:bg-muted' }}">
                <i class="bi bi-bar-chart w-5 text-lg"></i>
                <span>Analytics</span>
            </a>
        </nav>
    </aside>

    <!-- Content -->
    <main class="flex-1 min-w-0">
        @yield('club-admin-content')
    </main>
</div>
@endsection
