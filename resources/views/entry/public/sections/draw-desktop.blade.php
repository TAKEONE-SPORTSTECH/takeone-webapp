...
<div x-data="{ divisions: [], division: null }"
    ...
    <div class="px-5 pt-2.5">
        ...
        <div x-show="divisions.length > 1" x-cloak class="mt-4 -mx-1 px-1 overflow-x-auto">
            <div class="flex items-center gap-2 w-max">
                <template x-for="d in divisions" :key="d.id">
                    <button type="button" @click="window.BracketBoard.show(d.id)"
                            class="flex items-center gap-2 px-3.5 py-2 rounded-full border text-xs font-bold whitespace-nowrap transition-colors"
                            :class="d.id === division ? 'text-white shadow-sm' : 'bg-white text-foreground border-gray-200 hover:bg-muted/60'"
    ...
    @if($e['draw']['published'] ?? false)
        ...
        <p class="px-5 pb-5 text-[11.5px] text-muted-foreground/80 leading-relaxed">{{ __('events.public_draw_note') }}</p>
    @else
        {{-- Not drawn yet. A 70vh board saying "no draw" is a lot
             of empty page; one honest line is the answer. --}}
