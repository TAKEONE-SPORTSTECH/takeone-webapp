...
<button @click="switcher = !switcher" type="button" class="flex items-center gap-1.5 max-w-full" {{ $hasBusiness ? '' : 'disabled' }}>
    <span class="text-[10px] text-muted-foreground font-medium flex items-center gap-1">
        ...
        {{ $current === 'business' ? __('header.club_management') : __('header.personal_view') }}
...
<div x-show="switcher" @click.outside="switcher=false" x-cloak x-transition.opacity
    ...
    <p class="px-3 py-2 text-[10px] font-semibold text-muted-foreground uppercase tracking-wide">{{ __('header.switch_view') }}</p>
    ...
    @if($current === 'personal')
        <div class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg bg-accent/60">
            ...
            <span class="flex-1 text-left"><span class="block text-sm font-medium text-foreground">{{ __('header.personal_view') }}</span><span class="block text-[11px] text-muted-foreground">{{ __('header.personal_desc') }}</span></span>
    ...
    @else
        <form action="{{ route('view.switch') }}" method="POST">@csrf<input type="hidden" name="mode" value="personal">
            <button type="submit" class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-accent transition-colors text-left">
                <span class="w-8 h-8 rounded-full bg-muted flex items-center justify-center"><i class="bi bi-person text-foreground"></i></span>
                <span class="flex-1"><span class="block text-sm font-medium text-foreground">{{ __('header.personal_view') }}</span><span class="block text-[11px] text-muted-foreground">{{ __('header.personal_desc') }}</span></span>
            </button>
        </form>
    @endif
    ...
    @if($current === 'business')
        <div class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg bg-accent/60">
            ...
            <span class="flex-1 text-left"><span class="block text-sm font-medium text-foreground">{{ __('header.club_management') }}</span><span class="block text-[11px] text-muted-foreground truncate">{{ $businessName }}</span></span>
            <i class="bi bi-check-lg text-primary"></i>
        </div>
    @else
        <form action="{{ route('view.switch') }}" method="POST">@csrf<input type="hidden" name="mode" value="business">
            <button type="submit" class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-accent transition-colors text-left">
                <span class="w-8 h-8 rounded-full bg-accent flex items-center justify-center"><i class="bi bi-building text-primary"></i></span>
                <span class="flex-1"><span class="block text-sm font-medium text-foreground">{{ __('header.club_management') }}</span><span class="block text-[11px] text-muted-foreground truncate">{{ $businessName }}</span></span>
            </button>
        </form>
    @endif
</div>
