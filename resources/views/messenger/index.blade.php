...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4" x-data="messenger()" x-init="init()">
    ...
    <div class="flex items-center justify-between mb-4">
        <div>
            ...
            <p class="text-sm text-muted-foreground" x-text="connected ? 'Connected · realtime' : 'Messages deliver instantly'"></p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden grid grid-cols-1 lg:grid-cols-3"
        ...
        <aside class="lg:col-span-1 border-r border-gray-100 flex flex-col min-h-0"
            ...
            <div class="p-3 border-b border-gray-100 relative">
                <div class="relative">
                    <i class="bi bi-search absolute left-3 top-1/2 -translate-y-1/2 w-4 text-gray-400"></i>
                    <input type="text" x-model="searchTerm" @input.debounce.300ms="searchUsers()"
                           placeholder="Search people to message…"
                           class="w-full pl-9 pr-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent text-sm">
                </div>
                {{-- search results dropdown --}}
                <div x-show="searchResults.length > 0" x-cloak
                     class="absolute left-3 right-3 top-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-20 max-h-72 overflow-y-auto">
                    <template x-for="u in searchResults" :key="u.id">
                        <button type="button" @click="startWith(u)"
                                class="w-full flex items-center gap-3 p-2.5 hover:bg-muted/50 text-left">
            ...
            <div class="flex-1 overflow-y-auto" id="conv-list">
                @forelse($conversations as $c)
                    <button type="button"
                            class="conv-row w-full flex items-center gap-3 p-3 border-b border-gray-50 hover:bg-muted/50 transition-colors text-left"
                            data-conv-id="{{ $c->id }}"
                        ...
                        <span class="flex-1 min-w-0">
                            <span class="flex justify-between items-center">
                                ...
                                <span class="text-[11px] text-muted-foreground shrink-0 ml-2 conv-time">{{ $c->last_at_human }}</span>
                            </span>
                            <span class="block text-xs text-muted-foreground truncate conv-preview">{{ $c->last_mine ? 'You: ' : '' }}{{ $c->last_body }}</span>
                        </span>
                ...
                <div id="conv-empty" class="text-center py-16 px-4 {{ count($conversations) ? 'hidden' : '' }}">
                    ...
                    <p class="text-sm text-muted-foreground mt-2">No conversations yet.<br>Search above to start one.</p>
                </div>
            </div>
        </aside>
        ...
        <section class="lg:col-span-2 flex flex-col min-h-0" :class="{ 'hidden lg:flex': activeId === null }">
            ...
            <div class="flex-1 flex flex-col items-center justify-center text-center p-8" x-show="activeId === null">
                ...
                <h5 class="mt-3 font-semibold text-gray-700">Your messages</h5>
                <p class="text-sm text-muted-foreground">Pick a conversation or search for someone to start chatting.</p>
            </div>
            ...
            <template x-if="activeId !== null">
                <div class="flex flex-col h-full min-h-0">
                    <div class="flex items-center gap-3 px-4 py-3 border-b border-gray-100">
                        <button type="button" class="lg:hidden p-1 -ml-1" @click="closeThread()"><i class="bi bi-arrow-left text-xl"></i></button>
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="text-[11px] text-muted-foreground mb-0" x-text="connected ? 'Active now' : ''"></p>
                        </div>
                        <div class="ml-auto relative" x-data="{ o: false }">
                            <button type="button" @click="o = !o" class="w-9 h-9 rounded-lg flex items-center justify-center text-muted-foreground hover:bg-muted" aria-label="Chat options"><i class="bi bi-three-dots-vertical"></i></button>
                    ...
                    <div class="flex-1 overflow-y-auto px-4 py-4 space-y-1.5 bg-muted/20" id="thread-scroll">
                        <template x-if="loadingThread"><div class="text-center text-muted-foreground text-sm py-8"><i class="bi bi-arrow-repeat"></i> Loading…</div></template>
                            ...
                            <div class="flex group" :class="m.mine ? 'justify-end' : 'justify-start'">
                                <div class="relative max-w-[72%]">
                                    <div class="rounded-2xl shadow-sm overflow-hidden"
                                        ...
                                        <template x-if="!m.deleted && m.kind === 'audio' && m.attachment">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <a :href="m.attachment.url" :download="m.attachment.name" class="shrink-0 w-8 h-8 rounded-full bg-black/10 hover:bg-black/20 flex items-center justify-center" title="Download"><i class="bi bi-download"></i></a>
                                            </div>
                                        </template>
                                        {{-- file attachment --}}
                                    ...
                                    <button type="button" x-show="!m.deleted && !m.kind" @click="toggleMenu(m.id)"
                                            ...
                                            :class="m.mine ? '-left-7' : '-right-7'" aria-label="Message actions">
                                    ...
                                    <div x-show="menuId === m.id" x-cloak @click.outside="menuId = null"
                                        ...
                                        <button type="button" @click="copyMsg(m)" class="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-foreground hover:bg-muted/60 text-left"><i class="bi bi-clipboard"></i> Copy</button>
                                        <template x-if="m.can_edit">
                                            <button type="button" @click="startEdit(m)" class="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-foreground hover:bg-muted/60 text-left"><i class="bi bi-pencil"></i> Edit</button>
                                        </template>
                                        <button type="button" @click="deleteForMe(m)" class="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-foreground hover:bg-muted/60 text-left"><i class="bi bi-eye-slash"></i> Delete for me</button>
                                        <template x-if="m.mine && !m.deleted">
                                            <button type="button" @click="deleteMsg(m)" class="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-red-600 hover:bg-red-50 text-left"><i class="bi bi-trash"></i> Delete for everyone</button>
                                        </template>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div x-show="editing" x-cloak class="flex items-center gap-2 px-3 pt-2 -mb-1 text-xs text-primary">
                        ...
                        <button type="button" @click="cancelEdit()" class="ml-auto text-muted-foreground hover:text-foreground"><i class="bi bi-x-lg"></i></button>
                    </div>
                    ...
                    <form class="flex items-end gap-2 px-3 py-3 border-t border-gray-100" @submit.prevent="editing ? saveEdit() : send()">
                        ...
                        <button type="button" @click="$refs.fileInput.click()" :disabled="editing"
                                class="w-10 h-10 shrink-0 rounded-full text-muted-foreground hover:bg-muted flex items-center justify-center transition-colors disabled:opacity-40" aria-label="Attach file">
                            <i class="bi bi-paperclip text-lg"></i>
                        </button>
                        <textarea x-ref="composer" x-model="draft" rows="1" placeholder="Aa" @keydown.enter.prevent="editing ? saveEdit() : send()" @keydown.escape="cancelEdit()"
    ...
    return {
        ...
        onIncoming(detail) {
            ...
            if (detail.action === 'edit' || detail.action === 'delete') {
                ...
                if (detail.is_latest) {
                    this.updateRow(id, detail.action === 'delete' ? 'This message was deleted' : (detail.body || ''), false);
                }
                return;
            }
        ...
        ensureRow(id, user) {
            ...
            b.type = 'button';
            b.className = 'conv-row w-full flex items-center gap-3 p-3 border-b border-gray-50 hover:bg-muted/50 transition-colors text-left';
            ...
            b.innerHTML =
                ...
                `<span class="font-semibold text-sm truncate">${esc(name)}</span><span class="text-[11px] text-muted-foreground shrink-0 ml-2 conv-time">now</span></span>` +
        ...
        updateRow(id, text, mine) {
            const row = document.querySelector(`.conv-row[data-conv-id="${id}"]`);
            if (!row) return;
            const p = row.querySelector('.conv-preview'); if (p) p.textContent = (mine ? 'You: ' : '') + text;
            const t = row.querySelector('.conv-time'); if (t) t.textContent = 'now';
