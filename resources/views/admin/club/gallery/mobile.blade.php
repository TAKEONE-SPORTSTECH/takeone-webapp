...
@section('title', ($club->club_name ?? 'Club') . ' · Gallery')
...
<div class="space-y-4">
    ...
    @if($club->youtube_url)
        <div class="m-card p-4 flex items-center gap-3">
            ...
            <a href="{{ $club->youtube_url }}" target="_blank" class="m-press text-sm text-primary font-medium truncate">Watch club video</a>
        </div>
    @endif
    ...
    @if($images->isEmpty())
        <div class="m-card p-8 text-center">
            ...
            <p class="text-sm text-muted-foreground mt-2">No images yet.</p>
        </div>
    @else
    ...
    <p class="text-xs text-muted-foreground text-center px-4">Upload &amp; reorder images from the desktop view.</p>
</div>
@endsection
