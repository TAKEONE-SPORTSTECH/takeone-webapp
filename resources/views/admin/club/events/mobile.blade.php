...
@section('title', ($club->club_name ?? 'Club') . ' · Events')
...
<div class="space-y-4 mobile-stagger">
    ...
    @if($events->isEmpty())
        <div class="m-card p-8 text-center">
            ...
            <p class="text-sm text-muted-foreground mt-2">No events yet.</p>
        </div>
    @else
        @foreach($events as $e)
            <div class="m-card p-4 {{ $e->is_archived ? 'opacity-60' : '' }}">
                <div class="flex items-start gap-3">
                    ...
                    <div class="min-w-0 flex-1">
                        <div class="flex items-start justify-between gap-2">
                            ...
                            @if($e->is_archived)<span class="px-2 py-0.5 rounded-full text-[10px] font-medium bg-gray-100 text-gray-500 flex-shrink-0">Archived</span>@endif
                        </div>
                ...
                @if($e->max_capacity)<p class="text-xs text-muted-foreground mt-2"><i class="bi bi-people mr-1"></i>Capacity {{ $e->max_capacity }}</p>@endif
            </div>
        @endforeach
    @endif

    <p class="text-xs text-muted-foreground text-center px-4">Create &amp; manage events from the desktop view.</p>
</div>
@endsection
