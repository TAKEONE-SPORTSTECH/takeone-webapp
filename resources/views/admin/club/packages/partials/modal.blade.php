<!-- Unified Add / Edit Package Modal -->
<div x-show="showPackageModal"
     x-cloak
    ...
    <!-- Backdrop -->
    <div class="fixed inset-0 bg-black/50" @click="showPackageModal = false"></div>
            ...
            <div class="modal-body px-6 py-6 max-h-[65vh] overflow-y-auto">
                <form id="packageForm" action="{{ route('admin.club.packages.store', $club->slug) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div x-show="currentTab === 'basic'" x-cloak>
                        ...
                        <!-- Availability Period -->
                        <div class="card border-0 shadow-sm" x-data="{ alwaysAvailable: true }">
                            <div class="card-body p-4">
                        ...
                        <div class="card border-0 shadow-sm">
                            <div class="card-body p-4">
                                ...
                                <div class="rounded-lg border border-border p-4 mb-4 mt-4 bg-muted/10">
                                    <div class="mb-4">
                                        <x-schedule-time-picker id="packageSchedule" :required="false" />
                                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateSchedulesUI() {
        ...
        document.querySelectorAll('.edit-schedule').forEach(btn =>
            btn.addEventListener('click', () => editScheduleEntry(parseInt(btn.dataset.index)))
        );
    ...
    function editScheduleEntry(index) {
        ...
        ScheduleTimePicker.setSelectedDays(schedulePickerId, s.days);
        ScheduleTimePicker.setStartTime(schedulePickerId, s.startTime);
        ScheduleTimePicker.setEndTime(schedulePickerId, s.endTime);
    ...
    window.updateTrainerAssignmentsUI = function() {
        const activityMap = {};
        schedules.forEach(s => {
            if (s.activityId && !activityMap[s.activityId]) {
                activityMap[s.activityId] = { id: s.activityId, name: s.activityName };
            }
        });
        ...
        document.querySelectorAll('.trainer-assignment').forEach(sel =>
            sel.addEventListener('change', function() {
                if (this.value) trainerAssignments[this.dataset.activityId] = this.value;
                else delete trainerAssignments[this.dataset.activityId];
    ...
    window.resetPackageForm = function() {
        ...
        form.action  = '{{ route('admin.club.packages.store', $club->slug) }}';
        document.getElementById('packageFormMethod').value = 'POST';
        ...
        document.getElementById('packageMaxAge').value      = '';
        document.getElementById('packageCapacity').value    = '';
        ...
        if (preview.length) {
            preview.html(`<div id="preview_packageImageCropper" class="cropper-preview-placeholder" style="width:300px;height:169px;border-radius:8px;"><i class="bi bi-image" style="font-size:2rem;"></i></div>`);
            preview.removeClass('has-image');
        }
        $('#hiddenInput_packageImageCropper').val('');
    };

    // ── Populate form for Edit mode ───────────────────────────────────────────
    window.populatePackageForm = function(pkg) {
        const form = document.getElementById('packageForm');
        form.action = `{{ url('admin/club/' . $club->slug . '/packages') }}/${pkg.id}`;
        document.getElementById('packageFormMethod').value = 'PUT';
        ...
        document.getElementById('packageMaxAge').value      = pkg.age_max || '';
        ...
        if (pkg.activities && pkg.activities.length > 0) {
            const dayAbbr = { saturday:'Sat', sunday:'Sun', monday:'Mon', tuesday:'Tue', wednesday:'Wed', thursday:'Thu', friday:'Fri' };
            pkg.activities.forEach(activity => {
                if (activity.instructor_id) {
                    trainerAssignments[activity.id] = activity.instructor_id;
                }
                if (activity.schedule && activity.schedule.length > 0) {
                    ...
                    activity.schedule.forEach(s => {
                        ...
                        const key   = `${start}-${end}`;
                        if (!timeGroups[key]) {
                            timeGroups[key] = { days: [], startTime: start, endTime: end, facilityId: s.facility_id || '', facilityName: s.facility_name || '' };
                        }
                        timeGroups[key].days.push({ value: day.toLowerCase(), name: dayAbbr[day.toLowerCase()] || day.substring(0, 3) });
                    });
                    Object.values(timeGroups).forEach(group => {
                        schedules.push({ id: Date.now() + Math.random(), days: group.days, startTime: group.startTime, endTime: group.endTime, activityId: String(activity.id), activityName: activity.title || activity.name, facilityId: group.facilityId, facilityName: group.facilityName, notes: '' });
                    });
                } else {
                    schedules.push({ id: Date.now() + Math.random(), days: [], startTime: '', endTime: '', activityId: String(activity.id), activityName: activity.title || activity.name, facilityId: '', facilityName: '', notes: '' });
                }
            });
        }
