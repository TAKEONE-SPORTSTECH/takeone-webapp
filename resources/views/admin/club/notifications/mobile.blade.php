...
@section('title', ($club->club_name ?? 'Club') . ' · Notifications')
...
<div class="space-y-4">
    ...
    @if($notifications->isEmpty())
        <div class="m-card p-8 text-center">
            ...
            <p class="text-sm text-muted-foreground mt-2">No notifications sent yet.</p>
        </div>
    @else
            ...
            @foreach($notifications as $n)
                <div class="m-card p-4">
                    ...
                    <div class="flex items-center gap-3 mt-2 text-[11px] text-muted-foreground">
                        <span><i class="bi bi-person mr-1"></i>{{ $n->sender->full_name ?? 'System' }}</span>
                        <span><i class="bi bi-people mr-1"></i>{{ $n->recipient_type === 'all' ? 'All members' : ($n->recipient_count ?? 0).' selected' }}</span>
                    </div>
                </div>
            @endforeach
        </div>
