...
<div class="{{ $triggerOnly ? '' : 'px-4 mt-4' }}"
    ...
    <div class="rounded-2xl p-4 bg-white border border-border">
        <p class="text-xs font-bold uppercase tracking-wide text-muted-foreground flex items-center gap-1.5">
            <i class="bi bi-arrow-left-right text-primary"></i> Substitute trainer
        </p>
        ...
        <template x-if="current">
            <div class="mt-3 rounded-xl p-3 flex items-center gap-3 bg-amber-50 border border-amber-100">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <p class="text-[11px] text-muted-foreground">Covering on {{ $occurrenceLabel }}</p>
                </div>
                <button type="button" @click="remove()" :disabled="saving"
                        class="m-press text-xs font-bold text-red-500 px-2 py-1 rounded-lg hover:bg-red-50">Remove</button>
            </div>
        </template>

        <p class="text-sm text-muted-foreground mt-2" x-show="!current">
            Can’t make it on {{ $occurrenceLabel }}? Assign someone to cover this class — from your club or anyone on the platform.
        </p>
        ...
        <button type="button" @click="openSheet()"
            ...
            <span x-text="current ? 'Change substitute' : 'Assign a substitute'"></span>
        </button>
    </div>
    @endunless
        ...
        <div x-show="open" x-cloak
            ...
            <div class="flex-shrink-0 px-5 pt-3 pb-3 border-b border-border/70 rounded-t-3xl text-white"
                ...
                <div class="flex items-center justify-between mt-3">
                    <div class="min-w-0">
                        <h2 class="text-base font-black leading-tight truncate">Assign a substitute</h2>
                        <p class="text-[11px] text-white/80 truncate">Search by name, email, phone or ID</p>
                    </div>
            ...
            <div class="flex-1 overflow-y-auto px-4 py-4 space-y-4">
                {{-- Date --}}
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Date to cover</label>
                ...
                {{-- Search --}}
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Find a trainer</label>
                    <div class="relative">
                        <i class="bi bi-search absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"></i>
                        <input type="text" x-model="q" @input.debounce.300ms="search()" placeholder="Name, email, phone or ID…"
                ...
                <template x-if="selected">
                    <div class="rounded-xl p-3 flex items-center gap-3 border-2" :style="`border-color:${color}`">
                        ...
                        <button type="button" @click="selected=null" class="m-press text-xs font-semibold text-muted-foreground px-2 py-1">Change</button>
                    </div>
                </template>
                ...
                <div class="space-y-2" x-show="!selected">
                    <template x-for="u in results" :key="u.id">
                        <button type="button" @click="!u.busy && (selected=u)" :disabled="u.busy"
                                class="m-press w-full text-left rounded-xl p-3 flex items-center gap-3 bg-white border border-gray-100 transition-colors"
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                <p class="text-[11px] truncate" :class="u.busy ? 'text-red-500' : 'text-muted-foreground'"
                                   x-html="u.busy ? ('Busy — ' + (u.busy_reason || 'has a session then')) : ratingHtml(u)"></p>
                            </div>
                            <span x-show="u.busy" class="text-[10px] font-bold px-2 py-0.5 rounded-full bg-red-50 text-red-500 flex-shrink-0">Busy</span>
                    ...
                    <div x-show="q.length>0 && !searching && results.length===0" class="text-center py-8 text-sm text-muted-foreground">
                        ...
                        <p class="mt-2">No one found for “<span x-text="q"></span>”.</p>
                    </div>
                    <div x-show="searching" class="text-center py-6 text-sm text-muted-foreground"><i class="bi bi-arrow-repeat animate-spin"></i> Searching…</div>
                    <div x-show="q.length===0" class="text-center py-8 text-sm text-muted-foreground">
                        <i class="bi bi-people text-2xl text-gray-300"></i>
                        <p class="mt-2">Search for the trainer who’ll cover this class.</p>
                    </div>
                </div>
            </div>
            ...
            <div class="flex-shrink-0 px-4 py-3 border-t border-border bg-white" style="padding-bottom: calc(0.75rem + env(safe-area-inset-bottom));">
                <button type="button" @click="save()" :disabled="saving || !selected"
                        ...
                        :style="`background:${color}`">
                    <i class="bi" :class="saving ? 'bi-arrow-repeat animate-spin' : 'bi-check2-circle'"></i> Assign substitute
                </button>
            </div>
        </div>
    </div>
    ...
    function substitutePicker(cfg) {
        return {
            ...
            ratingHtml(u) {
                if (!u || !u.rating) return '<span class="text-gray-400">No ratings yet</span>';
                const c = u.rating_count ? (' · ' + u.rating_count + (u.rating_count === 1 ? ' rating' : ' ratings')) : '';
            ...
            async save() {
                ...
                try {
                    ...
                    if (!res.ok || !data.success) { window.showToast('error', data.message || 'Could not assign.'); this.saving = false; return; }
                    window.showToast('success', data.message || 'Substitute assigned.');
                    this.open = false;
                    this._backToList();
            ...
            async remove() {
                ...
                try {
                    ...
                    const data = await res.json().catch(() => ({}));
                    if (!res.ok || !data.success) { window.showToast('error', data.message || 'Could not remove.'); this.saving = false; return; }
                    window.showToast('success', data.message || 'Substitute removed.');
                    this._backToList();
                } catch (e) { window.showToast('error', 'Network error — please try again.'); }
                finally { this.saving = false; }
            },
