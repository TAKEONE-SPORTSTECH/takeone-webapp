...
<div x-data="{ mode: 'versus', seg: 'active' }" class="-mx-4 -mt-4 pb-4">
    ...
    <header class="m-hero px-5 pt-7 pb-12 text-white relative overflow-hidden">
        ...
        <div class="flex gap-2 mt-5 relative z-10">
            ...
            <div class="flex-1 rounded-2xl bg-white/12 border border-white/20 backdrop-blur px-3 py-2.5">
                ...
                <p class="text-[10px] text-white/75 mt-1 uppercase tracking-wide">Record</p>
            </div>
            <div class="flex-1 rounded-2xl bg-white/12 border border-white/20 backdrop-blur px-3 py-2.5">
                <p class="text-lg font-black leading-none">{{ $activeDuels->count() }}</p>
                <p class="text-[10px] text-white/75 mt-1 uppercase tracking-wide">Duels</p>
            </div>
        </div>
    </header>
    ...
    <div x-show="mode==='versus'" x-transition class="mt-4">
               ...
               style="background: linear-gradient(135deg, #7c3aed, #ef4444);">
                ...
                <div class="relative flex items-center gap-3">
                    ...
                    <div class="min-w-0 flex-1">
                        <h3 class="font-black text-base leading-tight">Challenge someone</h3>
                        <p class="text-xs text-white/85 mt-0.5">Send a fight or athletic duel invite</p>
                    </div>
            ...
            <div class="px-4 mt-6">
                <div class="flex items-center justify-between mb-2.5">
                    <h2 class="text-sm font-black text-foreground flex items-center gap-2">
                        <i class="bi bi-envelope-paper-heart text-primary"></i> Invitations
                    </h2>
                    <span class="px-2 py-0.5 rounded-full text-[10px] font-bold bg-red-100 text-red-600 animate-pulse">{{ $incoming->count() }} new</span>
                </div>
                <div class="space-y-3">
        ...
        @if($activeDuels->isNotEmpty())
            <div class="px-4 mt-6">
                <h2 class="text-sm font-black text-foreground flex items-center gap-2 mb-2.5">
                    <i class="bi bi-lightning-charge-fill text-primary"></i> Live duels
                </h2>
                <div class="space-y-3">
        ...
        @if($sent->isNotEmpty())
            <div class="px-4 mt-6">
                <h2 class="text-sm font-black text-foreground flex items-center gap-2 mb-2.5">
                    <i class="bi bi-send text-primary"></i> Awaiting reply
                </h2>
                <div class="space-y-3">
                    @foreach($sent as $d)
        ...
        <div class="px-4 mt-6">
            <a href="{{ route('me.challenge.history') }}" data-shell-link data-route="me.challenge"
               class="m-press flex items-center justify-center gap-2 w-full py-3 rounded-2xl bg-white border border-gray-100 text-sm font-bold text-primary">
                <i class="bi bi-trophy"></i> See past results &amp; history
            </a>
        </div>
    </div>
                ...
                <div class="relative p-5">
                    <div class="flex items-center justify-between">
                        <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide bg-white/20 backdrop-blur">
                            <i class="bi bi-fire"></i> {{ $feat['streak'] }} day streak
                        </span>
                        <span class="text-[11px] font-semibold text-white/85">{{ $feat['days_left'] }}d left</span>
                    </div>
                    <h2 class="text-xl font-black mt-3 leading-tight">{{ $feat['title'] }}</h2>
                    <p class="text-sm text-white/85 mt-1 flex items-center gap-1.5">
                        <i class="bi {{ $feat['icon'] }} text-xs"></i>{{ $feat['tag'] }} · {{ $feat['points'] }} pts
                    </p>
                    <div class="mt-4">
                    ...
                    <div class="flex items-start gap-3.5">
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex items-center gap-2">
                                ...
                                @if($c['status'] === 'active')
                                    <span class="px-2 py-0.5 rounded-full text-[10px] font-medium bg-muted text-muted-foreground">{{ $c['days_left'] }}d left</span>
                                @elseif($c['status'] === 'upcoming')
                                    <span class="px-2 py-0.5 rounded-full text-[10px] font-medium bg-amber-50 text-amber-600">Upcoming</span>
                                @else
                                    <span class="px-2 py-0.5 rounded-full text-[10px] font-medium bg-green-50 text-green-600"><i class="bi bi-check2"></i> Done</span>
                                @endif
                            </div>
                            ...
                            <p class="text-xs text-muted-foreground mt-0.5 flex items-center gap-1.5">
                                <i class="bi bi-star-fill text-[10px] text-amber-400"></i>{{ $c['points'] }} pts
            ...
            @foreach(['active','upcoming','completed'] as $st)
                @if(collect($list)->where('status', $st)->isEmpty())
                    <div x-show="seg === '{{ $st }}'" x-transition class="bg-white rounded-2xl border border-gray-100 px-5 py-10 text-center">
                        ...
                        <p class="text-sm text-muted-foreground mt-3">No {{ $st }} challenges right now.</p>
                    </div>
                @endif
            @endforeach
        </div>
    ...
    <script>
        (function () {
            ...
            window.addEventListener('realtime:challenges', function (e) {
                var d = (e && e.detail) || {};
                var map = {
                    'duel:new': (d.actor || 'Someone') + ' challenged you — ' + (d.discipline || ''),
                    'duel:accepted': (d.actor || 'Your rival') + ' accepted your duel 🔥',
                    'duel:declined': (d.actor || 'Your rival') + ' declined your duel',
                    'duel:result': (d.actor || 'Your rival') + ' reported a result',
                };
