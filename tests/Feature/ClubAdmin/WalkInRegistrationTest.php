...
class WalkInRegistrationTest extends TestCase
{
    ...
    public function test_walk_in_charges_registration_fee_by_default(): void
    {
        $owner = $this->createUser();
        $club = $this->createClub($owner);
    ...
    public function test_walk_in_waives_registration_fee_for_legacy_member(): void
    {
        $owner = $this->createUser();
        $club = $this->createClub($owner);
        ...
        // Package subscription is still created; only the joining fee is skipped.
        $this->assertDatabaseHas('club_member_subscriptions', [
            'tenant_id' => $club->id,
            'package_id' => $package->id,
        ]);
        $this->assertDatabaseMissing('club_transactions', [
            'tenant_id' => $club->id,
            'category' => 'registration',
        ]);
    }
    ...
    public function test_walk_in_requires_guardian_fields(): void
    {
        ...
        $this->actingAs($owner)
            ...
            // Email, password and nationality are no longer required for a walk-in.
    ...
    public function test_walk_in_succeeds_without_email_or_password(): void
    {
        $owner = $this->createUser();
        $club = $this->createClub($owner);

        $payload = $this->validPayload();
        unset($payload['guardian']['email'], $payload['guardian']['password'], $payload['guardian']['nationality']);
        ...
        $this->assertDatabaseHas('users', [
            'full_name' => $payload['guardian']['name'],
            'email' => null,
        ]);
    }
